...
// The declaration EXACTLY as it sits on the probe row, for the one place a
// prospect is shown it verbatim (the demo's evidence line). The clause runs to
// the end of its sentence, so both the marker api/novus/probe.js writes today
// and the historical imported wording come back whole. Returns '' when there
// is no declaration — never a paraphrase, never a reconstruction.
export function vendorDeclarationText(probe) {
  const match = String(probe?.enquiry_text || '').match(/declared:\s*has a property to sell[^.!?]*/i);
  if (!match) return '';
