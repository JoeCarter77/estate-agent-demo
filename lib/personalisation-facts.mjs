...
function selectConsequences({ positive, problems }) {
  ...
  if (positiveTypes.has('persistent_follow_up') && problemTypes.has('seller_not_recognised') && buyerWeak) {
    consequences.push(fact('persistence_but_seller_not_recognised', 'There was persistence on the enquiry, but the declared seller opportunity was never recognised.', [{ derived_from: ['persistent_follow_up', 'seller_not_recognised'] }]));
  }
...
export function selectPersonalisationFacts({ findings = [], intelligence = {}, probe = {} } = {}) {
  ...
  if (
    sellerFinding
    && clean(intelligence.seller_recognition).toLowerCase() === 'none'
    && !selectedProblemTypes.has('seller_not_recognised')
  ) {
    secondaryFacts.push(fact(
      'seller_not_recognised_hook_only',
      'The declared seller opportunity was not recognised.',
      [findingSource(sellerFinding), intelligenceSource('seller_recognition'), probeSource('enquiry_text')],
    ));
  }
