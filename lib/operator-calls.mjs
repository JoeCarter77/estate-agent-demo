...
export async function startOperatorCall({
  ...
  const displayName = text(
    contactName
    || agencyRecord.obj?.outreach_contact_name
    || agencyRecord.obj?.contact_name
    || ''
  );
  ...
  // Persist the canonical call row first. The child-leg status callbacks cannot
  // be emitted until Joe answers and <Dial> starts, but writing COMMUNICATIONS
  // before the raw initiation event makes that correlation race even smaller.
