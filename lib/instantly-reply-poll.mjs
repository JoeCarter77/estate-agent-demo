...
export const DEFAULT_POLL_MAX_PAGES = 20;

export function buildReceivedEmailsUrl({ limit = DEFAULT_POLL_LIMIT, startingAfter = '', minTimestampCreated = '' } = {}) {
  const params = new URLSearchParams({
    email_type: 'received',
    limit: String(limit),
    sort_order: 'desc',
  });
  if (String(startingAfter || '').trim()) params.set('starting_after', String(startingAfter).trim());
  if (String(minTimestampCreated || '').trim()) params.set('min_timestamp_created', String(minTimestampCreated).trim());
...
function cursorFromPayload(payload) {
  if (!payload || typeof payload !== 'object') return '';
  return String(payload.next_starting_after ?? '').trim();
}
...
export async function fetchReceivedEmails({
  ...
  maxPages = DEFAULT_POLL_MAX_PAGES,
  minTimestampCreated = '',
  startingAfter = '',
  fetchImpl = globalThis.fetch,
} = {}) {
  if (!apiKey) throw new Error('INSTANTLY_REPLY_API_KEY is not set in this environment.');
  const boundedPages = Math.max(1, Math.min(20, Math.floor(Number(maxPages)) || DEFAULT_POLL_MAX_PAGES));
  const emails = [];
  let cursor = String(startingAfter || '').trim();
  let pages = 0;
  let truncated = false;
  while (pages < boundedPages) {
    const response = await fetchImpl(buildReceivedEmailsUrl({ limit, startingAfter: cursor, minTimestampCreated }), {
    ...
    const page = emailsFromPayload(payload);
    emails.push(...page);
    pages += 1;
    cursor = cursorFromPayload(payload);
    if (!page.length || !cursor) break;
    if (pages >= boundedPages) truncated = true;
  }
  Object.defineProperties(emails, {
    pages: { value: pages, enumerable: false },
    truncated: { value: truncated, enumerable: false },
    next_starting_after: { value: cursor, enumerable: false },
  });
  return emails;
}
...
export const MATCH_METHOD_INSTANTLY_LEAD_ID = 'INSTANTLY_LEAD_ID';
export const MATCH_METHOD_THREAD_HISTORY = 'THREAD_HISTORY';
export const MATCH_METHOD_CAMPAIGN_MEMBER_LEAD_ID = 'CAMPAIGN_MEMBER_LEAD_ID';
export const MATCH_METHOD_CAMPAIGN_MEMBER_EMAIL = 'CAMPAIGN_MEMBER_EMAIL';
...
function exactMatches(outboundRecords, field, value) {
  const key = String(value ?? '').trim().toLowerCase();
  if (!key) return [];
  return (outboundRecords || []).filter((record) => String(record?.obj?.[field] ?? '').trim().toLowerCase() === key);
}

function resultFromCandidates(candidates, matchMethod) {
  const base = { match_method: matchMethod, match: null, candidates: [] };
  if (!candidates.length) return { ...base, status: 'UNMATCHED' };
  if (candidates.length > 1) return { ...base, status: 'AMBIGUOUS', candidates: candidates.map((c) => summariseOutboundRow(c.obj)) };
  return { ...base, status: 'MATCHED', match: summariseOutboundRow(candidates[0].obj) };
}

function summariseCampaignMember(record) {
  const member = record?.obj || {};
  return {
    agency_id: String(member.agency_id ?? '').trim(),
    contact_id: String(member.contact_id ?? '').trim(),
    outbound_id: String(member.outbound_id ?? '').trim(),
    probe_id: String(member.probe_id ?? '').trim(),
    instantly_lead_id: String(member.instantly_lead_id ?? '').trim(),
    campaign_member_id: String(member.member_id ?? '').trim(),
  };
}

function campaignMemberResult(candidates, method) {
  const base = { match_method: method, match: null, candidates: [] };
  if (!candidates.length) return { ...base, status: 'UNMATCHED' };
  if (candidates.length > 1) return { ...base, status: 'AMBIGUOUS', candidates: candidates.map(summariseCampaignMember) };
  return { ...base, status: 'MATCHED', match: summariseCampaignMember(candidates[0]) };
}

// Campaign UI enrolments intentionally do not create legacy OUTBOUND rows.
// Their durable identity is CAMPAIGNS.instantly_campaign_id plus the member's
// Instantly lead ID or exact email. The campaign scope is required before an
// email match so a shared address cannot cross-link two campaigns.
export function matchCampaignMemberDeterministically(campaignRecords, memberRecords, reply) {
  const instantlyCampaignId = String(reply?.campaign_id ?? '').trim();
  const campaignIds = new Set((campaignRecords || [])
    .filter((record) => String(record?.obj?.instantly_campaign_id ?? '').trim() === instantlyCampaignId)
    .map((record) => String(record?.obj?.campaign_id ?? '').trim()).filter(Boolean));
  const scoped = campaignIds.size
    ? (memberRecords || []).filter((record) => campaignIds.has(String(record?.obj?.campaign_id ?? '').trim()))
    : [];
  const leadId = String(reply?.lead_id ?? '').trim();
  if (leadId) {
    const byLeadId = scoped.filter((record) => String(record?.obj?.instantly_lead_id ?? '').trim() === leadId);
    if (byLeadId.length) return campaignMemberResult(byLeadId, MATCH_METHOD_CAMPAIGN_MEMBER_LEAD_ID);
  }
  const email = normalizeEmailKey(reply?.lead_email);
  const byEmail = scoped.filter((record) => normalizeEmailKey(record?.obj?.email) === email);
  return campaignMemberResult(byEmail, MATCH_METHOD_CAMPAIGN_MEMBER_EMAIL);
}

export function matchReplyDeterministically({ outboundRecords, campaignRecords = [], campaignMemberRecords = [], reply, replyEventRecords = [] }) {
  // This campaign has its own scoped member identity. An old OUTBOUND row can
  // share an email address with it, so never let that older record claim the
  // reply or silently supply a different agency/property.
  const probeCampaign = (campaignRecords || []).some((record) =>
    String(record?.obj?.instantly_campaign_id ?? '').trim() === String(reply?.campaign_id ?? '').trim()
    && String(record?.obj?.campaign_type ?? '').trim().toUpperCase() === PROBE_CALL_CAMPAIGN_TYPE);
  if (probeCampaign) return matchCampaignMemberDeterministically(campaignRecords, campaignMemberRecords, reply);
  const legacy = matchOutboundDeterministically(outboundRecords, reply, replyEventRecords);
  if (legacy.status === 'MATCHED' || legacy.status === 'AMBIGUOUS') return legacy;
  const campaign = matchCampaignMemberDeterministically(campaignRecords, campaignMemberRecords, reply);
  return campaign.status === 'UNMATCHED' ? legacy : campaign;
}

// Strongest exact identity first; no fuzzy association is permitted.
export function matchOutboundDeterministically(outboundRecords, reply, replyEventRecords = []) {
  const byLeadId = exactMatches(outboundRecords, 'instantly_lead_id', reply?.lead_id);
  if (byLeadId.length) return resultFromCandidates(byLeadId, MATCH_METHOD_INSTANTLY_LEAD_ID);

  const threadId = String(reply?.thread_id ?? '').trim();
  if (threadId) {
    const outreachIds = [...new Set((replyEventRecords || [])
      .filter((record) => String(record?.obj?.thread_id ?? '').trim() === threadId)
      .map((record) => String(record?.obj?.outreach_id ?? '').trim()).filter(Boolean))];
    if (outreachIds.length) {
      const candidates = (outboundRecords || []).filter((record) => outreachIds.includes(String(record?.obj?.outbound_id ?? '').trim()));
      return resultFromCandidates(candidates, MATCH_METHOD_THREAD_HISTORY);
    }
  }
  return matchOutboundByEmail(outboundRecords, reply?.lead_email);
}
...
async function classifyAndPatch(event, reply, { repo, dryRun, aiCall, summary, contextLoader, matchedOutbound }) {
  ...
  let campaignNotes = {};
  try { campaignNotes = JSON.parse(event.row.notes || '{}'); } catch { /* no campaign metadata */ }
  ...
  if (String(campaignNotes.source_campaign_type || '').toUpperCase() === PROBE_CALL_CAMPAIGN_TYPE && decision.classification === 'CALL_REQUESTED') patch.priority = 'CRITICAL';
  event.classification_patch = patch;

  if (decision.classification === 'CALL_REQUESTED' && context?.previous_novus_full_text) {
    let existingNotes = {};
    try { existingNotes = JSON.parse(event.row.notes || '{}'); } catch { /* preserve a fresh object */ }
    const notes = JSON.stringify({ ...existingNotes, previous_novus_message: context.previous_novus_full_text, context_source: context.context_source });
    try {
      const result = await updateReplyEventExecution(event.row.reply_event_id, { notes }, { repo, dryRun });
      if (result.updated) event.row.notes = notes;
    } catch (err) {
      event.context_persistence_error = err?.message || 'previous message was not stored';
    }
  }
...
// Repairs durable replies that were ingested before a campaign-member match
// was available. It never appends and never calls Instantly: the stored email
// evidence is matched against the current Sheets identity tables, then only
// the blank correlation fields and derived classification fields are updated.
// The caller reconciles ACTIONS afterwards, using the repaired agency ID.
export async function recoverUnresolvedReplyEvents({ repo, dryRun = false, classify = true, aiCall } = {}) {
  const optionalRecords = async (tab, id) => {
    try { return await repo.getRecords(tab, id); } catch { return []; }
  };
  const [outboundRecords, campaignRecords, campaignMemberRecords, replyEventRecords] = await Promise.all([
    repo.getRecords(OUTBOUND_TAB, 'outbound_id'),
    optionalRecords('CAMPAIGNS', 'campaign_id'),
    optionalRecords('CAMPAIGN_MEMBERS', 'member_id'),
    repo.getRecords('REPLY_EVENTS', 'reply_event_id'),
  ]);
  const summary = { examined: 0, matched: 0, unmatched: 0, ambiguous: 0, repaired: 0, classified: 0, affected_agency_ids: [], events: [] };
  for (const record of replyEventRecords) {
    const row = record.obj || {};
    const unresolvedInbound = /^UNRESOLVED_INBOUND_(UNMATCHED|AMBIGUOUS)$/.test(String(row.error ?? '').trim());
    const hasAgency = Boolean(String(row.agency_id ?? '').trim());
    // A prior recovery could have repaired identity while classification was
    // disabled. Finish that durable row once; later classified rows stay
    // idempotently skipped even though their historical error marker remains.
    if (!unresolvedInbound || (hasAgency && String(row.classification ?? '').trim() !== 'OTHER_UNCLEAR')) continue;
    summary.examined += 1;
    const reply = {
      lead_email: row.lead_email, campaign_id: row.campaign_id, thread_id: row.thread_id,
      cleaned_reply_text: row.cleaned_reply_text, body_text: row.body_text, subject: row.subject,
      is_auto_reply: String(row.is_auto_reply).toUpperCase() === 'TRUE',
    };
    const match = matchReplyDeterministically({ outboundRecords, campaignRecords, campaignMemberRecords, reply, replyEventRecords });
    if (match.status !== 'MATCHED') {
      summary[match.status === 'AMBIGUOUS' ? 'ambiguous' : 'unmatched'] += 1;
      summary.events.push({ reply_event_id: row.reply_event_id, match_status: match.status, match_method: match.match_method });
      continue;
    }
    summary.matched += 1;
    const identityPatch = { agency_id: String(match.match?.agency_id ?? '').trim(), outreach_id: String(match.match?.outbound_id ?? '').trim() };
    if (!identityPatch.agency_id) {
      summary.unmatched += 1;
      continue;
    }
    await updateReplyEventMatch(row.reply_event_id, identityPatch, { repo, dryRun });
    Object.assign(row, identityPatch);
    let decision = null;
    if (classify) {
      decision = await classifyReply(reply, aiCall ? { aiCall } : {});
      await updateReplyEventClassification(row.reply_event_id, buildClassificationPatch(decision), { repo, dryRun });
      summary.classified += 1;
    }
    summary.repaired += 1;
    summary.affected_agency_ids.push(identityPatch.agency_id);
    summary.events.push({ reply_event_id: row.reply_event_id, agency_id: identityPatch.agency_id, match_status: match.status, match_method: match.match_method, classification: decision?.classification || '' });
  }
  summary.affected_agency_ids = [...new Set(summary.affected_agency_ids)];
  return summary;
}
...
export async function pollInstantlyReplies({
  ...
  // Classification may perform one additional /emails context sweep. Reserve
  // one request from Instantly's documented 20/min endpoint budget so a full
  // paginated pass cannot rate-limit its own classifier context read.
  const fetchPages = Math.min(Number(maxPages) || DEFAULT_POLL_MAX_PAGES, DEFAULT_POLL_MAX_PAGES - 1);
  const emails = await fetchReceivedEmails({ apiKey, limit, maxPages: fetchPages, minTimestampCreated, startingAfter, fetchImpl });
  ...
  const contextLoader = createThreadContextLoader({ apiKey, limit: contextLimit, fetchImpl, mailboxes });

  const summary = {
    dry_run: !live,
    fetched: emails.length,
    pages: emails.pages || (emails.length ? 1 : 0),
    truncated: Boolean(emails.truncated),
    next_starting_after: emails.truncated ? emails.next_starting_after : '',
    inbound_confirmed: 0,
  ...
  // Identity sources are loaded once for the batch. OUTBOUND covers legacy
  // uploads; CAMPAIGN_MEMBERS covers leads enrolled directly through Campaigns.
  let outboundRecords = [];
  let campaignRecords = [];
  let campaignMemberRecords = [];
  if (emails.length) {
    outboundRecords = await repo.getRecords(OUTBOUND_TAB, 'outbound_id');
    try {
      [campaignRecords, campaignMemberRecords] = await Promise.all([
        repo.getRecords('CAMPAIGNS', 'campaign_id'),
        repo.getRecords('CAMPAIGN_MEMBERS', 'member_id'),
      ]);
    } catch {
      // Campaign tabs predate this matcher in some workbooks. Legacy OUTBOUND
      // matching remains fully available until their Campaigns setup runs.
      campaignRecords = [];
  ...
  if (live && emails.length) {
    ...
    replyEventRecords = loaded.records || [];
  } else if (!live && emails.length) {
    try { replyEventRecords = await repo.getRecords('REPLY_EVENTS', 'reply_event_id'); }
    catch { replyEventRecords = []; }
  }
  ...
  for (const raw of emails) {
    ...
    const matchResult = matchReplyDeterministically({
      outboundRecords, campaignRecords, campaignMemberRecords, reply, replyEventRecords,
    });
    ...
    const decision = matchResult.status === 'MATCHED' ? routeReply(reply) : {
      classification: 'OTHER_UNCLEAR', confidence: null, suppression_type: 'NONE',
      next_action: 'MANUAL_REVIEW', priority: 'CRITICAL',
      reason: `Unresolved inbound reply — needs matching (${matchResult.status})`,
    };
    ...
    const sourceCampaign = campaignRecords.find((record) => String(record?.obj?.instantly_campaign_id ?? '').trim() === String(reply.campaign_id ?? '').trim())?.obj;
    const classifyThis = classify || String(sourceCampaign?.campaign_type || '').toUpperCase() === PROBE_CALL_CAMPAIGN_TYPE;
    const campaignAiCall = classify ? aiCall : async () => { throw new Error('Probe-call campaign uses deterministic classification'); };
    if (sourceCampaign && matchResult.status === 'MATCHED') row.notes = JSON.stringify({ source_campaign_type: String(sourceCampaign.campaign_type || ''), source_campaign_name: String(sourceCampaign.name || ''), probe_id: String(matchResult.match?.probe_id || '') });
    if (matchResult.status !== 'MATCHED') {
      row.error = `UNRESOLVED_INBOUND_${matchResult.status}`;
    ...
    if (!live) {
      ...
      // nothing is written.
      if (classifyThis && matchResult.status === 'MATCHED') {
        await classifyAndPatch(event, reply, {
          repo, dryRun: true, aiCall: campaignAiCall, summary, contextLoader, matchedOutbound: matchResult.match,
        });
      }
    ...
    //
    // Taken after direction/idempotency for every genuine inbound, including
    // unresolved replies. Every accepted inbound now leaves durable evidence.
    //
    ...
    try {
      ...
      if (classifyThis && matchResult.status === 'MATCHED') {
        await classifyAndPatch(event, reply, {
          repo, dryRun: false, aiCall: campaignAiCall, summary, contextLoader, matchedOutbound: matchResult.match,
        });
      }
    } catch (err) {
      summary.failed += 1;
