...
async function classifyAndPatch(event, reply, { repo, dryRun, aiCall, summary, contextLoader, matchedOutbound }) {
  ...
  if (decision.classification === 'CALL_REQUESTED' && context?.previous_novus_full_text && !event.row.notes) {
    const notes = JSON.stringify({ previous_novus_message: context.previous_novus_full_text, context_source: context.context_source });
    try {
...
export async function pollInstantlyReplies({
  ...
  const fetchPages = classify ? Math.min(Number(maxPages) || DEFAULT_POLL_MAX_PAGES, DEFAULT_POLL_MAX_PAGES - 1) : maxPages;
  ...
  for (const raw of emails) {
    ...
    if (!live) {
      ...
      // nothing is written.
      if (classify && matchResult.status === 'MATCHED') {
