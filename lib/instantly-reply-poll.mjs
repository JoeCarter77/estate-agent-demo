...
export function matchReplyDeterministically({ outboundRecords, campaignRecords = [], campaignMemberRecords = [], reply, replyEventRecords = [] }) {
  ...
  const probeCampaign = (campaignRecords || []).some((record) =>
    ...
    && isLockedCampaignType(record?.obj?.campaign_type));
...
async function classifyAndPatch(event, reply, { repo, dryRun, aiCall, summary, contextLoader, matchedOutbound }) {
  ...
  if (isLockedCampaignType(campaignNotes.source_campaign_type) && decision.classification === 'CALL_REQUESTED') patch.priority = 'CRITICAL';
...
export async function pollInstantlyReplies({
  ...
  for (const raw of emails) {
    ...
    const classifyThis = classify || isLockedCampaignType(sourceCampaign?.campaign_type);
    const campaignAiCall = classify ? aiCall : async () => { throw new Error('Locked preset campaigns use deterministic classification'); };
