...
// human) is selectable UNLESS it was found by a live Hunter Finder call this
// run (candidate.source === 'HUNTER'): Finder carries its own confidence that
...
export function verdictForCandidate(candidate, status, score = candidate.hunter_score, threshold = HUNTER_HIGH_CONFIDENCE_SCORE) {
  ...
  if (status === 'RISKY') {
    ...
    if (candidate.source === 'HUNTER') {
      // Hunter Finder scored this specific attribution this run: only a
      ...
      // Discovered directly from AGENCIES/COMMUNICATIONS/CONTACTS: no Finder
