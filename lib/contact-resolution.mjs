...
//                 spent on one agency. Finder metadata is NOT verification.
...
import { findDomainDecisionMakers, findEmail, verifyEmail, isHunterConfigured } from './hunter.mjs';
...
export async function resolveAgencyContact(repo, agencyId, {
  ...
  const {
    attempts, winner, verifier_calls_made: verifierCallsMade,
    verifier_cap: verifierCap, verifier_cap_reached: verifierCapReached,
  } = owner
    ? await runVerificationWaterfall(candidates, { cache, verifyEmailImpl, now, riskyScoreThreshold: threshold })
    : { attempts: [], winner: null, verifier_calls_made: 0, verifier_cap: MAX_HUNTER_VERIFIER_CALLS_PER_AGENCY, verifier_cap_reached: false };
  ...
  } else if (!owner || candidates.length === 0) {
    // Hunter could not identify a suitable named decision-maker. Do not guess
    // or fall back to external research; leave this agency for manual review.
  ...
  if (!dryRun) {
    // A Hunter identity miss must not erase a standing selection or mutate
    // CONTACTS. Only the unresolved status is written so a later batch can
    // safely move on without destroying prior human/provider data.
    if (owner) {
    ...
    const agencyPatch = owner
      ...
      : { contact_resolution_status: status, updated_at: now() };
