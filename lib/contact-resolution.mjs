...
//                 for them.
//   4. VERIFY     candidates in priority order with Hunter, STOPPING at the
//                 first selectable one. Lower
//                 priority candidates after a winner are never verified.
//                 Finder hits use the verification result Hunter already
//                 returned; only addresses from Sheets use Email Verifier.
...
//   - Deliverability is ONLY ever decided by Hunter. INVALID/DISPOSABLE are
//     hard rejects; UNKNOWN is not selectable. Accept-all is stored as RISKY
//     and is selectable only for a clearly senior direct contact with score
//     >= 80, or as the final generic fallback.
...
import { findEmail, verifyEmail, isHunterConfigured } from './hunter.mjs';
...
//
// ACCEPT-ALL RULE. RISKY is selectable for a clearly senior direct contact
// only when Hunter's score is at least the configured (default 80) threshold.
// A generic RISKY address is selectable only because generic addresses are the
// final priority tier. UNKNOWN is never a new selection.
export function verdictForCandidate(candidate, status, score = candidate.hunter_score, threshold = HUNTER_HIGH_CONFIDENCE_SCORE) {
  ...
  if (status === 'RISKY') {
    ...
    if (candidate.clearly_senior_direct && Number.isFinite(numericScore) && numericScore >= threshold) return 'SELECT';
  }
...
export async function runVerificationWaterfall(candidates, { cache, verifyEmailImpl, now, riskyScoreThreshold = HUNTER_HIGH_CONFIDENCE_SCORE }) {
  const attempts = [];
  let winner = null;

  const accept = (candidate, attempt) => {
    attempt.selected_on_caution = Boolean(cautionFor(candidate, attempt.verification_status, attempt.verification_score));
    attempt.caution = cautionFor(candidate, attempt.verification_status, attempt.verification_score);
    winner = { candidate, attempt };
  };

  for (const candidate of candidates) {
    // A standing selection carries its own stored verdict. Honour it without a
    // Hunter call even if it is older than the cache TTL: this run is
    // reconciling a decision, not making a new one.
    if (candidate.prior_selection && candidate.stored_status) {
      const attempt = {
      ...
      if (verdictForCandidate(candidate, attempt.verification_status, attempt.verification_score, riskyScoreThreshold) === 'SELECT') {
    ...
    // Hunter Finder already verified its own result. Use that verdict even
    // when it is UNKNOWN; never spend a second verifier call on a Finder hit.
    if (candidate.source === 'HUNTER') {
      const hasFreshFinderVerdict = Boolean(candidate.hunter_verification_status);
      const status = String(candidate.hunter_verification_status || candidate.stored_status || 'UNKNOWN').toUpperCase();
      const attempt = {
        email: candidate.email,
        priority_label: candidate.priority_label,
        verification_status: status,
        verification_score: candidate.hunter_score,
        verified_at: candidate.hunter_verification_date || candidate.stored_verified_at || now(),
        cached: !hasFreshFinderVerdict,
        hunter_verifier_called: false,
        verification_provider: hasFreshFinderVerdict ? 'HUNTER_FINDER' : 'CONTACTS_HUNTER_FINDER',
        raw_result: candidate.hunter_verification_raw,
      };
      cache.set(candidate.email, {
        verification_status: status,
        verified_at: attempt.verified_at,
        verified_at_ms: Date.parse(attempt.verified_at),
      });
      attempts.push(attempt);
      if (verdictForCandidate(candidate, status, attempt.verification_score, riskyScoreThreshold) === 'SELECT') {
        accept(candidate, attempt);
        break;
      }
      continue;
    ...
    attempts.push(attempt);
    if (verdictForCandidate(candidate, status, score, riskyScoreThreshold) === 'SELECT') { accept(candidate, attempt); break; }
...
async function writeAgencyCells(repo, agencyId, patch) {
  ...
  for (const [column, value] of Object.entries(patch)) {
    ...
    applied[column] = await repo.updateCell('AGENCIES', 'agency_id', agencyId, actualHeader, value);
    if (!applied[column]) missing.push(column);
  }
  if (missing.length) {
...
export async function resolveAgencyContact(repo, agencyId, {
  ...
  const { attempts, winner } = await runVerificationWaterfall(candidates, {
  ...
  if (!dryRun) {
    ...
    agencyWriteback = await writeAgencyCells(repo, id, agencyPatch);
  }
  ...
  return {
    ...
    candidates_verified: attempts.map((a) => ({
      ...
      cached: a.cached,
      hunter_verifier_called: a.hunter_verifier_called,
    ...
    hunter_verifier: {
      calls_made: attempts.filter((a) => a.hunter_verifier_called).length,
      cached_reuses: attempts.filter((a) => a.cached).length,
      finder_result_reuses: attempts.filter((a) => a.verification_provider === 'HUNTER_FINDER').length,
