...
export async function resolveAgencyContact(repo, agencyId, {
  ...
  dryRun = false,
  // Optional bulk-caller precondition, checked against the AGENCIES row below
  // — the same row this call already loads for itself. Lets a bulk run
  // recheck an agency's current status/physical row immediately before
  // resolving without a separate full-sheet read of its own.
  requireStatus = undefined,
  minSheetRow = undefined,
} = {}) {
  ...
  const sheetRowNumber = agencyRecord.rowNumber;

  if (requireStatus !== undefined || minSheetRow !== undefined) {
    const actualStatus = String(agency.contact_resolution_status || '').trim();
    const expectedStatus = String(requireStatus ?? '').trim();
    const rowOk = minSheetRow === undefined || sheetRowNumber >= Number(minSheetRow);
    if (actualStatus !== expectedStatus || !rowOk) {
      const err = new Error('Agency no longer eligible for resolution');
      err.statusCode = 409;
      err.details = { contact_resolution_status: actualStatus, sheet_row_number: sheetRowNumber };
      throw err;
    }
  }
