...
export function verdictForCandidate(candidate, status) {
  ...
  // reconciled, not re-litigated: it keeps its inconclusive verdict rather
  // than being dropped, which is what used to leave AGENCIES blank on rerun.
...
function cautionFor(candidate, status) {
  ...
  const basis = candidate.high_confidence_owner
    ? `high-confidence owner/MD address${score === null || score === undefined ? '' : ` (Hunter score ${score})`}`
    : 'reconciled standing selection from CONTACTS';
  return `Selected on a ${status} NeverBounce result: ${basis} — NOT fully verified.`;
}
...
export async function runVerificationWaterfall(candidates, { cache, verifyEmailImpl, now }) {
  ...
  for (const candidate of candidates) {
    ...
    // NeverBounce call even if it is older than the cache TTL: this run is
