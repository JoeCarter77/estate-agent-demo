...
export function resolvePhone({ agency, calls, actions, contacts = [] }) {
  ...
  if (contact) {
    ...
    if (contactPhone) candidates.push({ phone: text(contactPhone[1]), name: text(contact.contact_name), role: text(contact.contact_role), source: 'CONTACT' });
  }
  ...
  for (const agency of agencies) {
    ...
    const lead = {
      ...
      context: {
        ...
        campaign_name: text(campaignsByInstantlyId.get(text(sourceReply?.campaign_id))?.name || sourceReply?.campaign_id),
        ...
        property: propertyProbe ? resolvePropertyStreet(propertyProbe) : '',
      },
    ...
    for (const action of active) {
      ...
      callActions.push({
        ...
        source: text(meta.source), reply_text: text(meta.reply_text), reply_received_at: text(actionReply?.received_at),
