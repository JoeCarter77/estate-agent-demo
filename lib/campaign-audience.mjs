...
export function buildCampaignAudience(tables, campaignTables, {
  ...
  applyFilters = true, limit = 0, cohortIds = null,
} = {}) {
  const nowMs = Date.parse(now) || Date.now();
  const f = normaliseFilters(filters);
  const preset = lockedPreset(campaign.campaign_type);
  const p = normalisePolicy({ ...policy, requires_probe: preset ? preset.requires_probe : (policy?.requires_probe ?? (upper(campaign.campaign_type || 'ENQUIRY_FOLLOWUP') === 'ENQUIRY_FOLLOWUP')) });
  if (preset) Object.assign(p, { requires_probe: preset.requires_probe, allow_risky_email: false, block_active_campaign: true, block_active_conversation: true, block_active_followup: true, block_prior_negative: true, block_meeting_booked: true, block_opted_out: true });
  ...
  const members = memberRecords(campaignTables?.CAMPAIGN_MEMBERS).map((row) => ({ ...row, campaign_status: upper(campaignsById.get(text(row.campaign_id))?.status), campaign_name: text(campaignsById.get(text(row.campaign_id))?.name), campaign_type: upper(campaignsById.get(text(row.campaign_id))?.campaign_type) }));
  ...
  const selected = cohortIds ? all.filter((row) => cohortIds.includes(row.agency_id) && !f.exclude_agency_ids.includes(row.agency_id))
    : (applyFilters ? all.filter((row) => matchesFilters(row, f)) : all);
  selected.sort((a, b) => {
