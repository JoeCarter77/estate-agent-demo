...
export function buildSupportContext({
  ...
  const sellerDeclaredInEnquiry = hasVendorDeclaration(probe);
  return {
    findingsText,
    ...
    selectedFactsSpanEvents: sellerDeclaredInEnquiry
...
const POSTPOSED_CONTACT_UNIVERSAL = /\b(?:nam(?:e[ds]?|ing)|mention(?:ed|s|ing)?|rais(?:e[ds]?|ing)|referenc(?:e[ds]?|ing)|includ(?:e[ds]?|ing)|acknowledg(?:e[ds]?|ing)|ask(?:ed|s|ing)?|offer(?:ed|s|ing)?)\b[^.!?]{0,48}\b(?:each|every)\s+time\b/i;
const NON_ASSERTIVE_UNIVERSAL = /\?\s*$|\b(?:whether|may|might|could|would|should)\b[^.!?]{0,64}\b(?:each|every)\s+time\b/i;

export function addsUnsupportedUniversal(text, support) {
  const t = String(text || '').trim();
  if (!t || !UNIVERSAL_QUANTIFIER.test(t)) return false;
  if (NON_ASSERTIVE_UNIVERSAL.test(t)) return false;
  if (!CONTACT_SET_SUBJECT.test(t) && !POSTPOSED_CONTACT_UNIVERSAL.test(t)) return false;
...
const METRIC_NUMBER = String.raw`(?:one|two|three|four|five|six|seven|eight|nine|ten|\d{1,2})`;
const CONTACT_ATTEMPT_METRIC_RE = new RegExp(String.raw`\b(${METRIC_NUMBER})\s+contact\s+attempts?\b`, 'i');
const RAW_COMMUNICATION_METRIC_RE = new RegExp(String.raw`\b${METRIC_NUMBER}\s+communications?\b`, 'i');
...
export function addsUnsupportedContactCount(text, support) {
  const t = String(text || '').trim();
  if (!t) return false;
  // Do not weld the raw COMMUNICATIONS-row count to the prospect-facing
  // INTELLIGENCE.contact_attempts metric. Standalone noun-form attempt counts
  // retain their existing behaviour; the forbidden relationship is the mix.
  if (CONTACT_ATTEMPT_METRIC_RE.test(t) && RAW_COMMUNICATION_METRIC_RE.test(t)) return true;
