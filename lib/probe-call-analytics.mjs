...
export function probeCallMetrics(campaign, members, events, tables = {}) {
  ...
  return {
    ...
    // Automated / out-of-office replies are not a human response.
    human_replies: unique(replies.filter((row) => upper(row.classification) !== 'OOO_AUTOMATED'), 'reply_event_id'),
    interested_replies: unique(replies.filter((row) => ['CALL_REQUESTED', 'POSITIVE_MEETING', 'POSITIVE_SEND_DEMO', 'POSITIVE_INTEREST', 'INFO_REQUESTED'].includes(upper(row.classification))), 'reply_event_id'),
