...
// THE INTERPRETATION HALF, REUSABLE ON ITS OWN.
//
// lib/probe-assessment.mjs runs ONE model call that does this reading AND the
// commercial assessment together, so the pieces this module owns — the tool
// properties, the message transcript, the zero-communications answer and the
// quote-validation that turns a raw model result into trustworthy INTELLIGENCE
// fields — are exported rather than locked inside interpretProbe(). There is
// still exactly one implementation of the evidence rule: a quote that is not a
// literal substring of the message it cites is dropped, wherever the call was
// made from.
export const INTERPRETATION_TOOL_PROPERTIES = TOOL.input_schema.properties;
export const INTERPRETATION_REQUIRED_FIELDS = TOOL.input_schema.required;
export const INTERPRETATION_SYSTEM_RULES = SYSTEM_PROMPT;
export { buildPrompt as buildInterpretationPrompt };

// The deterministic answer for a probe that received nothing at all. No model
// call can add anything to "no communication of any kind was received", so it
// never gets one.
export function zeroCommunicationInterpretation(probe) {
  const vendorDeclared = hasVendorDeclaration(probe);
  return {
...
// A raw model result + the communications it claimed to read -> the eight
// validated INTELLIGENCE fields. Every quote is checked against its cited
// message; buyer_qualification is floored from the surviving question count,
// never taken from the model.
export function deriveInterpretationFields(result, probe, communications) {
  const vendorDeclared = hasVendorDeclaration(probe);
...
//      seller_recognition, communication_quality, did_well, missed, evidence }
//
// STANDALONE INTERPRETATION IS NO LONGER PART OF THE PRODUCTION PIPELINE.
// Both the nightly pass and the webhook recompute now reach the model exactly
// once per probe, at close, through lib/probe-assessment.mjs. This function is
// retained for the debug/experimental path and for the existing interpretation
// self-tests, which is why its behaviour is unchanged.
export async function interpretProbe(probe, communications) {
  ...
  // Validation lives in deriveInterpretationFields() so the merged assessment
  // call in lib/probe-assessment.mjs enforces the identical evidence rule.
  return deriveInterpretationFields(result, probe, communications);
}
