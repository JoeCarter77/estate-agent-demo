...
import { normalizeInstantlyEmail, novusAccountsFor, normaliseEmailBodyText } from './reply-router.mjs';
// One-way: reply-classification.mjs does not import this module, so naming the
// CTA type here creates no cycle.
import { previousNovusCtaType } from './reply-classification.mjs';
...
export function extractQuotedNovusParent(rawBodyText, { accounts = [] } = {}) {
  // Normalised FIRST. A live Gmail reply separates quoted lines with <br>, not
  // \n, so without this the per-line ">" stripping below never fires and the
  // recovered parent comes back full of markup.
  const text = normaliseEmailBodyText(rawBodyText);
  if (!text.trim()) return '';
...
export function selectThreadContext(reply, threadIndex, { demoUrl = '', demoSlug = '' } = {}) {
  ...
  const previousNovusMessage = previousNovus
    ? excerpt(previousNovus.cleaned_reply_text || previousNovus.raw_body_text)
    : excerpt(quotedParent);

  return {
    previous_novus_message: previousNovusMessage,
    previous_novus_full_text: previousNovus
      ? String(previousNovus.cleaned_reply_text || previousNovus.raw_body_text || '').slice(0, 12000)
      : quotedParent.slice(0, 12000),
    // The PROPOSITION this reply is answering, named rather than left implicit.
    // A short reply's meaning lives entirely in this value, so it is carried on
    // the context object itself — visible in event.thread_context for audit,
    // and stated to the model in the prompt.
    previous_novus_cta_type: previousNovusCtaType(previousNovusMessage),
