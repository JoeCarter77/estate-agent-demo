...
// is SEPARATE, independently evidence-backed items, so it does not fit
...
// not the raw COMMUNICATIONS. So everything the email can legitimately say
// has to be HERE, as a structured finding — which is why a finding now
// carries a TYPE:
//   'problem'     — something that went wrong (the default, and what every
//                   finding written before this column existed is read as).
//   'opportunity' — commercial value that was sitting in the enquiry and was
//                   never taken (most often the declared seller / valuation).
//   'positive'    — something the agency genuinely did well, structured the
//                   same way as any other finding. These are what Diagnosis's
//                   `strengths` PROSE used to be: the email's fair
//                   observation is now SELECTED from one of these rows, not
//                   distilled out of a paragraph Personalisation had to read.
//
// Live tab header (created by hand, matched literally here):
//   probe_id | finding_index | finding_type | finding | evidence | significance_note
//
// finding_type is written through the same header-driven row builder as every
// other column, so a workbook whose tab does not carry it yet simply drops the
// value and every finding reads back as 'problem' — the pre-existing shape.
//
...
// The three kinds of finding. 'problem' is the default for anything written
// before the column existed, so a legacy row is never silently promoted to a
// positive — the one direction that would put invented praise into an email.
export const FINDING_TYPES = ['positive', 'problem', 'opportunity'];
export const DEFAULT_FINDING_TYPE = 'problem';

// The types that can carry the email's MAIN or WIDER story. A positive can
// never be either: it is the fair observation, and nothing else.
export const STORY_FINDING_TYPES = ['problem', 'opportunity'];

export function normaliseFindingType(value) {
  const t = String(value || '').trim().toLowerCase();
  return FINDING_TYPES.includes(t) ? t : DEFAULT_FINDING_TYPE;
}

export function isPositiveFinding(finding) {
  return normaliseFindingType(finding?.finding_type) === 'positive';
}

// A finding the main/wider story can be built on — i.e. anything that is not
// a positive. Used everywhere "are there any genuine problems?" is asked, so
// adding positives to the list can never make a probe look worse than it is.
export function isStoryFinding(finding) {
  return !isPositiveFinding(finding);
}
...
// -> Map(probe_id -> [{ finding_index, finding_type, finding, evidence, significance_note }])
...
export function groupFindingsByProbe(table) {
  ...
  for (const { obj } of recordsOf(table)) {
    ...
    const item = {
      ...
      // A blank cell (a row written before the column existed) reads as
      // 'problem', never as a positive — see DEFAULT_FINDING_TYPE.
      finding_type: normaliseFindingType(obj.finding_type),
      finding,
      evidence,
...
// always describe a finding the same way. The TYPE is printed on the same
// line as the index, because Personalisation now selects findings BY INDEX
// and its selection is validated against the type — a model that cannot see
// which rows are positives cannot pick a positive for the fair observation.
