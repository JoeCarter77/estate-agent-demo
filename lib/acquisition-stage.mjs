...
// nothing else for probe history. It deliberately does not consult PROBES
// history, the resolved lifecycle stage, probe completion or any downstream
// state: those can all disagree with the sheet (a deleted PROBES row, a
// re-imported agency, a probe logged out of band), and disagreeing with the
// sheet is exactly how an already-probed agency was being handed back to the
// operator.
//
...
// Canonical contact field for outreach. Do not substitute primary_contact_email
// or other_known_emails here — those are not what the outbound campaign sends
// to. A populated address alone is no longer enough to justify a probe.
export function hasOutreachContactEmail(agency) {
  return text(agency?.outreach_contact_email) !== '';
}

// PROBING IS ONLY WORTH DOING FOR AN ADDRESS WE WILL ACTUALLY SEND TO.
// Hunter accept-all addresses are stored as RISKY; UNKNOWN/INVALID/etc. also
// fail this gate. Only a verifier-backed VALID status can enter the Prober.
export function hasValidOutreachContactEmail(agency) {
  return hasOutreachContactEmail(agency) && upper(agency?.email_verification_status) === 'VALID';
}

// SKIPPED IN THE PROBER. A skip is a recorded decision on a kept AGENCIES row
// (probe_skip_status / probe_skip_reason / probe_skipped_at), never a delete
// and never a probe. RESTORED (or blank) puts the agency back in the queue.
export function isProbeSkipped(agency) {
  return upper(agency?.probe_skip_status) === 'SKIPPED';
}

export function isProbeQueueEligible(agency) {
  return isProbeSentBlank(agency) && !isProbeSkipped(agency) && isEligibleForProbe(agency) && hasValidOutreachContactEmail(agency);
}

// WHY A LEAD SITS IN LEAD_POOL, IN OPERATOR LANGUAGE. isProbeQueueEligible is
// a single boolean; this walks the same three gates in the same order to say
// which one is actually failing, so the lead-pool review queue tells a human
// something they can act on instead of the one generic sentence every
// LEAD_POOL agency used to get regardless of cause.
export function leadPoolBlockReason(agency) {
  if (!isProbeSentBlank(agency)) return 'AGENCIES.probe_sent is set but no active probe or outbound record exists';
  if (isProbeSkipped(agency)) return `skipped in the Prober: ${text(agency?.probe_skip_reason) || 'no reason recorded'}`;
  if (!isEligibleForProbe(agency)) {
    const url = text(agency?.rightmove_sales_branch_url);
    if (!/^https?:\/\//i.test(url)) return 'no Rightmove sales branch URL on file';
    if (upper(agency?.suppression_status) === 'SUPPRESSED') return 'agency is suppressed';
    const pipeline = upper(agency?.current_pipeline_status);
    return `agency pipeline status is ${pipeline || 'blank'}`;
  }
  if (!hasOutreachContactEmail(agency)) return 'no outreach email has been resolved for this agency yet';
  return `outreach email verified ${upper(agency?.email_verification_status) || 'UNKNOWN'}, not VALID`;
}
...
export function resolveLifecycleStage(evidence) {
  ...
  if (latest && (!manualAt || latestInboundAt >= manualAt) && (!followupAt || latestInboundAt >= followupAt)) {
    ...
    if (classification === 'CALL_REQUESTED') return { stage: 'MEETING_INTENT', reason: 'latest reply requests a phone call' };
    ...
    if (['POSITIVE_INTEREST', 'QUESTION', 'INFO_REQUESTED', 'OBJECTION', 'OTHER_UNCLEAR', 'NOT_NOW'].includes(classification) || positiveAfterExistingDemo) {
  ...
  if (outbound) {
    if (text(outbound.instantly_lead_id)) {
      // OUTREACH EXECUTION. instantly_lead_id proves only that the lead was
      // HANDED to Instantly. Whether an email actually left is Instantly's
      // fact, supplied read-only by lib/instantly-execution-state.mjs and
      // carried on evidence.execution. Nothing here infers a send from the
      // lead id, and the pre-execution behaviour below is retained verbatim as
      // the fallback for when that read is unavailable.
      const execution = evidence.execution || null;
      if (execution && execution.source === 'INSTANTLY') {
        const sent = Number(execution.emails_sent_count) || 0;
        const step = execution.last_sequence_step;
        const stepNote = step === null ? '' : `, latest campaign step ${step}`;
        const sentNote = `Instantly records ${sent} sent campaign email${sent === 1 ? '' : 's'}${stepNote}`;
        switch (execution.execution_state) {
          case 'WAITING_FIRST_EMAIL':
            return { stage: 'WAITING_FOR_FIRST_EMAIL', reason: 'lead is in Instantly and Instantly records no sent campaign email yet' };
          case 'EMAIL_1_SENT':
            return { stage: 'EMAIL_1_SENT', reason: sentNote };
          case 'FOLLOWUP_1_SENT':
            return { stage: 'FOLLOWUP_1_SENT', reason: sentNote };
          case 'FOLLOWUP_2_SENT':
            return { stage: 'FOLLOWUP_2_SENT', reason: sentNote };
          case 'LATER_STEP_SENT':
            return { stage: 'SEQUENCE_RUNNING', reason: sentNote };
          default:
            break;
        }
      }
  ...
  // A deliberate Prober skip is a decision, not a blocker to sort — it must
  // not raise a SORT_LEAD task. CLOSED derives no action, so reconciliation
  // cancels the open PROBE_AGENCY; restoring the agency re-derives it.
  if (isProbeSkipped(agency)) return { stage: 'CLOSED', reason: `skipped in the Prober: ${text(agency.probe_skip_reason) || 'no reason recorded'}` };
  if (isProbeQueueEligible(agency)) return { stage: 'READY_TO_PROBE', reason: 'agency has an eligible Rightmove sales branch and a verified VALID outreach email' };
  return { stage: 'LEAD_POOL', reason: leadPoolBlockReason(agency) };
}
