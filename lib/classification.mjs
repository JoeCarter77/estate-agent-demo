...
// contact_quality (and the other per-message interpretation fields this
// feeds) is no longer written to COMMUNICATIONS — V2 schema §2.3 moved that
// judgement to the probe-level AI interpretation in
// lib/probe-interpretation.mjs. Exported for direct testing and because
// classifyCommunication() below still uses it internally.
...
export function classifyCommunication(comm, context = {}) {
  ...
  const contentSaysAutomated = isMailerDaemon || isKnownAutoSender || hasAutoAckPhrase || hasTemplateSignal || isTimingProximateAndGeneric;

  // context.assumeAutomatedOrHuman: a human/automated decision already recorded
  // for THIS row (see interpretCommunication below). When supplied it wins over
  // the content signals — a decision already on the sheet, possibly corrected by
  // hand, is never silently re-litigated — but every CONTENT-derived field below
  // is still recomputed consistently with it, which is the whole point of
  // re-running this over already-classified rows.
  const assumed = String(context.assumeAutomatedOrHuman || '').trim().toLowerCase();
  const isAutomated = assumed === 'automated' ? true : assumed === 'human' ? false : contentSaysAutomated;

  // A bounce/system mail is automated but is not an acknowledgement of the enquiry.
  const isAutoAcknowledgement = isAutomated && !isMailerDaemon
    && (isKnownAutoSender || hasAutoAckPhrase || hasTemplateSignal || isTimingProximateAndGeneric);
...
// ── Re-interpretation of ALREADY-CLASSIFIED rows ──────────────────────────────
//
// The integration gap this fixes: both rebuild paths used to skip any
// COMMUNICATIONS row that already carried an automated_or_human value
// ("isClassified" guard). That guard was written when automated_or_human was
// the ONLY thing classification produced. Since then classification also
// produces communication_classification, booking_attempt and contact_quality,
// and lib/vendor-intent.mjs produces intent — so every row classified before
// those detectors existed (i.e. every historical row, and every row from an
// earlier deploy) was permanently frozen with those columns blank. Rebuilding
// Intelligence could never fill them in, no matter how many times it ran.
//
// interpretCommunication() is the replacement: it re-runs the CURRENT
// classification logic over every non-overridden row, previously classified or
// not, while deliberately preserving a human/automated decision already
// recorded for that row (which may have been corrected by hand, and which A-H
// grading depends on — see classifyCommunication's assumeAutomatedOrHuman).
// The content-derived columns are always recomputed consistently with it.

// The human/automated decision already stored on a row: true (human),
// false (automated), or null when the row has never been decided (blank, or
// the deliberately non-committal 'unknown', both of which we want to resolve
// with the current logic rather than preserve).
export function storedHumanFlag(comm) {
  const stored = String(comm.automated_or_human || '').trim().toLowerCase();
  if (stored === 'human') return true;
  if (stored === 'automated') return false;
  const flag = comm.human_contact;
  if (flag === true || flag === 'TRUE' || flag === 'true') return true;
  if (flag === false || flag === 'FALSE' || flag === 'false') return false;
  return null;
}

// True when this row represents contact from an actual person, using the
// stored decision when there is one and the current classifier otherwise.
// Used wherever "human communications only" is the rule (vendor opportunity,
// evidence summary), so a historical row whose human_contact cell was never
// populated still counts as the human touch it plainly is.
export function isHumanCommunication(comm, context = {}) {
  const stored = storedHumanFlag(comm);
  if (stored !== null) return stored;
  return classifyCommunication(comm, context).human_contact;
}

// comm + { probeTimestamp } -> the COMMUNICATIONS patch to write for this row.
// Safe to call on every row on every rebuild: it is a pure function of the
// row's raw content plus its already-stored human/automated decision, so it is
// idempotent (running it twice writes the same values).
export function interpretCommunication(comm, context = {}) {
  const stored = storedHumanFlag(comm);
  const tags = classifyCommunication(comm, {
    ...context,
    ...(stored === null ? {} : { assumeAutomatedOrHuman: stored ? 'human' : 'automated' }),
  });
  return {
    ...
    contact_quality: tags.contact_quality,
  };
}
