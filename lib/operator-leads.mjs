...
export function buildOperatorLeads(tables, { now = new Date().toISOString() } = {}) {
  ...
  // A campaign-layer reply has no OUTBOUND journey but is attached to its lead
  // by agency_id (lib/operator-funnel.mjs), so it is not an orphan.
  const orphanReplyEvents = replyRecords.filter((r) => !knownOutboundIds.has(id(r.obj.outreach_id)) && !id(r.obj.agency_id));
