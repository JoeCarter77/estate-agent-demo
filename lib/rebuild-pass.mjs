...
function buildBacklogReport(tables) {
  ...
  for (const probeId of personalisedProbes) {
    ...
    if (!agency) { outboundBlocked.missing_agency += 1; blocked = true; }
      ...
      else if (text(agency.email_verification_status) !== 'VALID') { outboundBlocked.invalid_email += 1; blocked = true; }
    }
...
export async function runRebuildPass(repo, opts = {}) {
  ...
  // maxAiCalls governs.
  // Retired: no new model assessments, generated outreach copy, or demos.
  // Historical records remain readable; this pass only maintains operations.
  const assessmentSummary = { assessments_created: 0, ai_calls_used: 0, assessed_probe_ids: [] };
  ...
  const personalisationSummary = { personalisation_created: 0, personalised_probe_ids: [] };
  const demosSummary = { demos_compiled: 0, demos_created: 0, remaining_demos: 0, disabled: true };
  ...
  // hold the flag down.
  const complete = backlog.outbound_eligible_waiting === 0;
