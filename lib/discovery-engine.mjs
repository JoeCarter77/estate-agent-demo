...
import {
  ...
  optionOf, levelOf, effectiveLevel, answerValues, isVisible,
...
export function assessDimension(dimensionId, answers = {}, overrides = {}) {
  ...
  for (const condition of rule.answer_conditions || []) {
    ...
    if (condition.not) {
      ...
      if (condition.not.includes(value)) {
        if (condition.effect === 'INFEASIBLE') { feasibility = worse(feasibility, 'INFEASIBLE'); blockers.push(condition.note); }
        else { feasibility = worse(feasibility, 'REQUIRES_ASSESSMENT'); assessment.push(condition.note); }
      }
...
export function diagnose({ answers = {}, overrides = {}, notes = {} } = {}) {
  const assessments = assessDimensions(answers, overrides);
  ...
  const objective = {
    priority: text(answers.C1?.value), priority_label: optionOf(priorityQ, answers.C1?.value)?.label || '',
    obstacles: answerValues(answers.C2).map((value) => ({ value, label: optionOf(obstaclesQ, value)?.label || value })),
    notes: [noteOf(answers, 'C1'), noteOf(answers, 'C2')].filter(Boolean),
  };
  const facts = {
  ...
  const progress = {
    ...
    answered: visible.filter((q) => answered(answers[q.id])).length,
    skipped: visible.filter((q) => answers[q.id]?.skipped).length,
