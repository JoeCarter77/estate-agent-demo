...
const TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      missed_opportunities: { type: 'string', description: 'NON-AUTHORITATIVE — internal/sales-call prose only; no downstream step reads it. Named commercial value — from the BUYING or SELLING opportunity — that was on the table and not taken. Empty string if both were taken.' },
      commercial_implication: { type: 'string', description: 'NON-AUTHORITATIVE — internal/sales-call prose only; no downstream step reads it. What this costs THIS agency. Must contain at least one probe-specific fact (the property, a time, their own words) — never a generic sentence that would fit any agency.' },
