...
Your complete factual universe is the PERSONALISATION_FACTS JSON in the user message. Canonical fact text is authoritative. Do not reinterpret it.
...
COMMERCIAL HOOK
- Use only HOOK_SOURCE, which contains at most one deterministic consequence.
- If HOOK_SOURCE is empty, return an empty string.
- State the consequence plainly. Do not refer to "supported facts", "selected facts" or internal selection logic.

HOOK 2
- Use only HOOK_2_SOURCE: one secondary fact or a different supplied consequence.
- Preserve unknown or incomplete content as unknown or incomplete.
- If HOOK_2_SOURCE is empty, return an empty string.
...
function naturalFactText(fact) {
  ...
  switch (fact?.type) {
    ...
    case 'buyer_not_progressed':
      return 'my buyer enquiry did not reach an invitation, availability request, offered slot or booking';
    ...
    case 'buyer_received_no_further_progression':
      return 'there was no further recorded progress on the buyer enquiry';
    ...
    case 'response_speed_good_but_progression_weak':
      return 'the response came within 16 hours, but the buyer side still showed weak progression or qualification';
...
function fieldSources(facts) {
  const remaining = facts.consequences.slice();
  const hook = remaining.splice(0, 1);
  const hook2 = facts.secondary_facts.length ? facts.secondary_facts.slice(0, 1) : remaining.splice(0, 1);
  return {
    email_observation: [...facts.positive, ...facts.problems],
    email_commercial_hook: hook,
    email_commercial_hook_email_2: hook2,
  };
}
...
export function buildFactConstrainedPrompt(personalisationFacts, repairReasons = []) {
  const facts = validatedFacts(personalisationFacts);
  const sources = fieldSources(facts);
  const gold = renderCanonicalFactCopy(facts);
  return [
    'PERSONALISATION_FACTS:',
...
function factCoverage(output, source) {
  return Math.max(textCoverage(output, source.text), textCoverage(output, naturalFactText(source)));
}

function factualSurfaceFailure(output, sources) {
  const sourceText = sources.map((item) => item.text).join(' ');
  const authorisedNaturalText = sources.map((item) => naturalFactText(item)).join(' ');
...
function relationshipSupport(facts) {
  const all = CATEGORIES.flatMap((category) => facts[category]);
...
export async function personaliseProbeFromFacts(personalisationFacts, { enabled = false } = {}) {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
    const result = await callAi({
      ...
      prompt: buildFactConstrainedPrompt(facts, rejections),
      tool: TOOL,
    });
    const validated = validateFactConstrainedOutput(facts, result);
  ...
  const fallback = renderCanonicalFactCopy(facts);
