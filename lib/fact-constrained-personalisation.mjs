...
function observationHookFrames(observationFacts) {
  ...
  if (has('seller_not_recognised_hook_only')) {
    if (has('slow_human_response')) {
      return [
        frame('observation_commercial_consequence', 'That meant that the enquiry was sitting untouched for more than 16 hours while a potential valuation opportunity in the same message went unacknowledged.'),
        frame('observation_second_commercial_frame', "By the time the enquiry was picked up, the seller opportunity I'd already mentioned still hadn't been addressed."),
      ];
    }
    return [
      frame('observation_commercial_consequence', 'That meant that a potential valuation opportunity in the enquiry was left without any acknowledgement or next step.'),
      frame('observation_second_commercial_frame', "The seller opportunity I'd already mentioned still hadn't been addressed."),
    ];
  }
...
function fieldSources(facts) {
  ...
  const hookFacts = facts.secondary_facts.filter((item) => item.type === 'seller_not_recognised_hook_only');
  const [hook, hook2] = observationHookFrames([...observation, ...hookFacts]);
  return {
