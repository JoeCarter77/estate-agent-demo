...
function naturalFactText(fact) {
  ...
  switch (fact?.type) {
    ...
    case 'strong_handling':
      return 'your team replied promptly, qualified the buyer and offered a specific viewing slot';
    ...
    case 'seller_context_unresolved':
      return "the property I'd mentioned selling was not connected to the move";
...
// ── THE PRODUCTION PATH: DETERMINISTIC, ZERO AI CALLS ───────────────────────
//
// renderCanonicalFactCopy() above already renders both email sentences from the
// canonical PERSONALISATION_FACTS, and it was already the fallback the AI path
// fell back to. The model was only ever being paid to re-word sentences that
// existed before it was called — a surface realiser over facts a deterministic
// selector had already chosen, constrained so tightly by
// validateFactConstrainedOutput() that anything it added was rejected anyway.
//
// So the renderer IS the production path now. What it does NOT skip is the
// safety half: the output goes through the identical
// validateFactConstrainedOutput() gate — internal-language check, one-sentence
// check, word limit, the surface-coverage check, every existing validator
// (seller price attribution, invented loss, unsupported prospect reply, false
// chronology, unsupported voicemail claim, unasked questions, speculative
// prospect behaviour, response-dependent outcome criticism), the relationship
// support check, the forbidden commercial-loss and numeric-impact patterns, and
// the hook-vs-observation check. A rendering that fails any of them is a
// REFUSAL, exactly as an AI rendering that failed them was.
//
// -> { email_observation, email_commercial_hook, rejections, ai_calls_used: 0 }
export function renderPersonalisationFromFacts(personalisationFacts) {
  const facts = validatedFacts(personalisationFacts);
  const rendered = renderCanonicalFactCopy(facts);
  const validated = validateFactConstrainedOutput(facts, rendered);
  return { ...validated.result, rejections: validated.rejections, ai_calls_used: 0 };
}

// THE AI SURFACE REALISER — DEBUG / EXPERIMENTAL ONLY.
//
// No production path calls this any more: not the nightly cron, not the
// "Rebuild Intelligence" button, not the backlog recovery. It is retained
// behind its existing explicit { enabled: true } flag so a wording experiment
// or a comparison against the deterministic renderer is still possible, and so
// the existing dual-path self-tests keep exercising the validator against real
// model-shaped output.
...
export const _internal = {
  TOOL, SYSTEM_PROMPT, MAX_ATTEMPTS, validatedFacts, fieldSources, factualSurfaceFailure, naturalFactText,
