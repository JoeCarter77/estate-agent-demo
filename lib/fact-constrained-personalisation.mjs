...
function observationHookFrames(observationFacts) {
  ...
  if (has('seller_not_recognised')) {
    const hook = frame('observation_commercial_consequence', 'A potential valuation opportunity was left without an agency-controlled next step.');
    if (has('concrete_buyer_next_step')) {
      return [hook, frame('observation_second_commercial_frame', 'The enquiry contained buyer and seller opportunities, but only the buyer side received an agency-controlled next step.')];
    }
    ...
    if (has('human_response_fast') || has('human_response_within_16h') || has('slow_human_response')) {
      return [hook, frame('observation_second_commercial_frame', 'The seller opportunity was left untouched inside an enquiry the agency had already handled.')];
    }
    return [hook, frame('observation_second_commercial_frame', 'The enquiry exposed buyer and seller opportunities without an agency-controlled next step for either side.')];
  }

  if (has('seller_recognised_not_progressed')) {
    return [
      frame('observation_commercial_consequence', 'A recognised seller opportunity was left without a valuation offer.'),
      frame('observation_second_commercial_frame', 'The seller opportunity received recognition but no agency-controlled valuation next step.'),
    ];
  }

  if (has('complete_miss') || has('no_human_response')) {
    return [
      frame('observation_commercial_consequence', 'The enquiry was left without any agency response or next step.'),
      frame('observation_second_commercial_frame', 'The buyer opportunity received no agency-controlled handling during the observation window.'),
    ];
  }

  if (has('buyer_not_progressed')) {
    return [
      frame('observation_commercial_consequence', 'The buyer opportunity was left without an agency-controlled viewing next step.'),
      frame('observation_second_commercial_frame', 'The agency activity did not extend to a concrete viewing offer or availability request.'),
    ];
  }

  if (has('buyer_not_qualified')) {
    return [
      frame('observation_commercial_consequence', 'The buyer opportunity was handled without recorded qualification questions.'),
      frame('observation_second_commercial_frame', 'The agency had no recorded buyer-position information from its own questioning.'),
    ];
  }

  if (has('slow_human_response')) {
    return [
      frame('observation_commercial_consequence', 'The enquiry entered human handling with a material response delay.'),
      frame('observation_second_commercial_frame', 'Delayed response handling was the immediate commercial weakness in the enquiry.'),
    ];
  }

  if (has('communication_record_incomplete')) {
    return [
      frame('observation_commercial_consequence', 'The incomplete record limits what can fairly be concluded about the enquiry handling.'),
      frame('observation_second_commercial_frame', 'Commercial interpretation is constrained by the missing communication detail.'),
    ];
  }

  if (has('concrete_buyer_next_step')) {
    return [
      frame('observation_commercial_consequence', 'The buyer enquiry received a concrete agency-controlled next step.'),
      frame('observation_second_commercial_frame', 'The viewing opportunity moved into a clear agency action.'),
    ];
  }

  return [
    frame('observation_commercial_consequence', 'The enquiry received timely human handling.'),
    frame('observation_second_commercial_frame', 'Response speed was a commercial strength in the enquiry handling.'),
  ];
}
...
export async function personaliseProbeFromFacts(personalisationFacts, { enabled = false } = {}) {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
    ...
    const candidate = lockedObservation ? { ...result, email_observation: lockedObservation } : result;
...
export const _internal = {
  ...
  observationHookFrames, readsAsResponseDependentOutcomeCriticism,
};
