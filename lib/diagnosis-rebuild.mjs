// RETIRED FROM THE PRODUCTION PATH — kept for its regression coverage.
//
// The acquisition pipeline no longer runs a separate AI diagnosis after a
// separate AI interpretation. Both are now ONE call at probe close
// (lib/probe-assessment.mjs), driven by lib/assessment-rebuild.mjs, which is
// what lib/rebuild-pass.mjs and lib/observation-recompute.mjs use. Nothing in
// the nightly cron or the "Rebuild Intelligence" button reaches this file.
//
// It is retained, unchanged, because the existing self-tests that exercise the
// DIAGNOSIS_FINDINGS duplication invariant, the probe_id whitespace handling,
// the persistence contract and the targeting option all run through it, and
// deleting a module to avoid maintaining its tests is not a refactor. Treat it
// as a debug/comparison path: safe to run, never run automatically.
//
