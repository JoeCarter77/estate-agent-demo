...
//
// The AI call (one per probe) runs only when it hasn't run before, or when
// forced — same rule as lib/intelligence-rebuild.mjs — so a rebuild over an
// already-diagnosed, unchanged sheet makes zero AI calls.
//
...
// A row counts as "already diagnosed" once diagnosis_summary is set — every
// diagnosis, including the no-problem-found case, always writes one.
function needsDiagnosis(existingRecord, forceAi) {
...
// prompt, opts?: { forceAi?: boolean }
// -> { diagnoses_processed, diagnosis_created, diagnosis_updated,
//      ai_diagnoses_run, skipped_not_closed, problems }
export async function rebuildAllDiagnosis(repo, probesById, opts = {}) {
  ...
  for (const rec of intelligenceRecords) {
    ...
    try {
      ...
      if (!needsDiagnosis(existingRecord, forceAi)) continue;
