...
//
// Idempotent: DIAGNOSIS is upserted exactly one row per probe_id, and each
// of that diagnosis's findings is upserted as exactly one DIAGNOSIS_FINDINGS
// row per (probe_id, finding_index) — see lib/diagnosis-findings.mjs. Both
// go out in the SAME writeRowsBatch call, so persisting the findings costs
// no extra request and cannot leave a diagnosis written without its findings.

import { newDiagnosisId } from './ids.mjs';
import { diagnoseProbe, parseDiagnosisFindings } from './probe-diagnosis.mjs';
import { loadFindingsTable, findingsTabExists, buildFindingsWrites } from './diagnosis-findings.mjs';
...
export async function rebuildAllDiagnosis(repo, probesById, opts = {}) {
  ...
  let nextFindingsRow = findingsTable.rows.length + 2;
  const allocateFindingsRow = () => { const n = nextFindingsRow; nextFindingsRow += 1; return n; };
  ...
  for (const rec of intelligenceRecords) {
    ...
    try {
      ...
      const findingsWrites = buildFindingsWrites(findingsTable, probeId, findings, allocateFindingsRow);
      findingsWritten += findings.length;
      writes.push(...findingsWrites);
    } catch (err) {
