...
import {
  ...
  ANALYTICS_COLUMNS, buildDemoRow, demoRecords, demosTabExists,
  loadDemosTable, slugOwners,
} from './demos.mjs';
import { buildShortDemoSlug } from './demo-slug.mjs';
...
// story for it. email_observation is a live downstream field and the same
// finalised signal lib/personalisation-rebuild.mjs freezes on. The legacy
// primary_narrative fallback keeps historical rows working.
export function isPersonalised(personalisation) {
  return Boolean(text(personalisation?.email_observation) || text(personalisation?.primary_narrative));
}
...
export function compileDecision({
  personalisation, existing, probe = null, justPersonalised = false, force = false,
}) {
  ...
  // agency_id is copied from PROBES (the source of truth) on every compile —
  // see buildDemoRow. If the two have drifted (the row was compiled before
  // PROBES carried an agency_id, or was hand-edited), recompile so the row
  // self-heals instead of needing a manual fix in the sheet.
  if (text(existing.agency_id) !== text(probe?.agency_id)) return { compile: true, reason: 'agency_id_mismatch' };
...
export async function compileDemos(repo, opts = {}) {
  ...
  const [probesTable, agenciesTable, intelligenceTable, personalisationTable, diagnosisTable, communicationsTable] = await Promise.all([
    repo.getTable('PROBES'),
    repo.getTable('AGENCIES'),
    repo.getTable('INTELLIGENCE'),
    repo.getTable('PERSONALISATION'),
    // Degrades to empty rather than throwing: a workbook (or a fixture) without
    // a DIAGNOSIS tab still compiles demos, falling back to the
    // DIAGNOSIS_FINDINGS projection for its findings exactly as before.
    repo.getTable('DIAGNOSIS').catch(() => ({ header: [], rows: [] })),
  ...
  // FINDINGS COME FROM THE CANONICAL RECORD, NOT THE TAB. Each probe's
  // DIAGNOSIS row carries the whole findings array as JSON — the same items
  // DIAGNOSIS_FINDINGS holds one per row — so the demo reads it from the
  // DIAGNOSIS table it now loads instead of downloading a large granular tab on
  // every pass. DIAGNOSIS_FINDINGS stays the audit projection and is still read
  // LAZILY below, for any historical probe whose canonical cell is blank.
  const diagnosisByProbe = new Map(recordsFromTable(diagnosisTable, 'probe_id').map((o) => [o.probe_id, o]));
  let findingsTabCache = null;
  async function findingsFor(probeId) {
    const canonical = parseDiagnosisFindings(diagnosisByProbe.get(probeId))
      .map((f, i) => ({
        ...
        significance_note: text(f?.significance_note),
      }))
      .filter((f) => f.finding && f.evidence)
      .sort((a, b) => a.finding_index - b.finding_index);
    if (canonical.length > 0) return canonical;
    if (!findingsTabCache) findingsTabCache = groupFindingsByProbe(await loadFindingsTable(repo));
    return findingsTabCache.get(probeId) || [];
  }
  ...
  for (const personalisation of personalisationRows) {
    ...
    try {
      ...
      const findings = await findingsFor(probeId);
      ...
      const slug = text(existingRecord?.obj.demo_slug) || text(opts.slug) || buildShortDemoSlug({
