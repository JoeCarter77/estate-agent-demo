...
// Derives the next reference from the references that ALREADY EXIST, rather
// than from a row count.
//
// Why: a count-based sequence is wrong in two ways against the live workbook.
// (1) The live PROBES tab mixes reference schemes — 24 "HIST-####" imported rows
// and 2 "TEST-####" fixtures — so "26 rows" would mint RM-0027 while RM-0001..26
// were never used. (2) Two probes created close together both read the same
// count and mint the SAME reference, because the count is read before either
// row is appended.
//
// Taking max(existing RM-####) + 1 fixes (1) outright and narrows (2) to a true
// simultaneous double-append; probe-create additionally re-checks for a
// collision and steps past it, so a duplicate reference cannot be written.
// Historical references are never re-issued because max() only ever increases.
export function nextProbeReference(existingReferences, portal = 'rightmove') {
  const code = PORTAL_CODE[String(portal).toLowerCase()] || 'RM';
...
// ACTIONS.action_id — one per action raised off a probe's intelligence.
