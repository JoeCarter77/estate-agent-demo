...
// language can never leak into it. email_secondary_hook is the one email
// variable the model does NOT write: it is a single fixed intrigue line
// ("There were also a couple of other things...") shown only when a genuine
// finding sits outside the primary narrative, never a second paragraph of
// analysis — see SECONDARY_HOOK_LINE.
//
...
//     email_secondary_hook is never free text — see SECONDARY_HOOK_LINE.
...
const TOOL = {
  ...
  input_schema: {
    ...
    required: [
      ...
      'email_main_point', 'email_consequence',
    ],
    properties: {
      primary_narrative: {
        type: 'string',
...
THE EMAIL VARIABLES (fair_observation, email_main_point, email_consequence) are different from everything else you produce. They are merged, verbatim and unedited, into a fixed email template that a real estate agent will read. So:
...
// The fixed intrigue line for email_secondary_hook — see its assignment
// below. Never AI-authored: a short tease, not another paragraph of analysis.
const SECONDARY_HOOK_LINE = 'There were also a couple of other things from this enquiry that caught our attention.';
...
export async function personaliseProbe(probe, intelligence, diagnosis, findings, communications, agency) {
  ...
  // DET — not asked of the model. A short intrigue line, not another
  // paragraph of analysis: the model was previously trusted to write this
  // sentence itself and would sometimes turn it into a restatement of the
  // finding (or of its own reasoning) instead of a one-line tease. It is now
  // a single fixed line, shown only when a genuine finding sits outside the
  // primary narrative, blank otherwise.
  const emailSecondaryHook = hasUncoveredFindings ? SECONDARY_HOOK_LINE : '';
...
export const _internal = {
  ...
  HERO_JOURNEYS, NO_REPLY_LINE, SECONDARY_HOOK_LINE, INTERNAL_REASONING_PATTERNS,
};
