...
const TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      email_observation: {
        type: 'string',
        description: 'INSTANTLY VARIABLE. WHAT THE RESPONSE ACTUALLY DID. One conversational first-person sentence of 40 words or fewer, from ALL AND ONLY the selected indexes: one genuine positive, one main problem, at most one tightly connected second problem. Prioritise what the communication tried to achieve, whether a concrete next step (viewing, times, qualification, structured callback) was proactively offered, and whether the declared-seller signal was picked up; use response speed and repeated handling only where they are materially weak and relevant. Do not spend the space on basic hygiene such as my name or the correct property unless it is load-bearing. Never blame the agency simply because the enquiry did not progress when I deliberately did not answer — describe the quality and intent of their attempt instead. Prefer concrete actions and omissions ("no viewing was offered") over abstract summaries ("the enquiry wasn\'t progressed"), and never claim what happened on a call unless a voicemail or transcript in the evidence establishes it. State the pattern and stop — explaining it, or adding the consequence, is the hook\'s job and makes this line soft. Never a list of every finding. If there was no meaningful reply, state the absence. No CTA or NOVUS explanation, no deck language, never the enquirer in the third person, and never claim questions were asked that the enquiry never put.',
      },
...
The two lines are one pair, and they have different jobs.
  OBSERVATION = what the response actually did.   HOOK = what actually got missed.
Short. Specific. Concrete. Written by the person who sent the enquiry, to the agency. Never consultant language.

email_observation — one sentence, 40 words or fewer. What the agency's response actually DID.
...
WHAT TO SPEND THE SENTENCE ON — a flexible hierarchy, not a checklist. Lead with the strongest
commercially meaningful evidence available, and drop a lower point whenever it would displace a higher
one. Two or three points that work together beat five crammed in.
  1. What the communication actually tried to achieve — did it answer the enquiry, qualify me, offer
     help, or just ask me to make contact again?
  2. Whether a concrete next step was proactively offered: a viewing, viewing times, qualification, a
     structured callback, or another clear progression step.
  3. Whether the declared-seller signal was recognised or acted on — that is what tests whether a
     valuation or instruction opportunity was spotted at all.
  4. Response speed, where it is materially weak and relevant to the overall handling.
  5. Persistence or repeated handling, where it is commercially relevant and the findings support it.

- NOT WORTH THE SPACE: basic personalisation and expected hygiene — using my name, naming the right
  property, sending information about the property I asked about. Mention one only where it is genuinely
  load-bearing for the commercial point; otherwise it crowds out the evidence that matters.
- NEVER criticise the agency merely because the enquiry did not ultimately progress when I deliberately
  did not answer or reply. An unanswered call is my doing, not theirs. Judge the quality, structure and
  intent of their attempt instead.
- CONCRETE BEATS ABSTRACT, where the evidence supports the wording:
     "no viewing was offered"                    beats  "the enquiry wasn't progressed"
     "the voicemail only asked me to call back"   beats  "there was no qualification"
- GROUNDING: do not infer what happened on a call merely because a call happened. Where a finding's
  evidence quotes a voicemail or transcript, use what it actually says. Where there is no evidence of the
  call's content, never claim there was "no structure", "nothing was asked" or "no viewing was offered" —
  say only what the evidence establishes, such as that a call came and how long it took.
  LANDS:     "Terry called me, but it took 37 hours, and there was no attempt to book a viewing or pick up that I'd also said I had a property to sell."
  LANDS:     "It took nearly 19 hours to get a call, and the message was essentially just a request to call back — no viewing was offered and the property I'd said I was selling wasn't picked up."
  LANDS:     "You sent me two relevant property brochures, but across three emails nobody once asked about the property I said I had to sell."
  ...
  WASTED:    "Terry's callback used my name and the right property, but it took 37 hours, asked nothing, and never mentioned the property I said I had to sell."
             (the name and the correct property are basic expectations; they spend half the sentence proving nothing)
  UNFAIR:    "It took nearly 19 hours to get a call, and the enquiry wasn't progressed, while nobody picked up that I also had a property to sell."
             (I chose not to answer that call, so "wasn't progressed" blames them for my own silence — describe the attempt instead)
...
"nobody asked", "no viewing was offered", "the message was just a request to call back", "I'd also said I had a
property to sell", "the viewing side moved forward", "the seller side was missed". Reach for the concrete version
first; "the enquiry wasn't progressed" is a last resort, and never the wording for a call I chose not to answer.
...
// enquiry", "nobody asked about the property I said I had to sell", "no viewing
// was offered" — say the same thing truthfully, and concretely.
...
const REPAIR_NOTES = {
  ...
  email_observation: {
    ...
    too_long: 'email_observation is too long, or lists too many things. DROP the weakest point rather than compressing every point into an abstract summary: keep what the response actually tried to achieve, the concrete next step that was or was not offered, and the declared-seller signal ahead of speed, persistence, or any basic personalisation detail. 40 words or fewer.',
    ...
    unasked_questions: 'The enquiry asked for more details; it did not put specific questions to the agent, so "nobody answered my questions" is not supported. Say what IS supported and stay concrete: "got back to the enquiry", "no viewing was offered", "nobody asked about the property I said I had to sell".',
...
// 40. The cap exists to stop the sentence explaining itself instead of stating
// what happened, which the two versions of the same 19-hour observation show:
...
//
// Back up to 40 from 30: the observation now has to say what the agency's
// response actually DID — what it tried to achieve, and which concrete next
// step was or was not offered — and the grounded version of that
//   "It took nearly 19 hours to get a call, and the message was essentially
//    just a request to call back — no viewing was offered and the property
//    I'd said I was selling wasn't picked up."
// is 35 words of pure evidence. At 30 the model buys the room for it by
// falling back on the abstract summary ("the enquiry wasn't progressed"),
// which is exactly the wording this field is now meant to avoid. The
// anti-padding job is carried by the prompt's priority hierarchy, which tells
// it to DROP the weaker point rather than compress every point.
