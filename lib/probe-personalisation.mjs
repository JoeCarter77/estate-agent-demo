...
// allowedFigure is PROBES.property_price: the value of the property the
// prospect enquired about AS A BUYER. It is never the value of whatever
// property the prospect (as a declared seller) has to sell — the probe
// carries no separate figure for that, so a sentence that puts the allowed
// figure next to seller-side wording ("property to sell", "valuation",
// "instruction") is stating a seller value we were never given. Grounded,
// non-priced wording for that side ("the property I said I had to sell",
// "a valuation opportunity", "a potential seller instruction") stays fine —
// only the currency token next to that wording is the leak.
const SELLER_VALUE_ATTRIBUTION_RE = new RegExp(
  String.raw`[£$€]\s?[\d][\d,.]*\s*(?:k|m|bn|million|thousand)?[^.!?]{0,40}\b(?:property to sell|valuation(?:s)?(?:\s+opportunit(?:y|ies))?|instructions?|seller'?s?\s+property|vendor'?s?\s+property)\b`
  + '|'
  + String.raw`\b(?:property to sell|valuation(?:s)?(?:\s+opportunit(?:y|ies))?|instructions?|seller'?s?\s+property|vendor'?s?\s+property)\b[^.!?]{0,40}[£$€]\s?[\d][\d,.]*`,
  'i',
);

// Drops any sentence containing a currency figure that isn't the allowed one,
// and any sentence that attaches the allowed (buyer-enquiry) figure to
// seller-side wording as if it were the seller's own property value.
...
export function stripUnbackedCurrency(text, allowedFigure) {
  ...
  const kept = sentences.filter((sentence) => {
    ...
    if (!tokens.every((token) => allowed !== null && normalizeCurrencyFigure(token) === allowed)) return false;
    return !SELLER_VALUE_ATTRIBUTION_RE.test(sentence);
  });
