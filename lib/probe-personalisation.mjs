...
// ── The countable shape of this enquiry ─────────────────────────────────────
//
// THE HOOK'S MISSING INPUT. email_commercial_hook exists to turn the
// observation into a commercial/process problem — "That's 2 commercial
// opportunities from 1 enquiry, with neither fully progressed." Every hook of
// that shape is a COUNT, and the prompt used to hand the model no counts at
// all: findings prose, a property value and nothing else. With nothing to
// count, the model did the only thing left and restated the observation in
// different words, which is exactly the regression ("A potential seller went
// completely unengaged, meaning no valuation conversation ever had the chance
// to start" is the observation again).
//
...
export function buildOpportunityShape(probe, intelligence) {
  ...
  const opportunities = sellerDeclared ? 2 : 1;
  const progressed = (BUYER_PROGRESSED_STATES.has(viewing) ? 1 : 0)
    + (sellerDeclared && SELLER_PROGRESSED_STATES.has(seller) ? 1 : 0);

  const lines = [
    `Commercial opportunities inside this one enquiry: ${opportunities}`
      + (sellerDeclared ? ' (1 buyer viewing + 1 declared property to sell)' : ' (1 buyer viewing; no property to sell was declared)'),
    `Opportunities taken to a concrete next step: ${progressed} of ${opportunities}`,
  ...
  lines.push(noContact
    ? 'Contact attempts: 0 (follow-ups: 0) — no meaningful human response in the observation window'
  ...
  if (!noContact && Number.isFinite(hours)) {
    lines.push(`First human response: ${hours >= 24 ? `${(hours / 24).toFixed(1)} days` : `${hours.toFixed(1)} hours`} after the enquiry`);
  }
  lines.push(`Conversations genuinely created: ${noContact ? 0 : progressed}`);
  return lines.join('\n');
}
...
const TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      email_observation: {
        type: 'string',
        description: 'INSTANTLY VARIABLE. WHAT HAPPENED. One conversational first-person sentence of 40 words or fewer, from ALL AND ONLY the selected indexes: one genuine positive, one main problem, at most one tightly connected second problem. Never a list of every finding. If there was no meaningful human response, omit praise and state the absence. No consequence, CTA or NOVUS explanation. Never refer to the enquirer in the third person.',
      },
      email_commercial_hook: {
        type: 'string',
        description: 'INSTANTLY VARIABLE. WHAT IT MEANS. One concise sentence that states the commercial/process shape of the SAME selected findings email_observation used — e.g. "That is 2 commercial opportunities from 1 enquiry, with neither fully progressed." Quantify from the OPPORTUNITY SHAPE counts. Restating the observation in different words is a failure. It must not introduce an unselected problem. No CTA, generic sales language, invented value, fee or definite-loss claim.',
      },
      novus_counterfactual: {
        type: 'string',
...
INSTANTLY VARIABLES
The two lines are one pair: OBSERVATION = what happened. HOOK = what that means about the commercial opportunity and the process.

email_observation — one conversational sentence, 40 words or fewer.
- Structure, where a genuine positive exists: POSITIVE, but MAIN PROBLEM (+ at most one tightly connected second problem).
- One positive, one main problem, at most one second problem. Never a list of everything that went wrong — a shopping list of findings reads as an audit and is worse than a sharper, shorter sentence.
- With no meaningful human response, omit praise and simply state the absence, plus any connected opportunity the findings support.
- No consequence, no CTA, no NOVUS, no audit language.
  Good: "You got back to the enquiry almost instantly, but no follow-ups were sent and nobody picked up that I'd also said I had a property to sell."
  Good: "We didn't receive a response during the four-day observation period, and nobody picked up that I'd also said I had a property to sell."

email_commercial_hook — one concise sentence stating what the observation MEANS.
- It must answer: "what does this tell me about what our process failed to capture?"
- STATE THE SHAPE, using the OPPORTUNITY SHAPE counts above. A hook that describes the same events in different words has done nothing.
  Good: "That's 2 commercial opportunities from 1 enquiry, with neither fully progressed."
  Good: "So even with 3 contact attempts, half the commercial opportunity inside the enquiry was never worked."
  Good: "That's 2 live opportunities from 1 enquiry, with 0 conversations created."
  Good: "So 1 of the 2 commercial opportunities in that enquiry was effectively invisible to the process."
  Bad (restates the observation): "A potential seller went completely unengaged, meaning no valuation conversation ever had the chance to start."
  Bad (still description): "A self-declared seller not yet on the market is a live valuation lead, and it went completely unacknowledged alongside the viewing chase."
- Use ONLY the counts in the OPPORTUNITY SHAPE block. Never invent revenue, fees, conversion rates, annual losses, "revenue leakage", "lost revenue" or "costing you thousands".
...
function buildPrompt(probe, intelligence, findings, noHumanContact, scaleFact) {
  ...
  return [
    ...
    '',
    '=== OPPORTUNITY SHAPE (code-computed from this enquiry — the counts email_commercial_hook exists to state) ===',
...
// same events back in different words has done nothing, and that was the most
// common quality regression in the historical run:
//   observation: "This enquiry, from someone with a property to sell, received
//                 no contact on any channel across the full 4-day window..."
//   hook:        "A potential seller went completely unengaged, meaning no
//                 valuation conversation ever had the chance to start."
// Vocabulary overlap does NOT catch that — the two sentences share almost no
// words. What separates every good hook from every bad one is that the good
// one states the SHAPE of the enquiry: a count of opportunities, attempts,
// follow-ups or conversations, set against the one enquiry they came from.
// buildOpportunityShape() now hands the model exactly those counts, so
// requiring one back is a fair contract rather than a trap.
//
// Two independent ways to fail, both repairable:
//   restates_observation — an outright or near-outright rewording;
//   no_quantification    — no countable shape at all.

const QUANTIFIED_NOUNS = String.raw`opportunit(?:y|ies)|enquir(?:y|ies)|conversations?|attempts?|follow[- ]?ups?|viewings?|valuations?|contacts?|leads?|days?|hours?|messages?|emails?|calls?|replies|reply|next steps?|sides?`;

// Deliberately excludes "no" and "none". "no valuation conversation ever had
// the chance to start" is a DESCRIPTION of an absence, not a count of one, and
// treating it as quantification let the single worst hook in the historical
// run pass this check. A real count is a digit, a spelled-out number, or one
// of the words that states a proportion of a known set.
const NUMBER_WORD = String.raw`\d+(?:\.\d+)?|one|two|three|four|five|six|seven|eight|nine|ten|zero|neither|both|half`;

// "2 commercial opportunities", "0 conversations", "3 contact attempts",
// "1 of the 2 commercial opportunities", "neither fully progressed",
// "half the commercial opportunity". A bare year or a stray digit does not
// count: the number has to be attached to something countable in the enquiry.
export function quantifiesOpportunityShape(text) {
  const t = String(text || '').trim();
  if (!t) return false;
  const forward = new RegExp(String.raw`\b(?:${NUMBER_WORD})\b[^.!?]{0,40}?\b(?:${QUANTIFIED_NOUNS})\b`, 'i');
  const backward = new RegExp(String.raw`\b(?:${QUANTIFIED_NOUNS})\b[^.!?]{0,24}?\b(?:${NUMBER_WORD})\b`, 'i');
...
export function hookFailureAgainstObservation(hook, observation) {
  ...
  return quantifiesOpportunityShape(hook) ? null : 'no_quantification';
}
...
// Wording failures on true copy. Anything not listed is treated as hard.
const SOFT_REASONS = new Set([
  'too_long', 'snuck_criticism', 'detached_third_person', 'perspective_inversion',
  'restates_finding', 'speculative', 'third_person_prospect',
  'restates_observation', 'no_quantification',
]);

const isSoft = (reason) => SOFT_REASONS.has(reason);
...
const REPAIR_NOTES = {
  ...
  email_commercial_hook: {
    ...
    no_quantification: 'email_commercial_hook does not state the shape of the missed opportunity. Use the OPPORTUNITY SHAPE counts: how many commercial opportunities the enquiry contained, how many were progressed, how many contact attempts or conversations there were.',
...
// 40, not 45. The brief's ceiling is 35-40 words, and the historical run's
// worst observation was 42 words listing five separate findings ("...came over
// 37 hours late, asked no questions, offered no viewing, and never picked up
// on the fact I have a property of my own to sell"). Length is the one
// mechanical proxy for the overloaded-shopping-list failure that does not need
// the model's cooperation to measure.
const EMAIL_OBSERVATION_MAX_WORDS = 40;
...
function isBetterFallback(candidate, best) {
  if (!best) return true;
  const hard = (value) => value.rejections.filter(({ reason }) => !isSoft(reason)).length;
...
function buildCandidate(result, ctx) {
  ...
  const gate = (field, sanitised, checks) => {
    ...
    for (const { reason, failed } of checks) {
      ...
      if (isSoft(reason)) softFallbacks[field] = value;
      return '';
    }
    return value;
  };
  ...
  const emailObservation = gate('email_observation', observationText, [
    { reason: 'unselected_finding', failed: introducesUnselectedFinding(observationText, selectedFindings, orderedFindings) },
  ...
  const emailCommercialHook = gate('email_commercial_hook', hookText, [
    { reason: 'unselected_finding', failed: introducesUnselectedFinding(hookText, selectedFindings, orderedFindings) },
...
export const _internal = {
  ...
  SOFT_REASONS, SELECTION_FIELDS, EMAIL_OBSERVATION_MAX_WORDS,
