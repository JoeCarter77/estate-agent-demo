...
// email_commercial_hook must both describe
...
//
// THE TWO ACTIVE EMAIL FIELDS DO DIFFERENT JOBS, and the guards below exist
// to keep them apart:
...
// and email_commercial_hook. The demo-required
...
const TOOL = {
  ...
  description: 'Select one findings-grounded story and write the demo prose plus the two active Instantly variables from that same selection.',
  input_schema: {
    type: 'object',
    ...
    properties: {
      ...
      email_observation: {
        type: 'string',
        description: 'INSTANTLY VARIABLE. One natural UK-English sentence, ideally 25 words or fewer, with one central factual point. Credit materially good handling where relevant. No criticism pile-up, forced seller angle, invented certainty or judgemental language.',
      },
...
email_observation — one sentence, ideally 25 words or fewer, answering: "what actually happened?"
...
- primary_narrative, supporting_findings and novus_counterfactual are internal/demo prose, never substitutes for the two active Instantly variables.
...
export function hookFailureAgainstObservation(hook, observation) {
  ...
  // The active hook is curiosity-led, not a forced commercial-loss claim.
  // Distinct, plain and factual context is enough; it need not quantify an
  // opportunity or name a valuation-style outcome.
  return null;
}
...
const EMAIL_OBSERVATION_MAX_WORDS = 25;
...
export async function personaliseProbe(probe, intelligence, diagnosis, findings, agency) {
  ...
  const request = async (attempt, previousCandidate) => {
    if (attempt === 1 || previousCandidate === null) {
      return callAi({ system: SYSTEM_PROMPT, prompt: basePrompt, tool: TOOL, purpose: 'personalisation' });
    }
