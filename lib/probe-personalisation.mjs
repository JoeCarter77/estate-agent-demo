...
// nobody ever gave us.
//
// This is a PROVENANCE rule, not a phrase blacklist, and it is an ALLOW-LIST.
// The first version matched a fixed list of seller phrases ("property to
// sell", "valuation", "instruction") and every paraphrase walked through it.
// The second version generalised that to "a price token in the same clause as
// a seller term", which is still a blacklist: "I am selling a £450,000 home",
// "a second £450,000 opportunity", "a property in the same value range to
// sell" and "worth about the same" all walked through that one too.
//
// So the test is inverted. A price token is permitted ONLY where its clause
// says, in so many words, that the figure belongs to the property I enquired
// about AS A BUYER. Anything else — a seller clause, an ambiguous clause, a
// clause with no buyer anchor at all — loses the figure. There is no seller
// value on file, so anywhere the copy is not explicitly talking about the
// enquiry property it may not carry a figure at all.
//
...
const SELLER_CONTEXT_RE = /\b(?:sellers?|vendors?|valuations?|instructions?|to\s+sell|to\s+list)\b/i;

// The buyer anchor. Deliberately narrow: the clause must name the BUYER
// ENQUIRY itself, or say that I was the one who enquired/asked about it.
// Weaker candidates ("listing", "on the market", "property", "you asked") are
// left out on purpose — they describe the seller's property just as readily,
// and an allow-list that admits an ambiguous clause is not an allow-list.
const BUYER_ENQUIRY_ANCHOR_RE = /\bbuyer\s+enquir\w*\b|\b(?:I|we)\s+(?:enquired|asked|viewed|was\s+asking|were\s+asking)\b|\benquired\s+about\b/i;

// A COMPARISON IS AN ASSERTION. Only one property has a value on file, so
// "the same value range", "similarly priced", "worth about the same" can only
// mean "the property I have to sell is worth what the enquiry property is
// worth" — a valuation nobody gave us, carrying no figure for a clause test to
// find. These are removed wherever they appear; there is no clause in which
// comparing two property values is grounded.
const SELLER_VALUE_IMPLICATION_RE = new RegExp([
  String.raw`\s*\b(?:in|at|of|within|around)?\s*(?:the\s+|a\s+)?(?:same|similar|comparable|equivalent|matching)\s+(?:value|price|pricing|money)\s*(?:range|bracket|band|point|level|category)?\b`,
  String.raw`\s*\b(?:in|within)\s+(?:that|the\s+same|a\s+similar)\s+(?:price\s+|value\s+)?(?:bracket|range|band|ballpark)\b`,
  String.raw`\s*\b(?:similarly\s+priced|priced\s+similarly|comparably\s+priced)\b`,
  String.raw`\s*\bworth\s+(?:about\s+|around\s+|roughly\s+|approximately\s+)?the\s+same\b`,
  String.raw`\s*\bof\s+(?:a\s+)?(?:similar|comparable|equivalent)\s+value\b`,
].join('|'), 'gi');
...
// first non-space of the price reference. A figure survives only in a clause
// that anchors it to the buyer enquiry AND says nothing seller-side.
function priceIsBuyerEnquiryAnchored(whole, matchOffset, match) {
  const anchor = matchOffset + (match.length - match.trimStart().length);
  const clause = clauseAt(whole, anchor);
  return BUYER_ENQUIRY_ANCHOR_RE.test(clause) && !SELLER_CONTEXT_RE.test(clause);
}

// True when the copy states or implies a value that is not the buyer enquiry's
// own: a figure (or a bare "priced" modifier) outside a buyer-anchored clause,
// or a comparison that asserts a second property's value with no figure at all.
export function attributesEnquiryPriceToSeller(text) {
  const raw = String(text || '');
  if (!raw.trim()) return false;
  if (new RegExp(SELLER_VALUE_IMPLICATION_RE.source, 'i').test(raw)) return true;
  const re = new RegExp(`${CURRENCY_TOKEN_RE.source}|\\bpriced\\b`, 'gi');
  let match = re.exec(raw);
  while (match !== null) {
    if (!priceIsBuyerEnquiryAnchored(raw, match.index, match[0])) return true;
    match = re.exec(raw);
  }
  return false;
}
...
export function stripSellerPriceAttribution(text) {
  ...
  const unlessBuyerAnchored = (match, offset, whole) => {
    if (priceIsBuyerEnquiryAnchored(whole, offset, match)) return match;
  ...
  const stripped = raw
    // A value COMPARISON goes wherever it stands — see
    // SELLER_VALUE_IMPLICATION_RE. Removed first so the clause tests below
    // never read a phrase that is already on its way out.
    .replace(SELLER_VALUE_IMPLICATION_RE, (match) => (match.match(/[.!?]+$/) || [' '])[0])
    .replace(PRICE_WITH_CARRIER_RE, unlessBuyerAnchored)
    .replace(PRICED_MODIFIER_RE, unlessBuyerAnchored);
...
const UNFAIR_OUTCOME_CRITICISM = [
  // "with neither properly progressed", "neither opportunity became a
  // conversation", "neither side was ever converted"
  /\bneither\b[^.!?]{0,48}\b(?:progress(?:ed|ing)?|becom(?:e|ing)|became|convert(?:ed|ing)?|reach(?:ed|ing)?|happen(?:ed|ing)?|book(?:ed|ing)?)\b/i,
  ...
  /\b(?:enquiry|enquiries|conversation|thread|opportunit(?:y|ies)|side|sides|lead|leads|it|this)\b[^.!?]{0,28}\bnever\s+(?:really\s+|actually\s+|properly\s+|fully\s+)?(?:progress(?:ed)?|moved(?:\s+forward|\s+on)?|went anywhere|develop(?:ed)?|advanced|got going)\b/i,
  ...
  /\bnever\s+(?:really\s+|actually\s+)?(?:became|turned into|grew into)\s+(?:a|an|any)\s+conversation\b/i,
  ...
  // "with neither ever becoming a conversation" is caught above; this is the
  // "both" phrasing of the same claim.
  /\bboth\s+(?:sides|opportunit(?:y|ies)|leads)\b[^.!?]{0,32}\b(?:never|were left|stayed|remained)\b/i,
];
...
export function hasUnreliableVoicemailEvidence(findings) {
  return (findings || []).some((finding) => {
    ...
    return /\bvoicemail\b/i.test(text) && VOICEMAIL_UNRELIABLE_MARKERS.test(text);
  });
}
...
export function makesUnsupportedVoicemailClaim(text) {
  const t = String(text || '').trim();
  if (!t || !/\bvoicemail\b/i.test(t)) return false;
  return VOICEMAIL_NEGATIVE_CONTENT_CLAIMS.some((re) => re.test(t));
}
...
// WHY emailVariable() RETURNED NOTHING.
//
// A mandatory email line is ONE sentence, and three of the sanitisers above
// are sentence-level: stripUnbackedCurrency() drops a sentence carrying a
// figure that is not this probe's own, stripInventedLoss() drops one that
// turns a figure into the agency's loss (a fee, a commission, a percentage),
// and readsAsInternalReasoning() drops the whole value when it reads as a note
// to ourselves. On a one-sentence field each of those empties the field
// outright — and the gate then recorded the bare reason 'blank', which
// REPAIR_NOTES answers with "this field is required". The correction call was
// therefore told the line was missing but never told what had destroyed it,
// re-answered the same question the same way, and the second miss persisted a
// blank. That is a FORMATTING blank on copy the model actually wrote, which is
// exactly what this section's invariant forbids.
//
// So the cause is recovered from the model's own raw text and reported as its
// own reason, the same way seller_price_attribution already is. Nothing here
// changes what the sanitisers do or what the model is asked to write; it only
// names the failure the correction has to fix.
function blankCause(raw, allowedFigure) {
  const text = String(raw || '').trim();
  if (!text || hasUnresolvedPlaceholder(text)) return 'blank';
  if (readsAsInventedLoss(text)) return 'invented_loss';
  if (readsAsInternalReasoning(text)) return 'internal_reasoning';
  if (!stripUnbackedCurrency(text, allowedFigure).trim()) return 'unbacked_currency';
  return 'blank';
}
...
// The prose a later attempt may inherit from an earlier one. Deliberately NOT
// the index fields and NOT the deterministic columns: a second attempt that
// re-asked the whole question may legitimately settle on a DIFFERENT selection,
// and refilling last attempt's index behind it would persist a selection the
// prose no longer describes.
const BANKABLE_PROSE_FIELDS = [
  'fair_observation', 'main_finding', 'commercial_consequence',
  'email_observation', 'email_commercial_hook', 'email_commercial_hook_email_2',
];
...
const NEVER_PERSIST_REASONS = new Set([
  ...
  // The three causes blankCause() recovers from a line the sanitisers emptied.
  // They are truthfulness failures (invented money, our own analytical
  // vocabulary, a figure that is not on file) and there is nothing left of the
  // line to bank anyway — so they rank exactly as the bare 'blank' they
  // replace, and only the correction can fix them.
  'invented_loss',
  'internal_reasoning',
  'unbacked_currency',
]);
...
const SELLER_PRICE_REPAIR_NOTE = (field) =>
  `${field} attaches the enquiry property's price to my SELLER opportunity. That figure is the asking price of the property I enquired about as a buyer; nothing on file says what the property I have to sell is worth. Keep the same commercial point and remove the figure from the seller side — "a seller conversation", "a potential seller instruction", "a valuation opportunity". The figure may still be used where the sentence says in so many words that it is the property I enquired about buying — "the £450,000 property I asked about", "a £450,000 buyer enquiry". Never compare the two properties by value either: there is no value on file for the one I have to sell.`;

// The three sanitisation causes blankCause() recovers. Each says what to
// REMOVE and asks for the same line back — never for a different point, and
// never for a different story.
const INVENTED_LOSS_REPAIR_NOTE = (field) =>
  `${field} was emptied because it stated or implied money the agency lost — a fee, a commission, revenue, a percentage, "worth £...". We do not know their fee scale and one enquiry does not prove a lost instruction. Write the SAME point again with no money in it at all.`;

const INTERNAL_REASONING_REPAIR_NOTE = (field) =>
  `${field} was emptied because it referred to the machinery behind the email (findings, evidence, diagnosis, analysis, the probe) or explained itself instead of speaking to the agency. Write the SAME point as one plain sentence from the person who sent the enquiry.`;

const UNBACKED_CURRENCY_REPAIR_NOTE = (field) =>
  `${field} was emptied because it cited a monetary figure that is not the asking price of the property I enquired about. That price is the ONLY figure on file; there is no value for the property I said I have to sell, and no other figure may appear. Write the SAME point again with no figure in it.`;
...
const REPAIR_NOTES = {
  ...
  email_observation: {
    ...
    invented_loss: INVENTED_LOSS_REPAIR_NOTE('email_observation'),
    internal_reasoning: INTERNAL_REASONING_REPAIR_NOTE('email_observation'),
    unbacked_currency: UNBACKED_CURRENCY_REPAIR_NOTE('email_observation'),
  },
  email_commercial_hook_email_2: {
    ...
    invented_loss: INVENTED_LOSS_REPAIR_NOTE('email_commercial_hook_email_2'),
    internal_reasoning: INTERNAL_REASONING_REPAIR_NOTE('email_commercial_hook_email_2'),
    unbacked_currency: UNBACKED_CURRENCY_REPAIR_NOTE('email_commercial_hook_email_2'),
  },
  email_commercial_hook: {
    ...
    invented_loss: INVENTED_LOSS_REPAIR_NOTE('email_commercial_hook'),
    internal_reasoning: INTERNAL_REASONING_REPAIR_NOTE('email_commercial_hook'),
    unbacked_currency: UNBACKED_CURRENCY_REPAIR_NOTE('email_commercial_hook'),
  },
};
...
export async function personaliseProbe(probe, intelligence, diagnosis, findings, agency) {
  ...
  let previous = null;
  let best = null;
  // EVERY USABLE LINE EITHER ATTEMPT PRODUCED, kept per FIELD rather than per
  // candidate. `best` picks one whole candidate, so a line attempt 1 wrote
  // well enough to bank was thrown away whenever attempt 2 came back a worse
  // candidate overall — including when attempt 2 came back with that same
  // field empty. Fields are filled from here only where the chosen candidate
  // has nothing, so a good value is never overwritten by an older one.
  const bankedByField = new Map();
  for (let attempt = 1; attempt <= MAX_PERSONALISATION_ATTEMPTS; attempt += 1) {
    ...
    for (const field of BANKABLE_PROSE_FIELDS) {
      const value = candidate.row[field] || candidate.softFallbacks[field];
      if (String(value || '').trim()) bankedByField.set(field, value);
    }
  ...
  const row = { ...best.row, ...best.softFallbacks };
  for (const [field, value] of bankedByField) {
    if (!String(row[field] || '').trim()) row[field] = value;
  }

  // THE TERMINAL INVARIANT, enforced rather than hoped for.
  //
  // email_commercial_hook_email_2 has no evidence reason to be blank. Unlike
  // fair_observation (no positive finding) or main_finding (no problem
  // finding, or no response at all), Email 2's hook is required of every
  // personalised probe, so a blank one is always a failure of this function
  // and never a fact about the enquiry. Persisting it anyway is what put a
  // half row on the sheet: needsPersonalisation() then re-picks the probe on
  // every later pass, spends the AI again, and writes the same blank back.
  //
  // With both attempts spent and nothing usable banked for it from either,
  // there is no line to persist and nothing here may invent one. So the row is
  // refused instead — exactly as the truncated-result path above refuses one —
  // and lib/personalisation-rebuild.mjs records the probe in `problems` with
  // no row written, which is the state a later pass genuinely retries.
  if (!String(row.email_commercial_hook_email_2 || '').trim()) {
    throw new AiStructuredOutputError(
      'email_commercial_hook_email_2 is mandatory and came back unusable from both attempts; no PERSONALISATION row written',
      { truncated: false, missing: ['email_commercial_hook_email_2'] },
    );
  }

  return { ...row, ai_calls_used: MAX_PERSONALISATION_ATTEMPTS };
}
...
function buildCandidate(result, ctx) {
  ...
  //
  // `raw` (optional) is the model's own text before emailVariable() sanitised
  // it. When sanitisation EMPTIES a mandatory line, WHY it emptied is the one
  // thing the bounded correction needs to hear — see blankCause() — because
  // the bare reason 'blank' asks the model the same question that just failed.
  const gate = (field, sanitised, checks, raw) => {
    const value = String(sanitised || '').trim();
    if (!value) {
      rejections.push({ field, reason: raw === undefined ? 'blank' : blankCause(raw, allowedFigure) });
      return '';
    }
  ...
  const mainFindingText = asContinuation(emailVariable(result.main_finding, allowedFigure), FIXED_PREFIX_PATTERNS.mainFinding, protectedWords);
  ...
  const secondHookText = normaliseEmailSentence(result.email_commercial_hook_email_2, allowedFigure);
