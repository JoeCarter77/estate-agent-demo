...
export function pickHeroJourney(intelligence, diagnosis) {
  ...
  const findings = parseDiagnosisFindings(diagnosis);
  ...
  // Fast to first contact (<=16h, grade B/D territory) but the evidence
  // still shows a real problem: distinguish a stalled/shallow seller thread
  // from fast-but-shallow handling in general, rather than mislabelling
  // either as a speed gap it never had.
...
4. If Diagnosis's findings list is empty (the agency handled this well), do not manufacture a weakness anywhere — novus_counterfactual should say NOVUS would have matched the handling, wider_leakage and objection_response may legitimately be empty, and the story becomes about consistency/scale/the next opportunity (novus_opportunity), never a fabricated gap.
...
function findingsBlock(findings) {
  if (findings.length === 0) return 'Findings: (none — handled well)';
  return `Findings (${findings.length}, most commercially damaging first):\n${findings
    .map((f, i) => `  ${i + 1}. ${f.finding} — ${f.evidence}${f.significance_note ? ` [significance: ${f.significance_note}]` : ''}`)
    .join('\n')}`;
}
...
function buildPrompt(probe, intelligence, diagnosis, communications, scaleFact) {
  ...
  return [
    ...
    findingsBlock(parseDiagnosisFindings(diagnosis)),
