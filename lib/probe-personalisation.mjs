...
const UNFAIR_OUTCOME_CRITICISM = [
  ...
  /\b(?:never|none\s+of\s+(?:them|these|those|it)|not\s+one(?:\s+of\s+them)?)\s+(?:really\s+|actually\s+|ever\s+)?(?:became|turned\s+into|grew\s+into|developed\s+into)\s+(?:a|an|any)\s+(?:real|genuine|proper|actual|meaningful|two[- ]way|true)?\s*conversation\b/i,
