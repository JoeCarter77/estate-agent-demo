...
import {
  assembleEmail, ADDITIONAL_FINDINGS_HOOK_LINE, hasUnresolvedPlaceholder, propertyReference,
} from './email-assembly.mjs';
...
const TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      fair_observation: {
        type: 'string',
        description: 'EMAIL COPY, read by the prospect. WRITTEN FROM THE FINDING AT positive_finding_index (or, where the list carries no positive finding, from the plainest factual thing the findings establish about the contact). MANDATORY whenever there was human contact. CONTINUATION ONLY: the email hard-codes "I want to say upfront that ", so write only what follows it, starting lower-case. ONE concise sentence. Acknowledge the strongest genuine positive and use at most ONE proof point when it adds credibility — for example, "you followed up properly — three attempts by phone and email within a day." Do not inventory every channel, message, correct name, property reference, question and invitation. Even a weak interaction has one factual positive: they replied, called, acknowledged the enquiry, asked a question or followed up. Pick the strongest supported one and STOP. Never invent praise, hedge it with eventually/although/despite/however/finally/at least, or hint at the criticism to come. Write TO them ("you") and describe our side as "I"/"me"/"we"; never detached third person.',
      },
      main_finding: {
        type: 'string',
        description: 'EMAIL COPY, read by the prospect. WRITTEN FROM main_finding_index, and from no other finding. The PRIMARY thing the agency failed to do. CONTINUATION ONLY: the email hard-codes "What stood out, though, was ", so write only what follows it, starting lower-case. ONE concise sentence. Use enough evidence to make the point undeniable, but use ONE decisive anchor rather than narrating every detail: a meaningful delay, the useful number of attempts, or the category that captures a long list. Prefer "that all three attempts pushed the viewing without really establishing my position as a buyer" to listing budget, timescale, requirements and every unanswered question. It must NOT contain its commercial consequence. Keep it specific to this enquiry, written from the enquirer\'s perspective and about behaviour, never an abstract judgement or label. Address them as "you" and our side as "I"/"me"/"we".',
      },
      commercial_consequence: {
        type: 'string',
        description: 'EMAIL COPY, read by the prospect. MANDATORY for every sendable email and NEVER LEFT BLANK. THE MOST IMPORTANT FIELD. The email hard-codes "That meant ", so return ONLY the grammatically correct lower-case continuation. ONE concise sentence answering the actual "so what?": what opportunity the agency failed to capture, progress, qualify, convert or uncover because of the main finding. Move the story forward; do not repeat the finding or replay its evidence. Prefer the simplest commercially strong formulation, e.g. "you had a £650,000 buyer enquiry in front of you without knowing whether I was in a position to move forward." Preserve a confirmed property price when it makes the scale obvious, but never force one in or turn it into a fee, commission, percentage or lost revenue. Never claim a sale/instruction was definitely lost, speculate about prospect behaviour, or use vague filler. State only what remained unqualified, unbooked, unprogressed, unexplored or without a next step — then stop.',
      },
      wider_observation: {
        type: 'string',
        description: 'EMAIL COPY, read by the prospect. OPTIONAL, written from wider_finding_index and empty whenever that is null. ONE short standalone sentence naming a genuinely DISTINCT second opportunity, most commonly the evidenced seller side; never the main finding again and never a second evidence dump. Example: "I\'d also said I had a property of my own to sell that wasn\'t yet on the market." Return EMPTY unless the separate opportunity is real.',
      },
      wider_consequence: {
        type: 'string',
        description: 'EMAIL COPY, read by the prospect. OPTIONAL and only where wider_observation exists. The email hard-codes "That also meant ", so return ONLY the lower-case continuation. ONE short sentence stating the distinct commercial consequence, e.g. "a potential seller instruction inside the same enquiry was never explored." Do not repeat the observation or commercial_consequence, and never claim it would definitely have become an instruction. Leave empty rather than force one.',
      },
      novus_counterfactual: {
        type: 'string',
...
LENGTH AND IMPACT
For a normal-response email, aim for roughly 120–160 words IN TOTAL, including the fixed context, transition, CTA and sign-off that the assembler adds. No-response emails can be shorter. This is a target, not a quota: NEVER pad a clear story to reach it, and do not drop the one fact that makes a finding credible merely to save words. SAME INTELLIGENCE, FEWER WORDS, HARDER IMPACT.
The prospect-facing fields are not a transcript or an evidence summary. fair_observation, main_finding and commercial_consequence are each ONE concise sentence. Keep one decisive proof point where useful — a property value, important response delay, useful attempt count or seller opportunity — and compress long lists into the category they establish. Do not repeat timestamps, channels, questions or evidence already doing work in another field. wider_observation plus wider_consequence is at most TWO short sentences in total.
...
- fair_observation: ONE concise sentence acknowledging the strongest genuine positive, with at most one useful proof point; never a list or a hedged compliment.
- main_finding: ONE concise sentence naming the primary failure, with the minimum evidence that makes it undeniable. Compress a long question/detail list into what it establishes. It must NOT contain its commercial consequence.
- commercial_consequence: ONE concise sentence delivering the actual "so what?" — what was not captured, progressed, qualified, converted or uncovered. It must move forward, never restate the finding or repeat its evidence.
- wider_observation / wider_consequence: the optional pair, written from wider_finding_index; at most TWO short sentences together, kept when a genuinely distinct seller/valuation or other opportunity is evidenced.
...
EVERY SENTENCE NEEDS A JOB. The email is short, and each paragraph does one thing. Say it once, in the field that owns it, and stop:
  ...
  - The fair observation establishes fairness: one genuine positive and, at most, the single proof point that makes it concrete. Do not list everything they did well.
  - The main finding is what happened. Keep the decisive delay or attempt count when it matters, but turn a list such as budget/timescale/requirements/funding into "my position as a buyer" when that category makes the same point.
  - The consequence is what it meant. Make it simpler than the finding, not a second account of the evidence. Never put both jobs in one field, and never repeat the same point in both.
...
THE GRAMMAR CONTRACT is absolute, because the email supplies the fixed opening words of each paragraph and you supply the rest:
  ...
  - VOICE. You are the person who actually sent that enquiry, writing to the agency. They are "you". Our side is "I", "me", "we". Never write about them in detached third person — "They didn't let this one go cold" is wrong; "You didn't let this one go cold" is the same observation written by a human to a human. Keep sender and recipient straight: when the agency sent alternatives to the enquiry, write "I was sent three alternative properties", never "you were sent three alternative properties".
...
export function emailPropertyAddress(probe) {
  ...
  const cleaned = cleanAddressForEmail(probe?.property_address);
  return propertyReference(cleaned) ? cleaned : '';
}
...
const REPAIR_NOTES = {
  fair_observation: {
    ...
    placeholder: 'fair_observation was rejected because it contained an UNKNOWN, unresolved or placeholder value. Write only a genuine acknowledgement supported by the selected finding. Do not invent one; if nothing is supported, leave it empty and the email will remain unsendable.',
    perspective_inversion: 'fair_observation reversed sender and recipient. You are writing as the person who made the enquiry: if the agency sent alternatives to the enquiry, say "I was sent...", not "you were sent...". Keep legitimate references to the agency as "you".',
  },
  main_finding: {
    ...
    placeholder: 'main_finding was rejected because it contained an UNKNOWN, unresolved or placeholder value. Rewrite it only from the selected finding; never invent missing facts.',
    perspective_inversion: 'main_finding reversed sender and recipient. Write from the enquirer\'s perspective: what the agency sent to the enquiry was sent to "me"/"us", while the agency remains "you".',
  },
  positive_finding_index: {
  ...
  wider_observation: {
    ...
    perspective_inversion: 'wider_observation reversed sender and recipient. Write from the enquirer\'s perspective: what the agency sent to the enquiry was sent to "me"/"us", while the agency remains "you".',
  },
  commercial_consequence: {
    ...
    speculative_prospect_behaviour: 'commercial_consequence was rejected because it invented what the prospect might do next (for example lose interest or view something else). This probe cannot establish hypothetical behaviour. Keep the commercial force, but state only what remained unqualified, unbooked, unprogressed or unexplored, or that no next step was established.',
    placeholder: 'commercial_consequence was rejected because it contained an UNKNOWN, unresolved or placeholder value. Rewrite the consequence only from established facts; never invent replacement content.',
  },
};
...
export function classifyConsequenceLoss(raw, allowedFigure) {
  ...
  if (hasUnresolvedPlaceholder(text)) return 'placeholder';
...
const CONSEQUENCE_TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      commercial_consequence: {
        type: 'string',
        description: 'EMAIL COPY, read by the prospect. The email hard-codes "That meant ", so return ONLY the lower-case continuation. ONE concise sentence saying what the agency failed to establish, progress, capture, qualify, convert or uncover — never the finding or its evidence again. Prefer the simplest commercially strong formulation. NO monetary figures, fees, commission, percentages, lost revenue or analytical vocabulary. Never claim a sale/instruction was definitely lost or speculate about prospect behaviour. State only what remained unqualified, unbooked, unprogressed or unexplored, or that no next step was established.',
      },
    },
  },
};
...
3. Never claim a sale, a viewing or an instruction was definitely lost. Say what was never established or never explored.
...
5. Write only the continuation of "That meant ", starting lower-case. ONE concise sentence, then stop. Do not replay the evidence already stated in the finding.
6. Ground it in what you are given. Never invent anything that is not there.
7. Never speculate about what the prospect might do next — no losing interest, viewing something else, buying elsewhere or other hypothetical behaviour. State only what remained unqualified, unbooked, unprogressed or unexplored, or that no next step was established.`;
...
async function repairConsequence(best, ctx) {
  ...
  if (readsAsSpeculativeProspectBehaviour(consequence)) return '';
...
function buildCandidate(result, ctx) {
  ...
  if (!noHumanContact) {
    if (!fairObservationCandidate) {
      fairObservation = '';
      rejections.push({ field: 'fair_observation', reason: hasUnresolvedPlaceholder(rawFairObservation) ? 'placeholder' : 'blank' });
    ...
    } else if (readsAsPerspectiveInversion(fairObservationCandidate)) {
      fairObservation = '';
      rejections.push({ field: 'fair_observation', reason: 'perspective_inversion' });
    }
  }

  // The assembler supplies "What stood out, though, was ".
  const rawMainFinding = String(result.main_finding || '').trim();
  let mainFinding = noHumanContact
    ? ''
    : asContinuation(emailVariable(rawMainFinding, allowedFigure), FIXED_PREFIX_PATTERNS.mainFinding, protectedWords);
  if (!noHumanContact && !mainFinding) {
    rejections.push({ field: 'main_finding', reason: hasUnresolvedPlaceholder(rawMainFinding) ? 'placeholder' : 'blank' });
  } else if (readsAsPerspectiveInversion(mainFinding)) {
    mainFinding = '';
    rejections.push({ field: 'main_finding', reason: 'perspective_inversion' });
  }
  ...
  } else if (readsAsSpeculativeProspectBehaviour(consequenceCandidate)) {
    commercialConsequence = '';
    rejections.push({ field: 'commercial_consequence', reason: 'speculative_prospect_behaviour' });
  }
  ...
  if (widerObservation && readsAsPerspectiveInversion(widerObservation)) {
    widerObservation = '';
    rejections.push({ field: 'wider_observation', reason: 'perspective_inversion' });
  }
