...
import { TIMELINE_TABS, buildLeadTimeline } from './lead-timeline.mjs';
...
import { PROBE_CALL_CAMPAIGN_NAME, PROBE_CALL_CAMPAIGN_TYPE, PROBE_CALL_SEQUENCE, isProbeCallSequence, isMatchingInstantlyProbeCallCampaign } from './probe-call-campaign.mjs';
...
export function invalidateCampaignCache() { listCache = null; }
...
async function loadOptionalTables(repo, tabs, required = ['AGENCIES']) {
  const entries = await Promise.all(tabs.map(async (tab) => {
    try { return [tab, await repo.getTable(tab)]; }
      ...
      if (required.includes(tab)) throw err;
...
export async function handleCampaignsList(req, res) {
  ...
  try {
    ...
    const payload = {
      ...
      campaigns,
      enums: { statuses: CAMPAIGN_STATUSES, types: CAMPAIGN_TYPES, reason_labels: REASON_LABEL, default_policy: DEFAULT_POLICY, default_schedule: DEFAULT_SCHEDULE, default_sending: DEFAULT_SENDING, probe_call_sequence: PROBE_CALL_SEQUENCE, probe_call_name: PROBE_CALL_CAMPAIGN_NAME },
    };
...
export async function handleCampaignDetail(req, res) {
  ...
  try {
    ...
    if (upper(campaign.campaign_type) === PROBE_CALL_CAMPAIGN_TYPE) {
...
async function buildAudience(repo, { filters, policy, campaign, limit = 400, applyFilters = true }) {
  const [tables, loaded] = await Promise.all([loadOptionalTables(repo, AUDIENCE_TABS), readCampaignTables(repo)]);
  return buildCampaignAudience(tables, loaded.tables, { filters, policy, campaign, now: nowIso(), limit, applyFilters });
}
export async function handleCampaignAudience(req, res) {
  ...
  try {
    ...
    const audience = await buildAudience(getRepo(), {
      filters: body.filters || {}, policy: body.policy || {},
    ...
    return res.status(500).json({ success: false, error: err?.message || 'Failed to build the audience' });
  }
}
...
export async function handleCampaignCreate(req, res) {
  ...
  if (campaignType === PROBE_CALL_CAMPAIGN_TYPE) {
    if (name !== PROBE_CALL_CAMPAIGN_NAME) errors.push(`Campaign name must be ${PROBE_CALL_CAMPAIGN_NAME}`);
    if (!isProbeCallSequence(seq.sequence)) errors.push('The initial probe-call sequence must match the approved copy and delays');
    if (!snd.sending.stop_on_reply) errors.push('Stop on reply must be enabled');
    if (!normaliseFilters(body.filters || {}).agency_ids.length) errors.push('Select an explicit agency ID cohort before creating this campaign');
  }
  ...
  try {
    ...
    const policy = normalisePolicy({ ...(body.policy || {}), requires_probe: campaignType === PROBE_CALL_CAMPAIGN_TYPE || (body.policy?.requires_probe ?? (campaignType === 'ENQUIRY_FOLLOWUP')) });
    if (campaignType === PROBE_CALL_CAMPAIGN_TYPE) Object.assign(policy, { requires_probe: true, allow_risky_email: false, block_active_campaign: true, block_active_conversation: true, block_active_followup: true, block_prior_negative: true, block_meeting_booked: true, block_opted_out: true });
    ...
    const audience = await buildAudience(repo, { filters, policy, campaign: { campaign_id: campaignId, campaign_type: campaignType }, limit: 0 });
    ...
    return res.status(500).json({ success: false, error: err?.message || 'Campaign creation failed' });
  }
}
...
export async function handleCampaignUpdate(req, res) {
  ...
  try {
    ...
    if (!campaign) return res.status(404).json({ success: false, error: `No campaign ${campaignId}` });
    if (upper(campaign.campaign_type) === PROBE_CALL_CAMPAIGN_TYPE && ['name', 'sequence', 'sending', 'policy', 'campaign_type'].some((key) => body[key] !== undefined)) {
      return res.status(409).json({ success: false, error: 'The initial probe-call campaign copy and safety settings are fixed; create a new version for changes' });
    }
...
export async function handleCampaignPush(req, res) {
  ...
  try {
    ...
    if (probeCall && (!isProbeCallSequence(parseJson(campaign.sequence_json, {})) || !parseJson(campaign.sending_json, {}).stop_on_reply || !normaliseFilters(parseJson(campaign.audience_filters_json, {})).agency_ids.length)) {
      return res.status(409).json({ success: false, error: 'Probe-call campaign configuration is incomplete or changed; nothing was pushed' });
    }
    ...
    if (!instantlyId) {
      try {
        const created = await client.createCampaign(buildInstantlyCampaignPayload({
          name: text(campaign.name), sequence: parseJson(campaign.sequence_json, { steps: [] }),
          schedule: { ...DEFAULT_SCHEDULE, ...parseJson(campaign.schedule_json, {}) }, sending: { ...DEFAULT_SENDING, ...parseJson(campaign.sending_json, {}) },
        }));
    ...
    if (probeCall) {
      try {
        const remoteCampaign = await client.getCampaign(instantlyId);
        if (!isMatchingInstantlyProbeCallCampaign(remoteCampaign)) throw new Error('Instantly draft copy, threading, delays or stop-on-reply setting differs from the approved version');
        ...
        return res.status(502).json({ success: false, error: `Could not reconcile existing Instantly members: ${instantlyErrorPayload(err).detail || err.message}`, ...result });
      }
    }
