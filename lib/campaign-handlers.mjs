...
//        campaign-delete         remove an unsent NOVUS draft and its member snapshot
...
import { TIMELINE_TABS, buildLeadTimeline, buildLeadProfile, buildCallingProfile } from './lead-timeline.mjs';
...
export async function handleCampaignDetail(req, res) {
  ...
  try {
    ...
    const payload = {
      ...
      campaign: { ...view, bounce_rate: bounceRate(view), can_delete_draft: campaignDeletionBlockers(campaign, members, events).length === 0 },
...
// ── GET calling-lead-profile ───────────────────────────────────────────────
// The lead profile a SETTER may open (lib/novus-users.mjs SETTER_OPERATIONS):
// any agency, in or out of their calling queue, narrowed by
// buildCallingProfile. Same tables as lead-timeline; READ-ONLY, no writer and
// no external call is reachable from here.
export async function handleCallingLeadProfile(req, res) {
  noStore(res);
  const agencyId = text(req.query?.agency_id);
  if (!agencyId) return res.status(400).json({ success: false, error: 'Missing agency_id' });
  try {
    ...
    const tables = await loadOptionalTables(repo, TIMELINE_TABS);
    const now = nowIso();
    const lead = buildCallingProfile(buildLeadTimeline(tables, agencyId, { now }), buildLeadProfile(tables, agencyId, { now }));
    if (!lead) return res.status(404).json({ success: false, error: `No agency ${agencyId}` });
    return res.status(200).json({ success: true, generated_at: now, lead });
  } catch (err) {
    console.error('calling-lead-profile error:', err);
    return res.status(500).json({ success: false, error: err?.message || 'Failed to build the lead profile' });
  }
}
...
// The exact POST /campaigns body. Instantly's editor discarded the text nodes
// from bare text separated by <br/> on the live A1 draft, leaving only empty
// blocks. Put each line in a block, including blank lines, and escape copy as
// HTML text while leaving {{variables}} intact. NOVUS step N's
// delay_days is "wait before this step"; Instantly's step delay is "wait
// before the NEXT step", so the values shift by one.
function instantlyBodyHtml(body) {
...
export function buildInstantlyCampaignPayload({ name, sequence, schedule, sending }) {
  const steps = sequence.steps.map((step, i) => ({
    ...
    pre_delay: 0,
    pre_delay_unit: 'days',
    variants: step.variants.filter((v) => !v.disabled).map((v) => ({ subject: v.subject, body: instantlyBodyHtml(v.body) })),
  }));
  const payload = {
    name,
    campaign_schedule: {
...
// A draft label alone is not evidence that nothing was sent. Keep the event
// ledger as historical acquisition evidence, even after its draft is removed.
export function campaignDeletionBlockers(campaign, members, events) {
  const blockers = [];
  if (upper(campaign.source) === 'IMPORTED') blockers.push('Linked Instantly campaigns cannot be deleted from NOVUS');
  if (!['DRAFT', 'ERROR'].includes(upper(campaign.status))) blockers.push('Campaign is not a draft');
  if (text(campaign.launched_at) || text(campaign.paused_at) || text(campaign.completed_at)) blockers.push('Campaign was launched or resumed; sending history cannot be ruled out');
  if (Number(parseJson(campaign.analytics_json, {})?.emails_sent_count) > 0) blockers.push('Instantly analytics record sent emails');
  if (members.some((m) => Number(m.emails_sent_count) > 0 || text(m.replied_at) || upper(m.last_event_type) === 'EMAIL_SENT')) blockers.push('Campaign members record sent emails or replies');
  if (members.some((m) => text(m.unsubscribed_at) || text(m.bounced_at) || text(m.meeting_booked_at) || text(m.interest_status))) blockers.push('Campaign members hold suppression or outcome history');
  if (events.some((e) => ['CAMPAIGN_LAUNCHED', 'CAMPAIGN_RESUMED', 'EMAIL_SENT', 'REPLY_RECEIVED', 'AUTO_REPLY_RECEIVED', 'EMAIL_BOUNCED', 'MANUAL_REPLY_SENT'].includes(upper(e.event_type)))) blockers.push('Campaign event ledger contains launch or email activity');
  return blockers;
}

// Deletes only CAMPAIGNS and CAMPAIGN_MEMBERS rows. The append-only campaign
// event ledger, agencies, contacts, probes and suppression data are retained.
// An existing provider campaign must be removed there first; this endpoint
// never deletes Instantly leads or a remotely managed draft as a side effect.
export async function handleCampaignDelete(req, res) {
  noStore(res);
  const body = req.body && typeof req.body === 'object' ? req.body : {};
  const campaignId = text(body.campaign_id);
  if (text(body.confirm) !== CONFIRM.DELETE) return res.status(400).json({ success: false, error: `Missing confirm=${CONFIRM.DELETE}` });
  if (!campaignId) return res.status(400).json({ success: false, error: 'Missing campaign_id' });
  try {
    const repo = getRepo();
    const { loaded, record, campaign, members, events } = await loadCampaign(repo, campaignId);
    if (!campaign) return res.status(404).json({ success: false, error: `No campaign ${campaignId}` });
    const blockers = campaignDeletionBlockers(campaign, members, events);
    if (blockers.length) return res.status(409).json({ success: false, error: blockers.join('; '), blockers });

    const instantlyId = text(campaign.instantly_campaign_id);
    if (instantlyId) {
      const client = clientFor('read');
      if (!client) return res.status(503).json({ success: false, error: 'Instantly read access is required to verify the linked draft is gone' });
      try {
        const remote = await client.getCampaign(instantlyId);
        return res.status(409).json({ success: false, error: `Instantly campaign ${remote.id} still exists. Delete that draft in Instantly first; NOVUS will not delete it automatically.` });
      } catch (err) {
        if (!(err instanceof InstantlyApiError && err.status === 404)) {
          return res.status(502).json({ success: false, error: 'Could not verify whether the linked Instantly campaign still exists; nothing was deleted', instantly: instantlyErrorPayload(err) });
        }
      }
      try {
        const history = await client.listCampaignEmails(instantlyId, { maxPages: 50 });
        if (history.truncated) return res.status(409).json({ success: false, error: 'Instantly email history was truncated; deletion cannot be verified' });
        if (history.items.length) return res.status(409).json({ success: false, error: 'Instantly still has email history for this campaign; the NOVUS record must be kept' });
      } catch (err) {
        return res.status(502).json({ success: false, error: 'Could not verify Instantly email history; nothing was deleted', instantly: instantlyErrorPayload(err) });
      }
    }

    const memberRows = parseRecords(loaded.tables[CAMPAIGN_MEMBERS_TAB], 'member_id')
      .filter((r) => text(r.obj.campaign_id) === campaignId).map((r) => r.rowNumber);
    // Remove member snapshots first. If the parent removal fails, the same
    // confirmed action can be retried; no orphan membership blocks recreation.
    if (memberRows.length) await repo.deleteRows(CAMPAIGN_MEMBERS_TAB, memberRows);
    await repo.deleteRows(CAMPAIGNS_TAB, [record.rowNumber]);
    invalidateCampaignCache();
    return res.status(200).json({ success: true, campaign_id: campaignId, members_deleted: memberRows.length, campaign_events_preserved: events.length, instantly_campaign_deleted: false });
  } catch (err) {
    invalidateCampaignCache();
    console.error('campaign-delete error:', err);
    return res.status(500).json({ success: false, error: err?.message || 'Campaign deletion failed' });
  }
}
