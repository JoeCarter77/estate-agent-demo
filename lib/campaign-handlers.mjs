...
export async function handleCampaignsList(req, res) {
  ...
  try {
    ...
    const payload = {
      ...
      campaigns,
      enums: { statuses: CAMPAIGN_STATUSES, types: CAMPAIGN_TYPES, reason_labels: REASON_LABEL, default_policy: DEFAULT_POLICY, default_schedule: DEFAULT_SCHEDULE, default_sending: DEFAULT_SENDING },
    };
...
export function normaliseSequence(input) {
  ...
  out.forEach((step) => {
    if (!step.variants.some((v) => v.subject && v.body && !v.disabled)) errors.push(`step ${step.step} needs a subject and a body`);
...
export async function handleCampaignCreate(req, res) {
  ...
  try {
    ...
    const policy = normalisePolicy({ ...(body.policy || {}), requires_probe: body.policy?.requires_probe ?? (campaignType === 'ENQUIRY_FOLLOWUP') });
...
export async function handleCampaignPush(req, res) {
  ...
  try {
    ...
    if (!instantlyId) {
      ...
      } catch (err) { result.errors.push({ stage: 'list_campaigns', ...instantlyErrorPayload(err) }); }
    }
    if (!instantlyId) {
      try {
