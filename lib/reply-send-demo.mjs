...
//
// CONCURRENCY. executeSendDemo is the ONLY function here that POSTs, and both
// the automatic send from the reply poll and the manual send-demo route go
// through it — so the cross-instance claim (lib/reply-claim.mjs) is taken
// inside it, immediately before the POST, and covers both callers plus any
// overlap between them. It is keyed on novus:send:<instantly_email_id>, NOT on
// reply_event_id: every per-row marker below (notes, action_completed_at,
// action_status) is blank on BOTH of the two rows a duplicate-append race
// produces for one email, so only an email-keyed claim collapses them to one
// send. The claim ADDS to the existing safety model and replaces none of it —
// see the release policy in executeSendDemo for why a successful send holds its
// claim to expiry rather than returning it.
//
...
// no OUTBOUND write. The live mode is executeSendDemo() below, and the
// payload/transport boundary it crosses is sendDemoReply — which now lives in
// lib/instantly-reply-send.mjs and is re-exported here under that name.
...
// THE GENERIC WIRE. Every one of these used to be defined in this file. They
// moved to lib/instantly-reply-send.mjs so Joe's manual replies go out over the
// SAME transport as an automatic SEND_DEMO rather than a copy of it. Nothing
// about their behaviour changed in the move, and they are re-exported below so
...
// The demo path's historical name for the generic transport. Kept as the same
// function object, not a wrapper, so the live send is literally unchanged.
export const sendDemoReply = sendInstantlyReply;
...
// be confused. A CLASSIFIER error means the row's verdict cannot be trusted, so
// it blocks. A SEND error means a previous ATTEMPT failed, which is precisely
// the state a retry exists for — blocking on it would make FAILED terminal and
// leave every transient Instantly failure permanently stuck.
//
// isSendExecutionError now lives with the code that PRODUCES those prefixes,
// in lib/instantly-reply-send.mjs, and is imported and re-exported above.
...
export const BLOCKED_REASONS = [
  ...
  // The last two are produced by executeSendDemo, NOT by evaluateSendDemoGate.
  // The gate is pure and takes no claim; these describe the cross-instance
  // guard taken immediately before the POST, which only the live path reaches.
  // They are listed here so every reason a send can be refused has one home.
  'CONCURRENT_SEND_IN_PROGRESS',
  'CLAIM_STORE_UNAVAILABLE',
];

// asText/trimmed/safeDetail are imported from the shared transport above, so
// both senders coerce and trim values identically.

// ---------------------------------------------------------------------------
// THE REPLY COPY. Fixed, minimal, and NOT generated.
//
// No extra CTA, no reselling, no model-written prose. The only variables are
// the demo URL and, when available, the prospect's first name (the greeting
// line is simply omitted when it is not — see buildDemoReplyBody). A property
// reference is DEFERRED: OUTBOUND carries property_street, but inserting it
// changes agreed copy for no gate-relevant reason, and an unsafe reference is
// worse than none.
// ---------------------------------------------------------------------------
const DEMO_REPLY_GREETING_LINE = 'Hi {{first_name}},';
...
//
// firstName is optional: when the resolved OUTBOUND row carries one (see
// deriveFirstName in lib/outbound.mjs), the greeting line is prepended; when
// it does not, the greeting line is omitted rather than inventing one — the
// same "no data, no line" behaviour the rest of this template already follows.
export function buildDemoReplyBody(demoUrl, firstName) {
  const url = trimmed(demoUrl);
  if (!url) throw new Error('buildDemoReplyBody requires a demo_url');
  const name = trimmed(firstName);
  const templateLines = name
    ? [DEMO_REPLY_GREETING_LINE, ...DEMO_REPLY_TEMPLATE_LINES]
    : DEMO_REPLY_TEMPLATE_LINES;
  const lines = templateLines.map((line) => line.replace('{{demo_url}}', url).replace('{{first_name}}', name));
  return {
    text: lines.join('\n'),
...
// against silently.
// ---------------------------------------------------------------------------
// Imported from lib/instantly-reply-send.mjs — every sender must answer "which
// OUTBOUND row is this conversation" with the same strictness. See that file.
...
export function evaluateSendDemoGate({
  ...
  return {
    ...
    reply_body: eligible ? buildDemoReplyBody(demoUrl, outbound.row?.first_name) : null,
    would_send: eligible,
...
// ---------------------------------------------------------------------------
// INSTANTLY_REPLY_URL and buildInstantlyReplyPayload are imported from
// lib/instantly-reply-send.mjs and re-exported above.
...
// ---------------------------------------------------------------------------
// THE SEND. One POST to Instantly, and the three-way classification of its
// outcome (SENT / REJECTED / AMBIGUOUS), now shared with every other sender:
// see sendInstantlyReply in lib/instantly-reply-send.mjs. It is re-exported at
// the top of this file under its historical name, sendDemoReply, as the SAME
// function object — the wire this path uses did not change, it only moved.
// ---------------------------------------------------------------------------
...
export async function executeSendDemo({
  ...
  // -------------------------------------------------------------------------
  // THE CROSS-INSTANCE SEND CLAIM. Taken HERE: after the gate said yes, and
  // immediately before the POST. Nothing between these two lines can send.
  //
  // WHY THIS FILE AND NOT THE CALLERS. executeSendDemo is the ONLY path that
  // POSTs, and both the automatic send from the reply poll and the manual
  // /api/novus/instantly/send-demo route go through it. Guarding here covers
  // both, and covers them overlapping with each other, without either caller
  // having to know the claim exists.
  //
  // KEYED ON instantly_email_id, NOT reply_event_id. This is the point of the
  // whole exercise: two overlapping polls can produce two REPLY_EVENTS rows
  // with DIFFERENT reply_event_ids for the SAME prospect email, and every
  // per-row marker (notes, action_completed_at, action_status) is blank on
  // both. Keying on the email is what collapses those two rows to one send.
  //
  // A LOST CLAIM WRITES NOTHING — same rule as a gate block above. A send that
  // was refused because another instance is mid-flight is not a failed send,
  // and must not stamp FAILED over a row whose real outcome is being decided
  // elsewhere.
  //
  // FAILS CLOSED. An unconfigured store throws out of getClaimStore(); a store
  // that is unreachable returns acquired:false with an error. Neither can
  // become a send.
  // -------------------------------------------------------------------------
  let claimStore;
  try {
    claimStore = getClaimStore();
  } catch (err) {
    return {
      ...base,
      eligible: false,
  ...
  const claimKey = sendClaimKey(gate.instantly_email_id);
  const claim = await claimStore.acquire(claimKey, SEND_CLAIM_TTL_SECONDS);
  if (!claim.acquired) {
    ...
    return {
      ...
      eligible: false,
      sent: false,
      blocked_reason: storeFailed ? 'CLAIM_STORE_UNAVAILABLE' : 'CONCURRENT_SEND_IN_PROGRESS',
      blocked_reasons: [storeFailed ? 'CLAIM_STORE_UNAVAILABLE' : 'CONCURRENT_SEND_IN_PROGRESS'],
      would_send: false,
      send_outcome: null,
      instantly_status: null,
      row_update: null,
  ...
  // -------------------------------------------------------------------------
  // RELEASE POLICY. The claim is a guard, not a lock to be tidily returned, and
  // when it is held to expiry that is the safety property rather than a leak.
  //
  //   REJECTED (4xx) — Instantly refused BEFORE sending. Definitively nothing
  //     left. Released at once, because a retry cannot duplicate anything and
  //     holding the key would stall a legitimate one for the full TTL.
  //
  //   SENT (2xx) — HELD to expiry. This is requirement-critical: releasing here
  //     would open exactly the window this change exists to close, where a
  //     concurrent caller that had already passed the gate on a stale read
  //     immediately re-sends. By the time the claim lapses, three independent
  //     markers have taken over — action_status=COMPLETED, the notes stamp, and
  //     the sent message now indexed in Instantly for demoSentEvidence. The
  //     claim covers precisely the interval before those become visible; it
  //     does not replace any of them.
  //
  //   AMBIGUOUS (5xx / transport / timeout) — HELD to expiry. The send may have
  //     happened. Releasing would invite a blind retry, which is the one thing
  //     the AMBIGUOUS_SEND_RESULT model exists to prevent.
  //
  // Best effort: a failed release only ever costs a delayed retry, so it is
  // recorded and never allowed to throw over a send that already happened.
  // -------------------------------------------------------------------------
  let claimReleased = false;
  let claimReleaseError = null;
  if (result.outcome === 'REJECTED') {
    try {
      claimReleased = await claimStore.release(claimKey, claim.token);
    } catch (err) {
  ...
  return {
    ...
    // Held deliberately on SENT and AMBIGUOUS; see the release policy above.
