...
export function resolveRow(sourceRow, liveRow) {
  ...
  // THE CORE RULE: without a genuine agency-profile URL, neither 'confirmed' nor
  // 'candidate' can stand. Confirmed asserts a verified profile; candidate
  // asserts a likely one. If we hold no profile URL, the honest status is
  // 'unresolved'. This is the specific failure the re-check existed to catch,
  // and it also stops unrecognised status wording (which falls back to
  // candidate) from implying a profile we do not actually have.
  //
  // 'not_applicable' is exempt: a lettings/non-sales agency is legitimately
  // classified without us ever holding a sales-profile URL for it.
  if (!isGenuine && (status === RIGHTMOVE_STATUS.CONFIRMED || status === RIGHTMOVE_STATUS.CANDIDATE)) {
    downgraded = {
      from: status,
      to: RIGHTMOVE_STATUS.UNRESOLVED,
      why: rawUrl
        ? `Status "${status}" is not supported by the URL supplied — it is not a Rightmove agency profile (${classified.reason})`
        : `Status "${status}" claims a profile but no agency-profile URL was supplied`,
    };
    status = RIGHTMOVE_STATUS.UNRESOLVED;
  }
...
export async function migrateRightmove(repo, rows, options = {}) {
  ...
  // Index the live table once, by agency_id, and keep each row's position too —
  // so every write below can go straight to its known row instead of each
  // repo.updateById() call re-reading the whole sheet to relocate it. That
  // per-row re-read (2 GET calls per write: one inside updateById, one inside
  // its own findById) was the source of the migration's 429 RESOURCE_EXHAUSTED:
  // one API round trip per row instead of the two total this loads.
  const liveRecords = await repo.getRecords('AGENCIES', 'agency_id');
  const { header } = await repo.getTable('AGENCIES');
  const live = new Map();
  const rowNumberByAgencyId = new Map();
  for (const r of liveRecords) {
    ...
    if (!live.has(r.obj.agency_id)) {
      live.set(r.obj.agency_id, r.obj);
      rowNumberByAgencyId.set(r.obj.agency_id, r.rowNumber);
    }
  }
  ...
  for (const raw of rows || []) {
    ...
    try {
      const merged = { ...liveRow, ...effective };
      await repo.writeRecordAtRow('AGENCIES', header, rowNumberByAgencyId.get(agencyId), merged);
