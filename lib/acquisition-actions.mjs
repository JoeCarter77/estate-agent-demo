  ...
  switch (stage) {
    ...
    case 'REPLIED_NEEDS_HUMAN': {
      if (upper(latest?.classification) === 'CALL_REQUESTED' && stage === 'MEETING_INTENT') {
        ...
        const phones = replyPhoneNumbers(body);
        ...
        const needsReview = phones.length > 1 || timing.needs_review || sameEmail.length > 1;
        let replyNotes = {};
        ...
        const action = expected(evidence, stage, 'CALL_PROSPECT', 'JOE', timing.due_at || now,
          needsReview ? 'Call requested by email — check number or timing before dialling' : 'Contact requested a call by email', anchor);
        ...
        if (sourceOutbound?.probe_id) action.probe_id = text(sourceOutbound.probe_id);
        action.metadata_json = JSON.stringify({
          call_action: true, source: 'EMAIL_REPLY', priority: 'HIGH', reply_event_id: text(latest?.reply_event_id),
          phone: phones.length === 1 ? phones[0] : null, phone_candidates: phones,
          ...
          timing, needs_review: needsReview, reply_text: body, campaign_id: text(latest?.campaign_id),
  ...
  for (const [key, row] of wanted) {
    ...
    if (!found && !completed.has(key)) create.push(row);
      ...
      if (status !== upper(found.action_status) || text(found.due_at) !== text(dueAt)
          || text(found.reason) !== text(reason) || upper(found.action_owner) !== owner) {
        update.push({ action_id: found.action_id, patch: { due_at: dueAt, reason, action_owner: owner, action_status: status, updated_at: now } });
      }
    }
  }
