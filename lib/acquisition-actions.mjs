  ...
  switch (stage) {
    ...
    case 'REPLIED_NEEDS_HUMAN': {
      if (upper(latest?.classification) === 'CALL_REQUESTED' && stage === 'MEETING_INTENT') {
        ...
        const probeCall = upper(replyNotes.source_campaign_type) === PROBE_CALL_CAMPAIGN_TYPE;
        ...
        const action = expected(evidence, stage, 'CALL_PROSPECT', 'JOE', timing.due_at || now,
          probeCall && !storedPhone ? 'CRITICAL: owner accepted five-minute call — obtain a callback number' :
            needsReview ? 'CRITICAL: owner requested a call — check number or timing before dialling' : 'CRITICAL: owner requested a five-minute call by email', anchor);
