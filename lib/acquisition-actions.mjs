...
export function reconcileActions(existing, expectedRows, now = new Date().toISOString()) {
  ...
  for (const [key, row] of wanted) {
    ...
    if (!found && !completed.has(key)) create.push(row);
      ...
      if (status !== upper(found.action_status) || text(found.due_at) !== text(row.due_at)
          || text(found.reason) !== text(row.reason) || upper(found.action_owner) !== owner) {
        update.push({ action_id: found.action_id, patch: { due_at: row.due_at, reason: row.reason, action_owner: owner, action_status: status, updated_at: now } });
      }
    }
  }
  for (const [key, row] of active) {
    ...
    if (!wanted.has(key) && !manual) cancel.push({ action_id: row.action_id, patch: {
