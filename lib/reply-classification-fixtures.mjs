...
// ---------------------------------------------------------------------------
// LIVE BODY FORMAT. This is what actually arrives — NOT plaintext.
//
// Gmail separates every line, including quoted ones, with <br> rather than \n,
// and the quote header carries the sending address in angle brackets. Testing
// only against clean plaintext is exactly why two live replies ("Yes",
// "Yeah sure thing mate") reached MANUAL_REVIEW while every fixture passed:
// cleaned_reply_text came out as "Yes<br><br>" and matched no affirmative.
//
// Keep these byte-faithful. Do not "tidy" them into plaintext.
const LIVE_QUOTED_PARENT_LINES = [
  '> Hi Joe,',
  '>',
  '> I sent your team an enquiry about Milton Road on 11 August at 22:19.',
  '>',
...
// U+202F, the narrow no-break space Gmail puts before "PM".
export function liveGmailReplyBody(currentReply, novusAddress) {
  return `${currentReply}<br><br>On Tue, Sep 1, 2026 at 2:15 PM Joe Carter <${novusAddress}> wrote:<br><br>${LIVE_QUOTED_PARENT_LINES}<br><br>`;
}

// The two real live regressions, in their real format, across BOTH live NOVUS
// sending identities.
export const LIVE_HTML_CASES = [
  {
    label: 'LIVE — "Yes" from a joe@trynovus.co.uk thread',
    reply: 'Yes',
    eaccount: 'joe@trynovus.co.uk',
    expectedCleaned: 'Yes',
    auto: true,
  },
  {
    label: 'LIVE — "Yeah sure thing mate" from a joe@novushq.co.uk thread',
    reply: 'Yeah sure thing mate',
    eaccount: 'joe@novushq.co.uk',
    expectedCleaned: 'Yeah sure thing mate',
    auto: true,
  },
  // Contradiction safety must survive the same HTML-ish format.
  {
    label: 'LIVE — "Yes mate, but call me first"',
    reply: 'Yes mate, but call me first',
    eaccount: 'joe@novushq.co.uk',
    expectedCleaned: 'Yes mate, but call me first',
    auto: false,
  },
  {
    label: 'LIVE — "Yeah sure mate, actually don\'t send it"',
    reply: "Yeah sure mate, actually don't send it",
    eaccount: 'joe@novushq.co.uk',
    expectedCleaned: "Yeah sure mate, actually don't send it",
    auto: false,
  },
  {
    label: 'LIVE — "Yes, how much?"',
    reply: 'Yes, how much?',
    eaccount: 'joe@novushq.co.uk',
    expectedCleaned: 'Yes, how much?',
    auto: false,
  },
];
...
export const SEND_DEMO_CTA_CASES = [
  ...
  { phrase: 'Morning Please do. Many thanks. Kind regards, Simon Morris', auto: true, label: 'REAL MORRIS ARMITAGE alternate-sender reply' },
