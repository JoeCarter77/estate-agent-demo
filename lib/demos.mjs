...
import { rewriteInternalProspectLanguage } from './prospect-language.mjs';
...
export const DEMOS_HEADER = [
  ...
  // Structured upstream diagnosis, copied verbatim for the next demo
  // renderer. Existing UI continues to use its deterministic reading.
  'enquiry_signals', 'unresolved_context', 'recommended_actions',
  'handling_summary', 'handling_quality',
...
function normaliseCollection(items) {
  return (Array.isArray(items) ? items : []).map((item) => {
    ...
    for (const [key, value] of Object.entries(out)) {
      if (key === 'source_type' || typeof value !== 'string') continue;
      out[key] = normaliseDashes(rewriteInternalProspectLanguage(value));
    }
    return out;
  });
}

function prospectCollectionJson(value) {
  return JSON.stringify(normaliseCollection(parseCollection(value)));
}

const PROSPECT_COPY_COLUMNS = new Set([
  'demo_headline', 'demo_hook', 'positive_observation', 'demo_reveal',
  'demo_reveal_support', 'main_finding', 'commercial_consequence',
  'novus_transition', 'scale_line', 'systemic_bridge', 'cta_headline',
  'handling_summary',
]);
...
// show their property, and show NOVUS recognising something real. Anything
// missing from that list is a reason for a human to look, not a reason to
// hide the demo — so it is recorded on the row (review_reasons) and the
// status becomes needs_review.
//
// Pure, and deliberately separate from buildDemoRow() so the rule can be read
// and tested on its own.
//
// commercial_consequence and positive_observation are NOT gated here even
// though the row still carries them (legacy PERSONALISATION compatibility
// columns, kept for audit history). The live page never reads either: beat
// 2's credit line and beat 3's commercial meaning are rendered from
// `reading.credit` / `reading.commercialMeaning`, which toRenderReady()
// derives via buildDemoReading() straight from this row's own ordinals
// (response time, contact attempts, viewing/seller progression) — see
// lib/demo-journeys.mjs. readingCredit() already returns blank cleanly when
// there is genuinely nothing to credit, and readingCommercialMeaning() always
// produces a safe, non-empty lead/body. Gating on the stored prose being
// blank was therefore holding otherwise-complete, factual demos at
// needs_review for a compatibility field nothing downstream consumes.
...
export function buildDemoRow({
  ...
  const row = {
    ...
    enquiry_signals: prospectCollectionJson(personalisation?.enquiry_signals),
    unresolved_context: prospectCollectionJson(personalisation?.unresolved_context),
    recommended_actions: prospectCollectionJson(personalisation?.recommended_actions),
    handling_summary: normaliseDashes(rewriteInternalProspectLanguage(text(personalisation?.handling_summary))),
    handling_quality: text(personalisation?.handling_quality),
...
// The five hero headlines are selected from the strongest commercial finding,
...
export function toRenderReady(row) {
  ...
  for (const [key, value] of Object.entries(row || {})) {
    ...
    out[key] = PROSPECT_COPY_COLUMNS.has(key) && typeof value === 'string'
      ? normaliseDashes(rewriteInternalProspectLanguage(value))
      : value;
  }
  ...
  // New upstream structured diagnosis is exposed as arrays, not JSON-shaped
  // strings, so the next renderer never has to reverse-engineer prose or
  // parse Sheet storage itself.
  out.enquiry_signals = normaliseCollection(parseCollection(row?.enquiry_signals));
  out.unresolved_context = normaliseCollection(parseCollection(row?.unresolved_context));
  out.recommended_actions = normaliseCollection(parseCollection(row?.recommended_actions));
