...
// The three story labels. Fixed, so the section always reads in the same
// order whatever a given probe's data turns out to contain.
export const STORY_LABELS = {
  enquiry: 'Enquiry sent about this property',
  firstResponse: 'First response',
  otherAttempts: 'Other contact attempts',
};
...
// What those attempts still never covered, from INTELLIGENCE.seller_recognition
// alone. A position that DID reach a valuation gets no gap sentence at all —
// there was no gap.
const SELLER_GAP_CLAUSE = {
  none: 'The seller opportunity was still never explored.',
  asked_position: 'The seller position was asked about once and never taken any further.',
  acknowledged: 'The seller position was acknowledged and never qualified.',
};

// -> the FIRST RESPONSE and OTHER CONTACT ATTEMPTS blocks, in that order.
// Returns [] when no human ever touched the enquiry, which is the honest
// answer for a complete_miss probe.
export function selectCommunicationEvidence({ communications, intelligence = {}, sellerDeclared = false } = {}) {
  const touches = sortedHumanTouches(communications);
  if (touches.length === 0) return [];

  const events = [];

  // 1) FIRST RESPONSE — the most useful sentence of the first human touch. An
  // unanswered call has no useful sentence in it: the fact IS the voicemail.
  ...
  events.push({ label: STORY_LABELS.firstResponse, detail: firstDetail, tone: 'good' });

  // 2) OTHER CONTACT ATTEMPTS — the remaining genuine attempts, summarised.
  // Same 30-minute grouping lib/observation.mjs uses for the INTELLIGENCE
  // count, so "3 contact attempts" in the metric strip and "2 further" here
  // can never disagree.
  const further = groupContactAttempts(touches).slice(1);
  const gapClause = sellerDeclared ? (SELLER_GAP_CLAUSE[text(intelligence?.seller_recognition)] || '') : '';
  ...
  if (further.length > 0) {
    ...
    attemptsSentence = 'No further contact attempts were made.';
  }

  events.push({
    label: STORY_LABELS.otherAttempts,
    detail: [attemptsSentence, gapClause].filter(Boolean).join(' '),
    tone: gapClause || further.length === 0 ? 'gap' : 'good',
  });
...
export function buildObservedEvents({
  ...
  if (sellerDeclared) {
    events.push({
      kind: 'evidence',
      label: STORY_LABELS.enquiry,
      detail: sellerDeclarationSummary(sellerDeclarationText),
      tone: 'neutral',
    });
  }

  for (const event of selectCommunicationEvidence({ communications, intelligence, sellerDeclared })) {
