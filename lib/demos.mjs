...
export function selectCommunicationEvidence({
  ...
  responseTime = '', responseHours = '', observationDays = null,
} = {}) {
  ...
  events.push({
    ...
    detail: [lag, firstDetail].filter(Boolean).join(' '),
    tone: 'good',
  });
...
export function buildObservedEvents({
  ...
  propertyAddress = '', enquiryDate = '', enquiryTime = '', observationDays = null,
}) {
  const events = [];
  ...
  const commEvents = selectCommunicationEvidence({
    ...
    responseHours: intelligence.response_hours, observationDays,
  });
