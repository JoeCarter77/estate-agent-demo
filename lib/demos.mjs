...
export function buildObservedEvents({
  intelligence, sellerDeclared, enquiryDate, enquiryTime, responseTime, communications = [],
}) {
  const events = [];
  ...
  // Real evidence where COMMUNICATIONS rows are matched to this probe (the
  // normal case — the compiler always passes them). Falls back to the old
  // INTELLIGENCE-only summary when there is nothing to draw from (a thin or
  // legacy row), so this section is never blank.
