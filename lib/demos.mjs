...
// half-rendered — and when the COPY frozen onto the row changes, since a row
// compiled by an earlier build keeps its old sentences until something makes
// the next pass recompile it.
...
// The automated side of the same list, in the same order. Kept separate from
// sortedHumanTouches() rather than derived from it, because the two are read
// for opposite reasons: one is what the team did, the other is what the
// systems did in place of them.
function sortedAutomatedTouches(communications) {
  return (communications || [])
    .filter((c) => !isHumanCommunication(c))
    .map((c) => ({ ...c, _occurredAt: occurredAtDate(c) }))
    .filter((c) => c._occurredAt !== null)
...
// ── the acknowledgement that arrived before anybody did ──────────────────────
//
// AN ACKNOWLEDGEMENT IS NOT PROGRESSION. That distinction is the whole reason
// this block exists, and nothing below is allowed to blur it: the sentence
// names the message as automated, and the human lag is stated straight after
// it, in the same breath.
//
// But saying nothing about it is its own kind of wrong. An owner who knows an
// auto-responder went out inside a minute reads "the first response came 3
// days later" as though we missed it. Naming it, and then naming what it was
// not, is both fairer and harder to argue with.

// Under this, the acknowledgement genuinely was instant — which is the half of
// the fact worth stating: the systems answered immediately, the people did not.
const IMMEDIATE_ACK_MINUTES = 15;

// What the acknowledgement itself promised, read from its own stored text.
// Fixed phrases, in the same spirit as every other classifier in this file —
// never a model call, and never a claim the message did not make.
const ACK_PROMISES_CONTACT =
  /\b(be in touch|been in touch|get back to you|getting back to you|come back to you|contact you|call you back|respond to you|reply to you)\b/i;

// What the acknowledgement WAS, as a thing that can be sent or made.
const ACK_CHANNEL_NOUNS = { voice: 'call', email: 'email', sms: 'text message', whatsapp: 'WhatsApp message' };

function ackSaysSomeoneWillFollow(comm) {
  return ACK_PROMISES_CONTACT.test(
    `${text(comm?.subject)} ${text(comm?.body_text)} ${text(comm?.transcript)}`,
  );
}

// True only where the enquiry timestamp is known AND the gap is genuinely
// small. An unknown enquiry time produces the timing-free wording rather than
// an assumed "immediately".
function ackWasImmediate(comm, enquiryAt) {
  const sent = occurredAtDate(comm);
  const enquiry = new Date(enquiryAt);
  if (!sent || Number.isNaN(enquiry.getTime())) return false;
  const minutes = (sent.getTime() - enquiry.getTime()) / 60000;
  return minutes >= 0 && minutes <= IMMEDIATE_ACK_MINUTES;
}

// -> the sentence(s) that go in FRONT of the first human response, or ''.
// MULTIPLE ACKNOWLEDGEMENTS ARE SUMMARISED, never listed: three auto-replies
// is one fact about the agency's systems, not three pieces of evidence.
export function automatedAckPrefix(autos, { enquiryAt = '', responseTime = '' } = {}) {
  if (!autos || autos.length === 0) return '';

  const first = autos[0];
  const immediate = ackWasImmediate(first, enquiryAt);
  const humanLag = text(responseTime);

  if (autos.length > 1) {
    return `Automated messages acknowledged the enquiry${immediate ? ' straight away' : ''}, but no`
      + ` meaningful human response followed${humanLag ? ` for ${humanLag}` : ''}.`;
  }

  // "An automated phone was sent" is not a sentence. The noun and its verb
  // come from the channel together, so a recorded call and an auto-reply each
  // read the way they actually happened.
  const noun = ACK_CHANNEL_NOUNS[String(first.channel || '').trim().toLowerCase()] || 'message';
  const verb = noun === 'call' ? 'was made' : 'was sent';
  const said = ackSaysSomeoneWillFollow(first)
    ? 'saying someone would be in touch'
    : 'confirming the enquiry had been received';

  return `An automated ${noun} ${verb}${immediate ? ' immediately' : ''} ${said}.`
    + ` The first meaningful human response came${humanLag ? ` ${humanLag}` : ''} later.`;
}
...
export function selectCommunicationEvidence({
  ...
  // WHAT ANSWERED BEFORE A PERSON DID. Only acknowledgements that went out
  // ahead of the first human touch belong in this line — an auto-reply that
  // arrived afterwards is not part of the story it tells.
  const ackPrefix = automatedAckPrefix(
    sortedAutomatedTouches(communications).filter((c) => c._occurredAt < first._occurredAt),
    { enquiryAt, responseTime },
  );
  // The acknowledgement sentence states the human lag itself, so the bare
  // "3 days after the enquiry." would say the same thing twice.
  const lag = ackPrefix || !text(responseTime) ? '' : `${responseTime} after the enquiry.`;
  ...
  events.push({
    ...
    detail: [ackPrefix, lag, firstDetail].filter(Boolean).join(' '),
    tone: 'good',
  });
...
export function reviewReasonsFor(row, { journeyWarning = '' } = {}) {
  ...
  // NO LONGER A REVIEW REASON: positive_observation is still compiled onto the
  // row (it is the sentence the outreach email opens with, and it is how a
  // human checks the demo is being fair), but the standalone credit line was
  // removed from beat 2 — so a blank one costs the prospect nothing, and
  // holding a link at needs_review over copy the page does not render would
  // 404 a demo that is genuinely complete.
...
export function buildDemoRow({
  ...
  const observedEvents = buildObservedEvents({
    ...
    enquiryDate,
    enquiryTime,
    // The raw timestamp, so the evidence can tell an acknowledgement that
    // arrived inside a minute from one that took half a day.
    enquiryAt,
