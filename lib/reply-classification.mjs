...
export const AI_CLASSIFICATIONS = CLASSIFICATIONS.filter(
  (c) => c !== 'OOO_AUTOMATED' && c !== 'OPT_OUT' && c !== 'POSITIVE_INTEREST',
);
...
CALL_REQUESTED — they explicitly invite or request a phone call, give a number to call, or accept the immediately preceding offer of a phone call. A proposed callback time still belongs here.
INFO_REQUESTED — they ask for general information or details by email. A specific request for the demo belongs in POSITIVE_SEND_DEMO.
POSITIVE_MEETING — they ask to book or coordinate a meeting or appointment. Do not use this for a simple phone call request.
...
OBJECTION — they describe an existing supplier, competing product, or reason to hesitate without clearly rejecting all further conversation. A human should answer the objection.
...
function meaningfulParts(cleanedReplyText) {
  ...
  for (const part of parts) {
    ...
    // A sign-off starts the signature block. Everything after it is identity,
    // contact details or a footer, not additional reply intent. Continuing
    // past "Many thanks" made a two-word name look like substantive prose and
    // defeated an otherwise exact "Please do" acceptance.
    if (isSignOff) break;
...
export function contextualAffirmativeDecision(reply, context) {
  const cta = extractCtaRegion(context?.previous_novus_message);
  if (previousNovusCtaType(context?.previous_novus_message) === 'CALL_OR_MEETING'
      && /\b(?:call|phone|ring|bell)\b/i.test(cta)
      && !SEND_DEMO_CTA_PATTERN.test(cta)
      && isSimpleAffirmative(reply?.cleaned_reply_text)) {
    return routed('CALL_REQUESTED', 'bare affirmative accepting the preceding phone-call CTA', CONTEXTUAL_AFFIRMATIVE_CONFIDENCE, { source: 'DETERMINISTIC_CONTEXTUAL' });
  }
...
// A CLEAR EXPRESSION OF INTEREST, which becomes a high-priority reply for a
// human — never a send. Two deterministic routes, both after every call, demo,
// opt-out and deferral rule above has declined:
//   1. an explicit interest phrase anywhere ("sounds interesting", "tell me
//      more"), with no negation or hedge;
//   2. a bare affirmative ("yes please", "go on then") answering an open
//      question that asked for neither a call nor a send — the founding-pilot
//      CTAs ("Would it be worth me explaining how?"). With no known question
//      a bare "yes" stays ambiguous and goes to review.
const INTEREST_PATTERN =
  /\b(?:sounds (?:interesting|intriguing|good|great)|(?:i'?m|i am|we'?re|we are|would be|i'?d be|we'?d be) (?:very |quite |definitely |certainly )?interested|(?:looks|seems) interesting|tell me more|keen to (?:hear|know|learn) more|(?:like|love|happy) to (?:hear|know|learn) more|interested to (?:hear|know|learn)|(?:go on|yes|yeah|sure),? (?:explain|tell me)|explain away|all ears)\b/i;
const INTEREST_CONTRADICTION_PATTERN =
  /\b(?:not|no|don'?t|do not|never|but|however|although|maybe|might|unsubscribe|remove|stop)\b/i;
export function interestDecision(reply, context) {
  const authored = String(reply?.cleaned_reply_text ?? '').trim();
  if (!authored) return null;
  if (INTEREST_PATTERN.test(authored) && !INTEREST_CONTRADICTION_PATTERN.test(authored)) {
    return routed('POSITIVE_INTEREST', 'prospect expressed clear interest', CONTEXTUAL_AFFIRMATIVE_CONFIDENCE, { source: 'DETERMINISTIC' });
  }
  if (previousNovusCtaType(context?.previous_novus_message) === 'OTHER' && isSimpleAffirmative(authored)) {
    return routed('POSITIVE_INTEREST', 'bare affirmative answering an open question in the preceding NOVUS message', CONTEXTUAL_AFFIRMATIVE_CONFIDENCE, { source: 'DETERMINISTIC_CONTEXTUAL' });
  }
  return null;
}

// "Try me in January": a deferral to a named month. NOT_NOW, so the follow-up
// is scheduled from the month (lib/reply-call-context.mjs) instead of landing
// in review. The timeframe vocabulary matches what the scheduler understands.
const TIMED_DEFERRAL_PATTERN =
  /\b(?:try (?:me|us)|come back to (?:me|us)|get back to (?:me|us)|(?:email|contact|drop|message) (?:me|us)|touch base|revisit (?:this|it)|speak|talk|chat|pick (?:this|it) up|reach out)\b[^.?!]*\b(?:in|early|mid|late|end of|beginning of|start of|after|until|from|around|towards|by)\s+(?:the\s+)?(?:new year|christmas|xmas|jan(?:uary)?|feb(?:ruary)?|march|april|may|june|july|aug(?:ust)?|sept?(?:ember)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)\b/i;
const TIMED_DEFERRAL_CONTRADICTION =
  /\?|\b(?:not interested|no thanks|remove|unsubscribe|stop|call me|ring me|give me a (?:call|ring|bell))\b/i;
export function timedDeferralDecision(reply) {
  const authored = String(reply?.cleaned_reply_text ?? '').trim();
  if (!TIMED_DEFERRAL_PATTERN.test(authored) || TIMED_DEFERRAL_CONTRADICTION.test(authored)) return null;
  return routed('NOT_NOW', 'prospect deferred to a named month', CONTEXTUAL_DEFERRAL_CONFIDENCE, { source: 'DETERMINISTIC' });
}
...
export async function classifyReply(reply, { aiCall = callAi, model, context = null } = {}) {
  ...
  const authored = String(reply?.cleaned_reply_text ?? '').trim();
  if (/\b(?:send (?:me|us) (?:some |more )?(?:information|info|details)|more information please)\b/i.test(authored)
      && !/\?|\b(?:demo|breakdown|don't|do not)\b/i.test(authored)) {
    return routed('INFO_REQUESTED', 'prospect requested more information by email', 0.97, { source: 'DETERMINISTIC' });
  }
  if (/\balready (?:got|have|use|using)\b/i.test(authored)
      && !/\?|\b(?:not interested|no thanks|remove me|unsubscribe)\b/i.test(authored)) {
    return routed('OBJECTION', 'prospect raised an existing-supplier objection', 0.93, { source: 'DETERMINISTIC' });
  }
  // A direct invitation is sufficiently explicit to work even when the AI
  // service is unavailable. Questions and contradictions still go to the model.
  if (/\b(?:give me (?:a |the )?(?:call|ring|bell)|call me|ring me|phone me|give us (?:a |the )?(?:call|ring|bell))\b/i.test(authored)
      && !/\?|\b(?:don't|do not|not interested|no thanks|maybe)\b/i.test(authored)) {
    return routed('CALL_REQUESTED', 'prospect explicitly requested a phone call', 0.97, { source: 'DETERMINISTIC' });
  }
  if (/\b(?:yes|yeah|sure|happy to)\b/i.test(authored)
      && (/\b(?:happy to (?:chat|talk)|my (?:mobile|number) is)\b/i.test(authored) || replyPhoneNumbers(reply?.raw_body_text || authored).length === 1)
      && !/\?|\b(?:don't|do not|not interested|no thanks|maybe)\b/i.test(authored)) {
    return routed('CALL_REQUESTED', 'prospect accepted a call or supplied a callback number', 0.97, { source: 'DETERMINISTIC' });
  }
  if (previousNovusCtaType(context?.previous_novus_message) === 'CALL_OR_MEETING'
      && !/\?|\b(?:don't|do not|not interested|no thanks|maybe)\b/i.test(authored)
      && (replyPhoneNumbers(reply?.raw_body_text || authored).length > 0 || /\b(?:happy to chat|happy to talk|yes,? happy to|sure,? (?:my mobile|my number))\b/i.test(authored))) {
    return routed('CALL_REQUESTED', 'prospect accepted the preceding call request', 0.97, { source: 'DETERMINISTIC_CONTEXTUAL' });
  }
  ...
  const contextual = contextualAffirmativeDecision(reply, context)
    ...
    || timedDeferralDecision(reply)
    || interestDecision(reply, context);
