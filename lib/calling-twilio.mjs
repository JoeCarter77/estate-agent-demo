...
import { CALLS_TAB, isDiscardedCall, patchCallCells, rowFor } from './calling-store.mjs';
import { CALL_TIMINGS_HEADER, CALL_TIMINGS_SCHEMA_NOTE, CALL_TIMINGS_TAB, newTimingId } from './call-sessions.mjs';
...
export async function handleVoiceOutbound(req, res) {
  ...
  try {
    ...
    if (isDiscardedCall(record.obj)) return reject('This call was discarded.');
...
export async function handleVoiceStatus(req, res) {
  ...
  try {
    ...
    // A discarded row keeps call_status=discarded whatever Twilio reports
    // later (metadata_json.discarded is the second, independent flag);
    // its timings are still filled in for the audit trail.
    if (!isDiscardedCall(row) && (STATUS_RANK[status] ?? -1) >= (STATUS_RANK[current] ?? -1) && status) patch.call_status = status === 'answered' ? 'in-progress' : status;
    ...
    // Twilio timing evidence for talk time (lib/call-sessions.mjs). Append-
    // only, in its own tab, so it can never race the CALLS writes above; a
    // failure here never affects the call.
    await appendTimingEvidence(repo, row, body, { isDialAction, status, now })
      .catch((err) => console.error('call-timings append failed:', err?.message || err));
    return done();
  } catch (err) {
...
const TIMING_STATUSES = new Set(['initiated', 'answered', 'in-progress', 'completed', 'busy', 'no-answer', 'failed', 'canceled']);
let timingsTabReady = false;
async function appendTimingEvidence(repo, row, body, { isDialAction, status, now }) {
  if (!TIMING_STATUSES.has(status)) return;
  // Only the dialled (child) leg's events measure the prospect's line; the
  // <Dial> action is kept for its DialCallDuration as a cross-check.
  const isChild = !isDialAction && Boolean(text(body.ParentCallSid));
  if (!isChild && !isDialAction) return;
  if (!timingsTabReady) {
    await repo.ensureTab(CALL_TIMINGS_TAB, [...CALL_TIMINGS_HEADER], ['SCHEMA NOTE', CALL_TIMINGS_SCHEMA_NOTE, ...CALL_TIMINGS_HEADER.slice(2).map(() => '')]);
    timingsTabReady = true;
  }
  const twilioTs = Date.parse(text(body.Timestamp));
  await repo.appendRowsBatch(CALL_TIMINGS_TAB, [rowFor(CALL_TIMINGS_HEADER, {
    timing_id: newTimingId(), call_id: text(row.call_id), parent_call_sid: text(body.ParentCallSid) || text(body.CallSid),
    leg_call_sid: isChild ? text(body.CallSid) : text(body.DialCallSid), source: isChild ? 'CHILD_STATUS' : 'DIAL_ACTION', status,
    twilio_timestamp: Number.isFinite(twilioTs) ? new Date(twilioTs).toISOString() : '', received_at: now,
    call_duration_seconds: isChild ? text(body.CallDuration) : '', dial_call_duration_seconds: isDialAction ? text(body.DialCallDuration) : '',
  })]);
}
