...
// The DIAGNOSIS and INTELLIGENCE rows are still READ here, for the two
// deterministic decisions code owns (the eligibility gate, the email variant
// and the hero-journey lookup) — they are simply never put in front of the
...
// same number: one probe costs one call unless its answer fails the email
// contract and has to be asked again (lib/probe-personalisation.mjs). Probes
...
export async function rebuildAllPersonalisation(repo, probesById, opts = {}) {
  ...
  for (const rec of intelligenceRecords) {
    ...
    try {
      ...
      // ai_calls_used, not 1: a probe whose first answer fails the email
      // contract is asked again (see THE CONTRACT GATE in
      // lib/probe-personalisation.mjs), and one whose commercial_consequence
      // is still the only thing missing after that costs one further,
      // consequence-only call, so billing the budget one-per-probe
      // would let a batch quietly make several times the AI calls this
      // invocation was capped at. It is accounting only — no PERSONALISATION
      // column holds it, so it is destructured off before the row is written.
      const { ai_calls_used: aiCallsUsed = 1, ...personalisation } = await personaliseProbe(probe, intelligence, diagnosis, findings, agency);
