...
//
// WHAT THIS STEP HANDS THE AI CALL: the PROBES row's factual context and that
// probe's DIAGNOSIS_FINDINGS rows. That is all. It no longer passes the
// DIAGNOSIS prose, the INTELLIGENCE prose or the raw COMMUNICATIONS into the
// story — every one of those restated, in paragraphs, what the findings
// already state as structured facts, and handing them back invited the model
// to re-diagnose from the transcript rather than select from the findings.
// The DIAGNOSIS and INTELLIGENCE rows are still READ here for the
// deterministic decisions code owns (the eligibility gate, no-response flag
// and hero-journey lookup) — they are simply never put in front of the
// model. COMMUNICATIONS is not loaded at all any more.
//
// AND THE DIAGNOSIS ROW ITSELF NO LONGER LEAVES THIS FILE. Not passing the
// prose to the MODEL was only half of it: the row object was still handed to
// personaliseProbe(), so "Personalisation does not read DIAGNOSIS prose" was a
// convention one edit away from being untrue. Only novus_opportunity — a
...
//
// Personalisation is the layer that judges which findings combine into one
// story; it never re-derives them and never invents one.
//
...
// EXCEPT AT THE WRITE. personaliseProbe() can legitimately return one of them
// blank: its last resort is `{ ...best.row, ...best.softFallbacks }`, and a
// rejection whose reason is in NEVER_PERSIST_REASONS deliberately banks no
// fallback, because a sentence asserting something untrue is worth less than
// nothing. That is correct behaviour — a blank there is a REFUSAL, not a
// result. What was missing is anything downstream treating it as one.
//
...
export async function rebuildAllPersonalisation(repo, probesById, opts = {}) {
  ...
  for (const rec of intelligenceRecords) {
    ...
    try {
      ...
      // ai_calls_used, not 1: an invalid finding selection or incoherent new
      // email variable may use one bounded correction call. It is accounting
      // only — no PERSONALISATION column holds it, so it is destructured off
      ...
      const { ai_calls_used: aiCallsUsed = 1, ...personalisation } = await personaliseProbe(probe, intelligence, diagnosisContext, findings, agency);
