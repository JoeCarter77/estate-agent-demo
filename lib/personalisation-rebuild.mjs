...
// content (non-blank personalised_opener) it is FROZEN for good: nothing
...
// personalised_opener is set. Deliberately does not take forceAi — see the
// file header.
function needsPersonalisation(existingRecord) {
  if (!existingRecord) return true;
  return !String(existingRecord.obj.personalised_opener || '').trim();
}
...
export async function rebuildAllPersonalisation(repo, probesById, opts = {}) {
  ...
  const [intelligenceTable, diagnosisTable, personalisationTable, communicationsTable, agenciesTable] = await Promise.all([
  ...
  for (const rec of intelligenceRecords) {
    ...
    try {
      ...
      const personalisation = await personaliseProbe(probe, intelligence, diagnosis, communications, agency);
