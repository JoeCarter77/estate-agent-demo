...
// written back to flip probe_status to 'closed' on the window elapsing;
// DIAGNOSIS is only ever read (to know which probes are finalised), never
// written here — that's lib/diagnosis-rebuild.mjs's job.
//
...
export async function rebuildAllIntelligence(repo, opts = {}) {
  ...
  const probeStatusPatchesById = new Map();
  ...
  for (const rec of probeRecords) {
    ...
    try {
      ...
      // lifecycle, and probe_status tracks the first one.
      if (det.observation_status === 'closed' && probe.probe_status !== 'closed') {
        probeStatusPatchesById.set(probeId, { probe_status: 'closed' });
      }
  ...
  for (const [probeId, patch] of probeStatusPatchesById) {
