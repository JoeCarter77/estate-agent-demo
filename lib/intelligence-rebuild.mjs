...
export async function rebuildAllIntelligence(repo, opts = {}) {
  ...
  for (const rec of probeRecords) {
    ...
    try {
      ...
      // The only semantic value that remains live is seller recognition. It is
      // deterministic and backed by the actual matched communications.
      aiFields.seller_recognition = classifySellerSignal(probe, classified).seller_recognition;
