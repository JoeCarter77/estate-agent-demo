...
//
// OUTBOUND remains the source of truth. Dry-run is read-only. Live mode is
// deliberately limited to one exact outbound_id and requires an explicit
// confirmation value. This module has no bulk-upload path and never changes
// outbound_status: uploading a lead is not the same as sending an email.
