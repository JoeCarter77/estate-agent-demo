...
export const OUTCOME_LABEL = Object.freeze({
  ...
  REFERRED_TO_EMAIL: 'Referred to email',
});
...
export const CALLING_POLICY = Object.freeze({
  ...
  referredToEmailFollowupWorkingDays: 2,
  defaultHourLondon: 9,
});
...
export function normaliseOutcomeInput(input, nowMs = Date.now()) {
  ...
  switch (outcome) {
    ...
    case 'DO_NOT_CALL':
      ...
      out.pitched_override = input?.pitched_override === true;
      break;
    case 'REFERRED_TO_EMAIL': {
      const ref = input?.referred_contact || {};
      const email = text(ref.email);
      if (!email) errors.push('an email address is required');
      out.referred_contact = {
        name: text(ref.name).slice(0, 120), role: text(ref.role).slice(0, 120),
        phone: text(ref.phone).slice(0, 40), email: email.slice(0, 160), notes: text(ref.notes).slice(0, 500),
      };
      out.followup_at = futureIso(input?.followup_at, 'follow-up date')
        || new Date(addWorkingDaysMs(nowMs, CALLING_POLICY.referredToEmailFollowupWorkingDays)).toISOString();
      // Stored on the CALLS row as callback_at, same convention as
      // MORE_INFO_REQUESTED, so the follow-up survives a re-derive from the row.
      out.callback_at = out.followup_at;
      break;
    }
    default:
...
export function normalisedFromRow(row) {
  ...
  return {
    ...
    followup_at: ['MORE_INFO_REQUESTED', 'REFERRED_TO_EMAIL'].includes(upper(row?.outcome)) ? text(row?.callback_at) : '',
  };
}
...
export function planOutcome(call, normalised, { nowMs = Date.now() } = {}) {
  ...
  switch (outcome) {
    ...
    case 'REFERRED_TO_EMAIL': {
      const ref = normalised.referred_contact || {};
      const dueAt = normalised.followup_at || normalised.callback_at
        || new Date(addWorkingDaysMs(nowMs, CALLING_POLICY.referredToEmailFollowupWorkingDays)).toISOString();
      // Ordinary manual-sales FOLLOW_UP, not a call_action: the lead should not
      // fall back into the phone queue while this is pending (buckets 1-4 only
      // pick up call_action-marked rows), and it is not the never-called
      // bucket 5 either, since a call now exists. See lib/calling-queue.mjs.
      plan.actions.push(base('FOLLOW_UP', dueAt,
        `Follow up ${ref.name || contact} by email — gatekeeper referred us${ref.email ? ` (${ref.email})` : ''}`,
        { referred_contact: ref, title: 'Follow up by email', channel: 'EMAIL' }));
      break;
    }
    default:
      break;
