...
export function buildLeadProfile(tables, agencyId, { now = new Date().toISOString() } = {}) {
  ...
  const open = actions.filter((a) => OPEN.includes(upper(a.action_status))).map((a) => {
    ...
    return { action_id: text(a.action_id), type: text(a.action_type), status: upper(a.action_status), owner: text(a.action_owner), due_at: text(a.due_at),
      ...
      // The stored assignee (ACTIONS metadata, lib/novus-users.mjs); blank = unassigned history.
...
// ── THE CALLING PROFILE (what a SETTER may read) ─────────────────────────────
// The same timeline and profile, narrowed for someone making a call rather
// than running the sale: agency and contacts, every call and outcome, probe
// facts, email activity, open work, meeting history and campaign membership
// all stay; what the discovery/proposal side records about a meeting — free-
// text outcome notes, diagnosis agreement, agreed scope, pitch counts — is
// left out. Protections (opt-out, do-not-call, agency suppression, live
// campaign membership, other people's open work) are surfaced, never hidden.
// Queue membership plays no part: any agency in the database may be read.
// Pure.
// A campaign that is sending or may resume (lib/campaign-store.mjs statuses)
// and in which this agency was actually handed to Instantly.
const LIVE_CAMPAIGN = new Set(['ACTIVE', 'PAUSED']);
export function buildCallingProfile(timeline, profile) {
  if (!profile) return null;
  const meetings = (profile.meetings || []).map((m) => pick(m, ['session_id', 'meeting_at', 'contact_name', 'status', 'outcome', 'follow_up_at', 'completed_at']));
  const entries = (timeline?.entries || []).map((e) => (e.kind === 'meeting' ? { ...e, detail: e.type === 'MEETING' ? e.detail : '' } : e));
  const campaigns = timeline?.campaigns || [];
  return {
    agency_id: profile.identity.agency_id,
    agency_name: profile.identity.agency_name,
    identity: profile.identity,
    contacts: profile.contacts,
    protections: {
      ...profile.suppression,
      live_campaigns: campaigns.filter((c) => LIVE_CAMPAIGN.has(c.status) && c.member_status === 'PUSHED').map((c) => ({ name: c.name, status: c.status })),
    },
