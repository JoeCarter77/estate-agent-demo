...
function styleGuidance() {
  return [
    ...
    '- Create a "how do they know this?" reaction using the exact probe evidence. Never a generic opening like "Hope you\'re well."',
...
function systemPrompt() {
  return [
    ...
    '5. You must call the emit_personalisation tool exactly once with all seven fields as non-empty strings (relevant_proof_point may be the fallback above, but the other six must always be evidence/strategy-backed prose — never leave them empty).',
    '',
    styleGuidance(),
  ].join('\n');
}
...
const TOOL_SCHEMA = {
  ...
  description: 'Return the seven NOVUS personalisation outputs.',
  input_schema: {
    type: 'object',
    properties: Object.fromEntries(
      PERSONALISATION_FIELDS.map((f) => [f, { type: 'string', description: `The ${f} output. Non-empty. Evidence-backed only.` }]),
    ),
...
// network — see scripts/novus-personalisation-selftest.mjs.
async function defaultCallAi(request) {
  ...
  return { raw: toolUse.input, model: data.model || request.model };
}
