...
//                         reported separately as `unclassified`. A row
//                         discarded as a technical issue (call_status=
//                         discarded) is neither — it is excluded entirely.
...
import { liveCallRecords, currentScript, objectionEventRecords, objectionRecords, scriptRecords } from './calling-store.mjs';
...
export function buildCallingAnalytics(tables, { now = new Date().toISOString(), range = 'all', from = '', to = '', script_id: scriptFilter = '' } = {}) {
  ...
  // Every classified call, with its facts, then the date/script window. A
  // discarded (technical-issue) row is not a call and not `unclassified`
  // either — liveCallRecords drops it before anything is counted.
  const allCalls = liveCallRecords(tables.CALLS).map((row) => ({ row, f: callFacts(row) }));
  const allClassified = allCalls.filter((c) => c.f.outcome);
  // An INBOUND ring nobody answered (lib/calling-inbound.mjs) has no outcome
  // by design — it was never an opened dial, so it is not `unclassified`.
  const unclassified = allCalls.filter((c) => !c.f.outcome && inRange(c.f.started_ms, window) && !(upper(metadata(c.row).direction) === 'INBOUND' && !text(c.row.connected_at))).length;
