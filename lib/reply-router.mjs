...
// The intended later match, recorded but NOT implemented: it waits on the real
// campaign-data relationship. Guessing an agency id is forbidden by the NOVUS
// matching rule, so until this is built, agency_id and outreach_id stay blank.
export const MATCH_PLAN = Object.freeze({
  primary: { from: 'lead_email', to: 'OUTBOUND.outreach_contact_email' },
  corroboration: 'campaign_id',
  implemented: false,
});
...
const PROVIDER_HINT_KEYS = [
  ...
  'eaccount', 'email_account', 'from_address_json', 'to_address_json', 'i_status',
];
...
// ---------------------------------------------------------------------------
// Direction — deterministic, address-based, never guessed.
//
// INBOUND  : from == lead_email AND recipients include a NOVUS mailbox.
// OUTBOUND : from is a NOVUS mailbox AND recipients include lead_email.
// UNKNOWN  : anything else, including missing fields or a lead_email that is
//            itself a NOVUS mailbox (which makes both tests true at once).
//
// is_unread is NOT consulted. It describes read state, not who sent what, and
// on the observed inbound reply it was 1 purely because nobody had opened it.
// ---------------------------------------------------------------------------
export function detectDirection({ fromEmail, toEmails, leadEmail, mailboxes }) {
  ...
  const novus = novusMailboxes(mailboxes);
  if (!from || !lead || !recipients.length) return 'UNKNOWN';

  const fromIsLead = from === lead;
  const fromIsNovus = novus.includes(from);
  const toHasNovus = recipients.some((r) => novus.includes(r));
  const toHasLead = recipients.includes(lead);

  const inbound = fromIsLead && toHasNovus;
  const outbound = fromIsNovus && toHasLead;
  // Both true means the lead address IS a NOVUS mailbox — genuinely ambiguous.
...
export function normalizeInstantlyEmail(raw, { mailboxes } = {}) {
  ...
  return {
    ...
    direction: detectDirection({ fromEmail: from_email, toEmails: to_emails, leadEmail: lead_email, mailboxes }),
    // Recorded for inspection against real traffic; not a direction input.
