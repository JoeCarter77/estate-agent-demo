...
// The match, implemented in lib/instantly-reply-poll.mjs. Legacy uploads use
// OUTBOUND identity; Campaigns UI enrolments use CAMPAIGNS.instantly_campaign_id
// plus CAMPAIGN_MEMBERS' lead ID or exact email. Callers that do not match still
// leave agency_id and outreach_id blank rather than guessing.
export const MATCH_PLAN = Object.freeze({
  legacy: { from: 'lead_email', to: 'OUTBOUND.outreach_contact_email' },
  campaign_member: { from: ['campaign_id', 'lead_id|lead_email'], to: 'CAMPAIGNS.instantly_campaign_id + CAMPAIGN_MEMBERS' },
  method: 'OUTBOUND_OR_CAMPAIGN_MEMBER',
  campaign_corroboration_available: true,
  implemented: true,
});
...
export const ROUTING_TABLE = {
  ...
  CALL_REQUESTED: { next_action: 'CALL_PROSPECT', priority: 'HIGH', suppression_type: 'NONE' },
  INFO_REQUESTED: { next_action: 'SEND_INFORMATION', priority: 'HIGH', suppression_type: 'NONE' },
  POSITIVE_MEETING: { next_action: 'HUMAN_REPLY', priority: 'CRITICAL', suppression_type: 'NONE' },
  // A clear expression of interest with no specific ask ("yes please" to an
  // open question, "sounds interesting"). Decided deterministically only.
  POSITIVE_INTEREST: { next_action: 'HUMAN_REPLY', priority: 'HIGH', suppression_type: 'NONE' },
  QUESTION: { next_action: 'HUMAN_REPLY', priority: 'HIGH', suppression_type: 'NONE' },
  OBJECTION: { next_action: 'HUMAN_REPLY', priority: 'HIGH', suppression_type: 'NONE' },
...
// joe@novushq.co.uk.
// Used ONLY when a message carries no `eaccount` of its own (eaccount is
// per-message and always wins). Both live sending identities are listed: a
// quoted parent from the one that is missing here would fail the authenticity
// gate and lose its thread context entirely.
export const DEFAULT_NOVUS_MAILBOXES = ['joe@novushq.co.uk', 'joe@trynovus.co.uk'];
...
export function detectDirection({ ueType, fromEmail, toEmails, leadEmail, eaccount, mailboxes }) {
  ...
  // A colleague/alias can reply to a lead enrolled under a different address.
  // Instantly's received lifecycle value plus delivery to the exact sending
  // account is deterministic inbound evidence; the external sender need not
  // equal the originally enrolled lead address.
  if (parsed === UE_TYPE.RECEIVED) {
    const from = asText(fromEmail).toLowerCase();
    const recipients = (toEmails || []).map((value) => asText(value).toLowerCase());
    const { accounts } = novusAccountsFor({ eaccount, mailboxes });
    const deliveredToNovus = recipients.some((recipient) => accounts.includes(recipient));
    if (from && asText(leadEmail) && deliveredToNovus && !accounts.includes(from)) return 'INBOUND';
  }
  // All other provider claims still require the original full address
  // relationship. A received message not delivered to a known NOVUS account
  // remains UNKNOWN.
...
// ---------------------------------------------------------------------------
// EMAIL BODY NORMALISATION, for INTERPRETATION ONLY.
//
// Real Instantly/Gmail bodies are HTML-ish, not plaintext: a reply of "Yes"
// arrives as "Yes<br><br>On Tue, Sep 1, 2026 at 2:15 PM Joe Carter
// <joe@novushq.co.uk> wrote:<br>> ...". Two things broke because of that:
//   - cleaned_reply_text came out as "Yes<br><br>", so every exact-match
//     affirmative check failed and a plainly positive reply went to review
//   - quoted lines are separated by <br>, not \n, so per-line ">" stripping
//     never fired and the recovered parent kept its quote markers
//
// THIS IS NEVER APPLIED TO raw_body_text. That column is the immutable
// evidence of what arrived and stays byte-identical; only the derived,
// interpreted views (cleaned_reply_text, the recovered parent) are normalised.
//
// Deliberately NOT a general tag stripper: /<[^>]+>/ would eat
// "<joe@novushq.co.uk>" out of a quote header and break the authenticity gate
// that stops a third-party quote impersonating NOVUS. Only the tags that
// actually carry a line break are touched.
const HTML_ENTITIES = [
  [/&nbsp;/gi, ' '],
  [/&lt;/gi, '<'],
  [/&gt;/gi, '>'],
...
export function normaliseEmailBodyText(value) {
  let text = asText(value);
  if (!text) return '';

  // Line-break-carrying tags become real newlines.
  text = text
    .replace(/<br\s*\/?>/gi, '\n')
  ...
  // Unicode separators. U+202F (narrow no-break space) shows up around "PM" in
  // Gmail quote headers; U+2028/9 are line separators, not spaces.
  text = text
    .replace(/[\u2028\u2029]/g, '\n')
    .replace(/[\u00a0\u1680\u2000-\u200a\u202f\u205f\u3000\ufeff]/g, ' ');
...
export function cleanReplyText(rawText) {
  const text = normaliseEmailBodyText(rawText);
  if (!text.trim()) return '';
...
export function normalizeInstantlyEmail(raw, { mailboxes } = {}) {
  ...
  return {
    ...
    lead_id: asText(pickField(raw, 'lead_id', 'leadId') ?? ''),
...
// FIRST PERSON PLURAL IS NOT OPTIONAL. A person replying from a work inbox
// speaks for the agency — "remove US off your email list", "take US off your
// mailing list" — and the singular-only list missed every one of them. A real
// prospect wrote "Please remove us off your email list" and it was classified
// NOT_INTERESTED with suppression_type NONE, i.e. an explicit opt-out that
// applied no suppression at all. Every pattern below therefore accepts me/us
// and my/our.
//
// Still PHRASES, not bare words: "remove", "stop" and "list" on their own are
// far too common in ordinary replies.
const OPT_OUT_PATTERNS = [
  /\bunsubscribe\b/,
  /\bremove (?:me|us)\b/,
  /\bremove (?:my|our) (?:details|name|email|address|data)\b/,
  /\btake (?:me|us) off\b/,
  /\b(?:do not|don't|dont) (?:contact|email|mail) (?:me|us)\b/,
  /\bstop (?:emailing|e-mailing|mailing|contacting) (?:me|us)\b/,
  /\bno more emails\b/,
  /\bopt (?:me|us) out\b/,
];
...
// CURRENT PROSPECT WORDS ONLY. The raw-body fallback is deliberately gone: an
// opt-out must be something THIS person just wrote, never something quoted from
// an older message in the thread. cleaned_reply_text is the prospect's newly
// authored text; the subject is theirs too, so both are in scope and the quoted
// history is not.
//
// Returns the matched text (for the audit reason), or null.
export function detectOptOut(reply) {
  const body = normalizeTextForMatching(reply?.cleaned_reply_text ?? '');
...
export function routeReply(reply) {
  const autoText = `${normalizeTextForMatching(reply?.subject)} ${normalizeTextForMatching(reply?.cleaned_reply_text)}`;
  const automatic = reply?.is_auto_reply === true
    || /\bautomatic reply\b|\bauto(?:matic)?[- ]?reply\b|\bout of (?:the )?office\b|\bcurrently on (?:annual )?leave\b|\bwe have received your (?:email|message)\b|\byour (?:email|message) has been received\b|\bdue to (?:a )?high volume of emails\b|\bdelivery (?:status|failure|notification)\b|\bmail(?:box)? (?:is )?unattended\b/.test(autoText);
  if (automatic) {
    return decision('OOO_AUTOMATED', reply?.is_auto_reply === true
      ? 'is_auto_reply flag set by Instantly'
      : 'deterministic automated-message pattern matched', 1);
  }
  ...
  const authored = normalizeTextForMatching(reply?.cleaned_reply_text);
  if (/^(?:not interested|no thanks|not for us)(?: thanks| thank you)?[.!]*$/.test(authored)) {
    return decision('NOT_INTERESTED', 'prospect explicitly declined the offer', 1);
  }
...
export async function loadProcessedReplyEventIds(repo) {
  ...
  return {
    ...
    records: rows.flatMap((row, index) => {
      ...
      return asText(obj.reply_event_id) && obj.reply_event_id !== 'SCHEMA NOTE'
        ? [{ rowNumber: index + 2, obj }] : [];
    }),
    header,
...
// A reply can arrive before its campaign membership has been synchronised into
// the matcher. These are correlation fields, not message evidence: a repair
// may fill them only on an already-persisted unresolved event. Keeping this
// narrow prevents recovery from ever changing the email's content, sender,
// timestamp, or provider identity.
export const MATCH_IDENTITY_FIELDS = ['agency_id', 'outreach_id'];

export async function updateReplyEventMatch(replyEventId, patch, {
  repo = null,
  dryRun = DEFAULT_DRY_RUN,
} = {}) {
  const id = asText(replyEventId).trim();
  if (!id) throw new Error('updateReplyEventMatch requires a reply_event_id');
  const illegal = Object.keys(patch || {}).filter((key) => !MATCH_IDENTITY_FIELDS.includes(key));
  if (illegal.length) throw new Error(`refusing to update non-match REPLY_EVENTS fields: ${illegal.join(', ')}`);
  if (dryRun) return { dryRun: true, updated: false, skipped: 'dry_run', reply_event_id: id, patch };
  if (!repo) throw new Error('updateReplyEventMatch requires a repo when dryRun is false');

  const { header, rows } = await repo.getTable(REPLY_EVENTS_TAB);
  const idIdx = header.indexOf('reply_event_id');
  const rowIndex = rows.findIndex((row) => asText(row?.[idIdx]).trim() === id);
  if (idIdx < 0 || rowIndex < 0) return { dryRun: false, updated: false, skipped: 'reply_event_not_found', reply_event_id: id, patch };
  const writes = Object.entries(patch).map(([column, value]) => {
    const columnNumber = header.indexOf(column) + 1;
    if (!columnNumber) throw new Error(`REPLY_EVENTS is missing match column: ${column}`);
    return { tab: REPLY_EVENTS_TAB, rowNumber: rowIndex + 2, columnNumber, value };
  });
  await repo.writeCellsBatch(writes);
  return { dryRun: false, updated: true, reply_event_id: id, patch, cells_written: writes.length };
}
