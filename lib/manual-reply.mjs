...
// ---------------------------------------------------------------------------
// IDEMPOTENCY KEY — defined here, acquired by Phase 3B execution.
//
// The pure Phase 3A gate still takes no claim. The Phase 3B live executor uses
// this exact builder immediately before the shared transport send.
//
...
// ---------------------------------------------------------------------------
// CAMPAIGN-LAYER JOURNEYS. Replies to a NOVUS campaign (lib/campaign-*.mjs)
// carry no OUTBOUND row — the lead was enrolled as a CAMPAIGN_MEMBERS row. Its
// journey identity is that member: exactly one non-excluded member of the
// campaign the reply arrived on, with the same lead email. Pure; ids only.
export function resolveCampaignMemberForReply({ campaignRecords = [], memberRecords = [], replyEvent = null } = {}) {
  const obj = (r) => (r && r.obj) || r || {};
  const instantlyCampaignId = trimmed(replyEvent?.campaign_id);
  const email = lower(replyEvent?.lead_email);
  if (!instantlyCampaignId || !email) return { status: 'MISSING', member: null };
  const campaignIds = new Set(campaignRecords.map(obj)
    .filter((c) => trimmed(c.instantly_campaign_id) === instantlyCampaignId).map((c) => trimmed(c.campaign_id)));
  if (!campaignIds.size) return { status: 'MISSING', member: null };
  const members = memberRecords.map(obj).filter((m) => campaignIds.has(trimmed(m.campaign_id))
    && lower(m.email) === email && trimmed(m.member_status).toUpperCase() !== 'EXCLUDED');
  if (members.length !== 1) return { status: members.length ? 'AMBIGUOUS' : 'MISSING', member: null };
  return { status: 'MATCHED', member: members[0] };
}
...
export function evaluateManualReplyGate({
  ...
  outboundRecords = [],
  // For a reply with no outreach_id: resolveCampaignMemberForReply()'s result.
  campaignMember = null,
  liveParent = null,
  ...
  // A campaign-layer reply (no outreach_id) resolves its journey through its
  // campaign member instead; the member's id is the journey key it records.
  const memberJourney = !outreachId && campaignMember?.status === 'MATCHED';
  const outbound = memberJourney
    ? { status: 'MATCHED', reason: null, row: campaignMember.member }
    : !outreachId && campaignMember?.status === 'AMBIGUOUS'
      ? { status: 'AMBIGUOUS', reason: 'OUTBOUND_MATCH_AMBIGUOUS', row: null }
      : resolveOutboundForSend(outboundRecords, { outreachId, leadEmail: replyEvent.lead_email });
  if (outbound.reason) add(outbound.reason);
  const journeyId = memberJourney ? trimmed(campaignMember.member.member_id) : outreachId;
  ...
  return {
    ...
    resolved: {
      ...
      // OUTBOUND.outbound_id, or CAMPAIGN_MEMBERS.member_id for a campaign reply.
      outreach_id: journeyId,
      journey: memberJourney ? 'CAMPAIGN_MEMBER' : 'OUTBOUND',
