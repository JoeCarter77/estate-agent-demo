...
import { evaluateManualReplyGate, resolveCampaignMemberForReply } from './manual-reply.mjs';
...
export async function gatherManualReplyInputs({
  ...
  if (!record) {
    return {
      record: null, replyEvent: null, outreachReplyEvents: [], outboundRecords: [], campaignMember: null,
  ...
  // A campaign-layer reply has no outreach_id: its journey is its campaign
  // member, and its history is every stored reply for the same agency (the
  // conservative scope for opt-out and staleness).
  ...
  if (!outreachId) {
    ...
    campaignMember = resolveCampaignMemberForReply({ campaignRecords, memberRecords, replyEvent });
  }
  const agencyId = text(replyEvent.agency_id);
  const outreachReplyEvents = outreachId
    ? allReplies.map((item) => item.obj).filter((obj) => text(obj.outreach_id) === outreachId)
    : campaignMember?.status === 'MATCHED' && agencyId
      ? allReplies.map((item) => item.obj).filter((obj) => text(obj.agency_id) === agencyId)
      : null;
  ...
  const leadEmail = text(outboundRow?.outreach_contact_email) || text(campaignMember?.member?.email) || text(replyEvent.lead_email);
...
export async function evaluateManualReplyRequest({
  ...
  const gate = evaluateManualReplyGate({
    ...
    campaignMember: inputs.campaignMember,
