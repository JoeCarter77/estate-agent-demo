...
export async function recomputeProbeObservation(repo, probeId) {
  ...
  // The probe itself closes the moment its 4-day window elapses — flip the
  // existing PROBES.probe_status lifecycle field, same as the batch path
  // (lib/intelligence-rebuild.mjs) does.
  if (det.observation_status === 'closed' && probe.probe_status !== 'closed') {
    await repo.updateById('PROBES', 'probe_id', probeId, { probe_status: 'closed' });
  }
