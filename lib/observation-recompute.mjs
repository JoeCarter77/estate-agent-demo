...
//      DIAGNOSIS fields. Upserts exactly one DIAGNOSIS row, and — through the
//      same writer lib/diagnosis-rebuild.mjs uses — exactly one
//      DIAGNOSIS_FINDINGS row per finding. A diagnosis written by this path
//      without its findings would be frozen findings-less for good, since
//      diagnosis_summary is the freeze signal and the rebuild skips it after.
//
...
export async function recomputeProbeObservation(repo, probeId) {
  ...
  if (det.observation_status === 'closed') {
    ...
    // 6b) DIAGNOSIS_FINDINGS — the same findings[] items, persisted one row
    // each, exactly as lib/diagnosis-rebuild.mjs does it (same writer, so the
    // one-row-per-(probe_id, finding_index) invariant holds identically here).
    //
    // This used to be missing, and that was a real hole rather than a tidy-up:
    // a probe diagnosed through THIS path came out with a non-blank
    // diagnosis_summary — which is the "finalised, frozen" signal — but with
    // no findings rows at all. The rebuild then skipped it for good, and
    // Personalisation saw an empty findings list for a probe that genuinely
    // had findings, and quietly told the model "(none — the evidence shows no
    // genuine problem)". Writing them here is what stops that silent fallback
    // at source.
