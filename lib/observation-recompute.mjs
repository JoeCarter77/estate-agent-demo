...
export async function recomputeProbeObservation(repo, probeId) {
  ...
  // 4b) TIER / SALES STRATEGY — lightweight, grade-decisive routing (lib/tier.mjs).
  // Agency context is read purely to enrich tier_reason/sales_angle; it never
  // gates or overrides the grade-based classification, and a missing AGENCIES
  // record (agency not found, or no agency_id on the probe) is neutral.
  const agencyRecord = probe.agency_id
    ? await repo.findById('AGENCIES', 'agency_id', probe.agency_id)
    : null;
  const { tier, tier_reason, sales_angle, segment } = classifyTier({
    grade,
    compromised: probe.compromised,
    agency: agencyRecord ? agencyRecord.obj : null,
