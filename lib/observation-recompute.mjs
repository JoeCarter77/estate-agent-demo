...
import { classifyTier } from './tier.mjs';
import { buildSalesStrategy } from './sales-strategy.mjs';
...
export async function recomputeProbeObservation(repo, probeId) {
  ...
  // Look up the existing INTELLIGENCE row (if any) up front — needed both to
  // pass its current `segment` through unchanged (lib/tier.mjs never derives
  // segment from grade; there is no authoritative upstream source for it yet,
  // so this simply preserves whatever value is already there) and for the
  // idempotent upsert below.
  ...
  // 4b) TIER — lightweight, grade-decisive routing (lib/tier.mjs).
  // Agency context is read purely to enrich tier_reason; it never gates or
  // overrides the grade-based classification, and a missing AGENCIES record
  // (agency not found, or no agency_id on the probe) is neutral.
  const agencyRecord = probe.agency_id
    ? await repo.findById('AGENCIES', 'agency_id', probe.agency_id)
    : null;
  const agency = agencyRecord ? agencyRecord.obj : null;
  const { tier, tier_reason, segment } = classifyTier({
    grade,
    compromised: probe.compromised,
    agency,
    segment: existing ? existing.obj.segment : '',
  });

  // 4c) SALES STRATEGY — deterministic, derived from grade + tier + the
  // observed evidence (lib/sales-strategy.mjs). Recomputed on every pass and
  // never cached or frozen: grade and tier legitimately change while the
  // 4-day window is open (a genuine late follow-up moves C -> A), so the
  // strategy must move with them. Emits STRATEGY, not copy — the
  // Personalisation Engine (§21) turns this into wording later.
  const strategy = buildSalesStrategy({
    grade,
    tier,
    observation,
    compromised: probe.compromised,
  ...
  const intelligencePatch = {
    ...
    tier_reason,
    segment,
    // Sales Strategy Engine (§20). `sales_focus` is the Master's "primary
    // sales angle" and persists into the canonical existing sales_angle
    // column — one concept, one column, one producer (see lib/tier.mjs).
    sales_angle: strategy.sales_focus,
  ...
  // (existing was already looked up above, before classifyTier.)
  let intelligenceId;
  if (existing) {
