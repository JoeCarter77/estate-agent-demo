...
export async function recomputeProbeObservation(repo, probeId) {
  ...
  // in place on every recompute rather than duplicated.
  ...
  if (det.observation_status === 'closed') {
    ...
    const existingDiagnoses = await repo.getRecords('DIAGNOSIS', 'diagnosis_id');
    ...
    if (existingDiag) {
      ...
      await repo.updateById('DIAGNOSIS', 'diagnosis_id', diagnosisId, diagnosisPatch);
    } else {
