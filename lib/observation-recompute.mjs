...
import { classifySellerSignal } from './seller-signal.mjs';
...
export async function recomputeProbeObservation(repo, probeId) {
  ...
  carriedSemantic.seller_recognition = classifySellerSignal(probe, classified).seller_recognition;
  ...
  let aiCallsUsed = 0;
  // New diagnosis/finding generation is retired; existing historical rows are
  // preserved and this path only maintains operational communication metrics.
  // Re-enabling this legacy path requires explicit legacy settings, so normal
  // webhook traffic can never spend.
  if (process.env.NOVUS_ENABLE_LEGACY_DIAGNOSIS === 'true'
    && process.env.NOVUS_ALLOW_RETIRED_AI === 'true'
    && det.observation_status === 'closed') {
