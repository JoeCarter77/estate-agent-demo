...
export async function recomputeProbeObservation(repo, probeId) {
  ...
  // 4b) LIFECYCLE — draft → observing → closed. mark-sent owns the draft→observing
  // transition (and the authoritative server timestamp); the only remaining
  // transition is the deadline passing, which is what this recompute has just
  // established. Closing is idempotent and never re-opens or re-dates a probe:
  // observation_closed_at is the deadline itself, not "whenever recompute
  // happened to run". Probe matching is unaffected — a closed probe is already
  // outside its window, so it was no longer matchable either way.
  let probeClosed = false;
  if (windowClosed && probe.probe_status === 'observing') {
    const closedAt = probe.observation_deadline || now.toISOString();
    await repo.updateById('PROBES', 'probe_id', probeId, {
      probe_status: 'closed',
      observation_closed_at: closedAt,
      updated_at: now.toISOString(),
    });
    probe.probe_status = 'closed';
    probe.observation_closed_at = closedAt;
    probeClosed = true;
  }
