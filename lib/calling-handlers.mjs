...
import {
  ACTIONS_TAB, assertActionsHeader, buildActionAppendPlan, buildActionPatchPlan, parseActionRecords, readActions,
...
import {
  ...
  activeObjections, callRecords, ensureCallingTabs, newCallId, newFamilyKey, newObjectionEventId, newObjectionId,
  newScriptId, objectionEventRecords, objectionRecords, patchCallCells, readCallingTables, rowFor, scriptRecords,
  currentScriptConflicts, validateCallRow, validateObjectionRow, validateScriptRow,
...
export function invalidateCallingCache() { workspaceCache = null; }

const CONTEXT_TABS = ['AGENCIES', 'REPLY_EVENTS', 'ACTIONS', 'CONTACTS', 'INTELLIGENCE', 'PROBES', 'DEMOS'];
...
export async function handleCallingWorkspace(req, res) {
  ...
  try {
    ...
    const calls = callRecords(tables.CALLS);
...
export async function handleScriptSave(req, res) {
  ...
  try {
    ...
    const callCount = existing ? callRecords(callsTable).filter((row) => text(row.script_id) === scriptId).length : 0;
...
export async function handleCallingSave(req, res) {
  ...
  try {
    ...
    if (existing && text(existing.agency_id) !== agencyId) return res.status(409).json({ success: false, error: 'call_id belongs to a different agency' });
    ...
    const attemptNumber = existing ? (Number(existing.attempt_number) || 1) : calls.filter((c) => text(c.agency_id) === agencyId && text(c.outcome)).length + 1;
    const row = existing
      ? { ...existing, ...Object.fromEntries(['contact_name', 'contact_role', 'phone', 'script_id', 'source_action_id', 'twilio_call_sid'].filter((k) => text(req.body?.[k])).map((k) => [k, text(req.body[k]).slice(0, 160)])) }
    ...
    row.metadata_json = JSON.stringify({
      followups: 'PENDING', owner_reached_input: normalised.owner_reached, pitched_override: normalised.pitched_override,
    });
    row.updated_at = now;
    const validation = validateCallRow(row);
    if (!validation.valid) return res.status(400).json({ success: false, error: validation.errors.join('; ') });
...
export async function handleCallingRepair(req, res) {
  ...
  try {
    const repo = getRepo();
    const pending = callRecords(await repo.getTable(CALLS_TAB)).filter(followupsPending).slice(0, 25);
    const results = [];
