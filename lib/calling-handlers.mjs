...
export async function handleCallingWorkspace(req, res) {
  ...
  if (!refresh && workspaceCache && nowMs - workspaceCache.at < WORKSPACE_CACHE_TTL_MS) {
    return res.status(200).json({ ...scoped(workspaceCache.payload, workspaceCache.calls), cached: true, cache_age_ms: nowMs - workspaceCache.at });
  }
  try {
    const repo = getRepo();
    ...
    return res.status(200).json({ ...scoped(payload, calls), cached: false, cache_age_ms: 0 });
  } catch (err) {
