...
export function replyCallbackTiming(body, receivedAt, now = new Date().toISOString()) {
  ...
  // A month with no day ("try me in January", "early March", "after
  // Christmas", "in the new year"): the first working day of the next such
  // month, 9am London. A preposition is required so the modal "may" in "you
  // may…" never reads as May. The current month is left for review.
  const monthOnly = !relative && lower.match(/\b(?:in|early|mid|late|end of|beginning of|start of|after|until|from|around|towards|by)\s+(?:the\s+)?(?:(new year)|(christmas|xmas)|(jan(?:uary)?|feb(?:ruary)?|march|april|may|june|july|aug(?:ust)?|sept?(?:ember)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?))\b/);
  if (monthOnly) {
    const months = ['jan','feb','mar','apr','may','jun','jul','aug','sep','oct','nov','dec'];
    const month = monthOnly[1] || monthOnly[2] ? 1 : months.indexOf(monthOnly[3].slice(0, 3)) + 1;
    const received = londonParts(receivedMs);
    if (month === received.month) return { language: value, due_at: '', precision: 'BROAD', needs_review: true, warning: 'Requested month is the current month' };
    const year = month > received.month ? received.year : received.year + 1;
    // 1 January is a bank holiday, never a working day.
    let firstDay = Date.UTC(year, month - 1, month === 1 ? 2 : 1, 12);
    while ([0, 6].includes(londonWeekday(firstDay))) firstDay += 86_400_000;
    const dueMs = londonTimeOn(firstDay, { hour: 9 });
    return { language: value, due_at: new Date(dueMs).toISOString(), precision: 'BROAD', needs_review: true };
  }
