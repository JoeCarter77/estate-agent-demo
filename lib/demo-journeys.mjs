...
const JOURNEYS = {
  // ── AUTHORED ──────────────────────────────────────────────────────────────
  weak_seller_qualification: {
    hook: (ctx) => `${teamDidClause(ctx)} ${sellerClause(ctx)}`,

    reveal: () => "The buyer was worked. The potential vendor wasn't.",

    // UNDERSTANDS — two lines, no more. The prospect has to grasp this at a
    // glance, not read an analysis of their own enquiry.
    detected: (ctx) => take([
      ctx.sellerDeclared && {
        label: 'Buyer + declared vendor',
        detail: 'Two opportunities inside one enquiry',
      },
      {
    ...
    // DECIDES — the decision itself, not an operating manual for it.
    decisions: () => ([
    ...
    // ACTS — NOVUS acts, or the right opportunity reaches a person.
    actions: () => ([
      { owner: 'novus', label: 'Qualifies · follows up · books', detail: '' },
      { owner: 'team', label: 'Or routes the right opportunity to your team', detail: '' },
    ]),
  },
