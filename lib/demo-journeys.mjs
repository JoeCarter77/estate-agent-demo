...
export function readingSignals(ctx = {}) {
  ...
  if (ctx?.sellerDeclared) {
    ...
    push({ value: ENQUIRY_CONSTANTS.locality, label: ENQUIRY_CONSTANTS.addressLabel });
  }
...
function unresolvedCandidates(ctx) {
  ...
  if (noHumanConversation(ctx)) {
    items.push({
      text: 'Anything about this buyer beyond the enquiry itself',
      note: 'No reply was recorded, so nothing was established either way.',
  ...
  if (ctx?.sellerDeclared && !SELLER_PROGRESSED.has(recognition)) {
    items.push({
      text: `Whether the ${ENQUIRY_CONSTANTS.locality} address is the property being sold`,
      note: SELLER_RECOGNISED.has(recognition)
        ? 'Your team raised the sale. The answer never came back.'
        : 'An address was supplied. Nothing in the conversation confirms what it is.',
      ask: `Is the ${ENQUIRY_CONSTANTS.locality} address the property you're looking to sell?`,
      key: 'sale_address',
    });
  }
  ...
  if (ctx?.sellerDeclared) {
    items.push({
      text: 'Whether the purchase depends on selling first',
      note: 'It changes how quickly this buyer could actually move.',
  ...
  if (!noHumanConversation(ctx) && !VIEWING_PROGRESSED.has(progression)) {
    items.push({
      text: 'Whether they want to view, or were gathering information',
      note: progression === 'mentioned'
        ? 'A viewing came up and never turned into a time.'
        : 'The enquiry asked for details. A viewing was never taken up.',
  ...
  items.push({
    text: 'What timescale they are working towards',
    note: 'Nothing in the conversation puts a date on the move.',
...
const ACTION_FOR = {
  ...
  sale_address: {
    ...
    detail: `Establish whether the ${ENQUIRY_CONSTANTS.locality} address is the property being sold.`,
  },
  chain: {
