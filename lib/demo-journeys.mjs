...
// hero title is derived here, by commercialPriority(), from the full findings:
//
...
// The hero title. Derived from the priority above AND from this probe's own
// ordinals, so no two agencies with different findings can be handed the same
// headline: an agency that progressed the viewing is credited with it, an
// agency that asked about the seller position is never told it ignored one,
// and an agency that did neither is told exactly that.
export function heroTitle(heroJourney, ctx) {
  const priority = commercialPriority(heroJourney, ctx);
  const journey = text(heroJourney);
  const buyerWorked = viewingWasProgressed(ctx);

  if (priority === 'complete_miss') {
    return ctx?.sellerDeclared
      ? 'One enquiry. Two opportunities. Neither progressed.'
      : 'One enquiry. No conversation.';
  }

  if (priority === 'seller_unprogressed') {
    if (journey === 'fast_response_stalled_follow_up') {
      return 'A fast response. Two opportunities left unfinished.';
    }
    if (journey === 'slow_response_gap') {
      return buyerWorked
        ? "The buyer enquiry was handled. The seller opportunity wasn't."
        : 'A buyer and a potential seller. Neither opportunity progressed.';
    }
    if (sellerWasRecognised(ctx)) return 'The seller opportunity was spotted. It still went nowhere.';
    return buyerWorked
      ? "The buyer was worked. The potential vendor wasn't."
      : 'The buyer got a reply. The potential vendor got nothing.';
  }

  if (priority === 'stalled_progression') {
    if (journey === 'fast_response_stalled_follow_up') return 'A fast response. An unfinished opportunity.';
    return buyerWorked
      ? 'The enquiry was answered. The opportunity was never finished.'
      : 'The enquiry was answered. It never got a next step.';
  }

  return delayWasSevere(ctx)
...
// single idea - NOVUS understands the context, decides what should happen,
// then acts - and the fastest way to lose that idea is to turn it into three
// lists of things a chatbot does. The only thing that varies is whether this
// enquiry actually carried a seller; nothing else here is per-journey, because
// the capability is not per-journey.

function novusUnderstands(ctx) {
  return take([{
    label: 'Sees the full opportunity',
    detail: ctx.sellerDeclared
      ? 'Buyer enquiry, potential seller, and what still needs to be established.'
      : 'Buyer enquiry, what it carries beyond the viewing, and what still needs to be established.',
  }], 1);
}

function novusDecides(ctx) {
  return take([{
    label: 'Chooses the right next steps',
    detail: ctx.sellerDeclared
      ? 'Progress the viewing. Qualify the seller. Escalate anything that needs the team.'
      : 'Progress the viewing. Establish what is missing. Escalate anything that needs the team.',
  }], 1);
}

function novusActs() {
  return take([{
    label: 'Moves the opportunities forward',
    detail: 'NOVUS takes the appropriate action automatically and hands the team the conversations that need a person.',
  }], 1);
}
