...
const JOURNEYS = {
  ...
  weak_seller_qualification: {
    ...
    // UNDERSTANDS - that the enquiry carries BOTH opportunities, and which of
    // them is still unresolved. Two lines, no more.
    detected: (ctx) => take([
      ctx.sellerDeclared && {
        label: 'Both opportunities in one enquiry',
        detail: 'A buyer to progress, and a declared seller to qualify',
      },
      {
        label: 'Seller position still unknown',
        detail: SELLER_UNKNOWN_DETAIL[text(ctx.intelligence?.seller_recognition)] || '',
      },
    ], 2),

    // DECIDES - what should happen next, for each side of the enquiry.
    decisions: () => ([
      { label: 'Progress both opportunities', detail: 'Neither side waits on the other' },
      { label: 'Qualify the seller position while the enquiry is still live', detail: '' },
    ]),

    // ACTS - progresses each opportunity, or puts it in front of the person
    // it belongs to.
    actions: () => ([
      { owner: 'novus', label: 'Progresses the viewing and qualifies the seller', detail: 'Follows up and books, across phone, email and text' },
      { owner: 'team', label: 'Takes the opportunity that needs a person', detail: 'Routed and escalated by NOVUS, with the context' },
    ]),
  },
