// lib/sales-messages.mjs — schema, append and read helpers for SALES_MESSAGES.
//
...
// sales conversation.
//
// Phase 3B appends SENT and AMBIGUOUS manual replies here. It never stores an
// inbound message and never changes an existing row.
//
...
export const SEND_OUTCOMES = ['SENT', 'AMBIGUOUS', 'FAILED', 'BLOCKED', 'DRY_RUN'];
...
//
// Phase 3A defined and tested this comparison; Phase 3B now records its result
// on every SENT or AMBIGUOUS manual-reply row. The retired Phase 2 placeholder
// values IN_THREAD/NEW_THREAD described a different question.
...
// Row array in header order. The Phase 3B append helper writes this exact row.
...
// Append exactly one validated row against the exact audited 20-column
// header. A reordered, missing or extended live tab fails closed instead of
// mapping a sent message into the wrong columns.
export async function appendSalesMessage(repo, obj, { tab = SALES_MESSAGES_TAB } = {}) {
  if (!repo) throw new Error('appendSalesMessage requires a repo');
  const row = buildSalesMessageRow(obj);
  const table = await repo.getTable(tab);
  const header = table?.header || [];
  if (header.length !== SALES_MESSAGES_HEADER.length
      || header.some((column, i) => column !== SALES_MESSAGES_HEADER[i])) {
    throw new Error(`${tab} header does not match the audited 20-column schema`);
  }
  await repo.appendRowsBatch(tab, [row]);
  return obj;
}
...
export async function readSalesMessagesForOutreach(repo, outreachId, { tab = SALES_MESSAGES_TAB, column = SALES_MESSAGES_OUTREACH_COLUMN } = {}) {
  const wanted = text(outreachId);
  if (!wanted) return { available: false, rows: [], error: `${column} is required` };
  let table;
  try {
  ...
  const rows = parseSalesMessageRecords(table)
    ...
    .filter((obj) => text(obj[column]) === wanted);
  return { available: true, rows, error: null };
}

// Every NOVUS-sent message for one agency: the journey view for a campaign-layer
// lead, whose replies carry no OUTBOUND outreach_id.
export function readSalesMessagesForAgency(repo, agencyId, options = {}) {
  return readSalesMessagesForOutreach(repo, agencyId, { ...options, column: 'agency_id' });
}
