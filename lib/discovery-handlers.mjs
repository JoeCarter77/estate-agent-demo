...
import { QUESTIONS, SECTIONS, STAGE_SECTIONS, DIMENSIONS, SKIP_REASONS, QUESTIONS_VERSION, COVERAGE_RULES, snapshotFor, SECTION_TRANSITIONS,
} from './discovery-questions.mjs';
import { conversationPayload, openingScript, TOPIC_BY_ID } from './discovery-conversation.mjs';
...
import { buildConclusion, buildFindings, cleanConclusion, mergeOverrides, polishConclusion, presentationPayload, CONCLUSION_STEPS, AGREEMENT_STATUSES, ADDITIONAL_VALUATIONS_RANGE, FOCUS_AREAS, UNDERSTANDING_LEVELS, UNDERSTANDING_LABELS, INTEREST_LEVELS, INTEREST_LABELS } from './discovery-conclusion.mjs';
import {
...
function registryPayload() {
  return {
    ...
    understanding_levels: UNDERSTANDING_LEVELS, understanding_labels: UNDERSTANDING_LABELS,
    interest_levels: INTEREST_LEVELS, interest_labels: INTEREST_LABELS,
  };
}
...
// GENUINE local research: other agencies in the same location that we have
// actually probed and graded. Only this licenses the opening's "a couple of
// things I've noticed from looking at other agencies in your area" — with
// none, the line is never offered.
function localResearch(tables, agency) {
  const location = lower(agency?.location);
  if (!location) return null;
  const graded = new Set(records(tables.INTELLIGENCE, 'intelligence_id').filter((row) => text(row.grade)).map((row) => text(row.probe_id)));
  const agencies = new Set(records(tables.PROBES, 'probe_id').filter((row) => graded.has(text(row.probe_id)) && text(row.agency_id) !== text(agency.agency_id)).map((row) => text(row.agency_id)));
  const local = records(tables.AGENCIES, 'agency_id').filter((row) => agencies.has(text(row.agency_id)) && lower(row.location) === location);
  return local.length ? { agencies: local.length, location: text(agency.location) } : null;
}
...
export function buildAgencyContext(tables, agencyId, { now = new Date().toISOString() } = {}) {
  ...
  return {
    ...
    local_research: localResearch(tables, agency),
...
export async function handleDiscoverySession(req, res) {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      // PRIVATE opening & framing — never part of the conclusion or presentation.
      opening: openingScript({ contact_name: session?.contact_name || agencyContext.contact_name, local_research: agencyContext.local_research }),
...
export async function handleDiscoveryStart(req, res) {
  ...
  try {
    ...
    Object.assign(row, {
      ...
      status: 'IN_PROGRESS', stage: 'opening', questions_version: String(QUESTIONS_VERSION), rules_version: String(RULES_VERSION),
...
function cleanAnswer(question, raw) {
  ...
  if (question.multi) {
    ...
    // An optional numerical target alongside the words. Never required: a
    // question with no number is a complete answer.
    if (question.target) { const n = Number(raw.target); if (raw.target !== '' && raw.target !== null && raw.target !== undefined && Number.isFinite(n)) out.target = n; }
    if (!out.value && out.target === undefined && !text(raw.note)) return null;
  } else {
    const allowed = new Set(question.options.map((o) => o.value));
  ...
  // Recorded while the owner was talking about ANOTHER topic (conversation
  // mode) — kept so the later topic can pick the thread back up.
  if (TOPIC_BY_ID[text(raw.volunteered_in)]) out.volunteered_in = text(raw.volunteered_in);
...
export async function handleDiscoveryConclusion(req, res) {
  ...
  try {
    ...
    if (req.body?.checkpoint !== undefined) next.checkpoint = req.body.checkpoint && typeof req.body.checkpoint === 'object' ? { ...(next.checkpoint || {}), ...req.body.checkpoint, at: new Date().toISOString() } : null;
    ...
    const now = new Date().toISOString();
    const stored = { agreement: cleaned.agreement, additional_valuations: cleaned.additional_valuations, scope_rule_ids: cleaned.scope_rule_ids, polish: cleaned.polish, checkpoint: cleaned.checkpoint, updated_at: now };
  ...
  try {
    ...
    if (outcome === 'PILOT_AGREED') {
      ...
      patch.agreed_scope_json = JSON.stringify({
        rule_ids: ruleIds, agreed_at: now, pitch_id: session.latest_pitch_id, plan: live.conclusion.deployment.plan, offer: FOUNDING_OFFER,
        ...
        conclusion: {
          ...
          // The ONE project the owner agreed to, exactly as presented.
          project: c.project ? {
            type: c.project.type, commercial_objective: c.project.objective, desired_outcome: c.project.desired_outcome,
            bottleneck: c.project.bottleneck.text, title: c.project.title, description: c.project.description_polished || c.project.description,
            objective_statement: c.project.objective_statement,
            components: c.project.components.map((x) => ({ id: x.id, label: x.label, heading: x.heading, sentence: x.sentence_polished || x.sentence, rule_ids: x.rule_ids })),
            supporting_setup: c.project.foundations, preserved: c.project.preserved, ongoing: c.project.ongoing,
            technical_conditions: c.project.conditions, future_scope: c.project.future_scope,
          } : null,
          deployment_scope: ruleIds,
          agency_responsibilities: c.guidance.project ? c.guidance.project.need_from_team : c.guidance.needs.act,
          success_criteria: c.pilot.success_criteria,
          baseline: Object.fromEntries(['enquiries_per_month', 'database_size', 'valuations_per_month', 'instructions_per_month', 'fee_per_instruction', 'conversion_pct'].map((k) => [k, c.understanding.situation[k].known ? c.understanding.situation[k].value : null])),
          deployment: c.deployment.phases.map((ph) => ({ key: ph.key, heading: ph.heading, summary: ph.summary, component_ids: ph.component_ids })),
          checkpoint: { understanding: c.checkpoint.understanding, interest: c.checkpoint.interest, concerns: c.checkpoint.concerns.map((x) => x.id), concerns_resolved: c.checkpoint.concerns_resolved, notes: c.checkpoint.notes, at: c.checkpoint.at },
        ...
        checklist: [
          ...live.conclusion.deployment.plan.required_access.map((item) => ({ phase: 'p1', owner: 'AGENCY', item: `Provide: ${item}`, done: false })),
          ...
          ...live.conclusion.deployment.plan.measurement.map((item) => ({ phase: 'p5', owner: 'NOVUS', item: `Measure: ${item}`, done: false })),
        ],
      });
    }
    await patchSessionCells(repo, sessionId, patch);
