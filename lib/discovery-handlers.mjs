    ...
    if (outcome === 'PILOT_AGREED') {
      ...
      patch.agreed_scope_json = JSON.stringify({
        ...
        conclusion: {
          ...
          focus_areas: c.solutions.map((f) => ({ id: f.id, name: f.name, heading: f.heading, sentence: f.sentence_polished || f.sentence, context: f.context, rule_ids: f.rule_ids })),
          deployment: c.deployment.phases.map((ph) => ({ key: ph.key, heading: ph.heading, summary: ph.summary, focus_ids: ph.focus_ids })),
