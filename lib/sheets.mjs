...
export function createRepo(valuesApi) {
  ...
  return {
    ...
    // Writes one already-merged record to a known row/header, with no read of
    // its own. Factored out of updateById so a caller that already holds a
    // fresh header + rowNumber (e.g. from one batch getRecords()) can write
    // without updateById's usual read-before-write round trip. Every existing
    // caller keeps going through updateById, which still reads fresh every
    // time — this only gives an *additional* option, it changes no default.
    async writeRecordAtRow(tab, header, rowNumber, mergedObj) {
      const row = header.map((key) => (mergedObj[key] ?? ''));
      const lastCol = colLetter(header.length);
      await valuesApi.update(`${tab}!A${rowNumber}:${lastCol}${rowNumber}`, [row]);
      return mergedObj;
    },
    ...
    async updateById(tab, idColumn, idValue, patch) {
      ...
      return this.writeRecordAtRow(tab, header, record.rowNumber, merged);
    },
