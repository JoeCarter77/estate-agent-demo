...
const QUOTA_RETRY_DELAYS_MS = [500, 1500];
...
// A 429 is normally transient but retrying it in a tight loop makes the quota
// problem worse. Retry at most twice with a short bounded backoff; all other
// failures still surface immediately with the same diagnostic shape.
export async function fetchSheetsJson(url, init = {}, {
  ...
  for (let attempt = 0; ; attempt += 1) {
    ...
    if (res.status === 429 && attempt < QUOTA_RETRY_DELAYS_MS.length) {
      const retryAfterSeconds = Number(res.headers?.get?.('retry-after'));
      const retryAfterMs = Number.isFinite(retryAfterSeconds) && retryAfterSeconds > 0
        ? retryAfterSeconds * 1000
        : 0;
      const delayMs = Math.min(3000, Math.max(QUOTA_RETRY_DELAYS_MS[attempt], retryAfterMs));
      console.warn('Sheets API quota exhausted; retrying with backoff', {
        attempt: attempt + 1,
        delay_ms: delayMs,
      });
    ...
    const err = new Error(`Sheets API ${init.method || 'GET'} ${url} failed (${res.status}): ${body.slice(0, 300)}`);
    // Preserve a real 429 through to the HTTP caller (instead of flattening it
    // to a generic 500) so a client's own 429 retry/backoff can actually fire.
...
function realValuesApi(spreadsheetId) {
  async function authedFetch(url, init = {}) {
    return fetchSheetsJson(url, init);
  }
  return {
    async get(range) {
    ...
    // Tab existence, for the one-time calling-workspace setup. Reads sheet
    // metadata only; creates a tab only when asked to by name.
    async listTabs() {
      const metaUrl = `${SHEETS_BASE}/${spreadsheetId}?fields=sheets.properties`;
      const meta = await authedFetch(metaUrl);
      return (meta.sheets || []).map((item) => item.properties?.title).filter(Boolean);
    },
    // A new sheet defaults to 26 columns; a header wider than that would be
    // rejected as outside the grid, so the width is sized to the header.
    async addTab(tab, columnCount = 26) {
      const url = `${SHEETS_BASE}/${spreadsheetId}:batchUpdate`;
      const properties = { title: tab, gridProperties: { rowCount: 1000, columnCount: Math.max(26, Number(columnCount) || 26) } };
      return authedFetch(url, { method: 'POST', body: JSON.stringify({ requests: [{ addSheet: { properties } }] }) });
    },
    async deleteRows(tab, rowNumbers) {
      const metaUrl = `${SHEETS_BASE}/${spreadsheetId}?fields=sheets.properties`;
      const meta = await authedFetch(metaUrl);
      const sheet = (meta.sheets || []).find((item) => item.properties?.title === tab);
      if (!sheet) throw new Error(`Sheet tab ${tab} does not exist`);
      // Delete from the bottom up so earlier row numbers remain stable.
      const requests = [...new Set(rowNumbers || [])].sort((a, b) => b - a).map((rowNumber) => ({
        deleteDimension: {
    ...
    // Widens a tab's grid so a header cell beyond the current last column is
    // writable (values writes outside the grid are rejected). Never narrows.
    async ensureColumnCount(tab, columnCount) {
      const metaUrl = `${SHEETS_BASE}/${spreadsheetId}?fields=sheets.properties`;
      const meta = await authedFetch(metaUrl);
      const sheet = (meta.sheets || []).find((item) => item.properties?.title === tab);
      if (!sheet) throw new Error(`Sheet tab ${tab} does not exist`);
      ...
      const url = `${SHEETS_BASE}/${spreadsheetId}:batchUpdate`;
      const requests = [{ appendDimension: { sheetId: sheet.properties.sheetId, dimension: 'COLUMNS', length: columnCount - current } }];
      return authedFetch(url, { method: 'POST', body: JSON.stringify({ requests }) });
    },
  };
}
...
export function createRepo(valuesApi) {
  ...
  return {
    ...
    // Append already header-ordered rows in bounded batches. Callers use this
    // only after loading and validating the destination header themselves, so
    // it performs no extra read and never maps against stale column positions.
    async appendRowsBatch(tab, rows, chunkSize = 200) {
      for (let i = 0; i < (rows || []).length; i += chunkSize) {
        const chunk = rows.slice(i, i + chunkSize);
        if (chunk.length) await valuesApi.append(`${tab}!A1`, chunk);
      }
    },
    ...
    // Writes individual cells whose row/column positions were resolved from an
    // already-loaded table. Unlike updateCell(), this performs no lookup/read
    // and sends the whole patch in one values.batchUpdate request.
    async writeCellsBatch(writes, chunkSize = 200) {
      for (let i = 0; i < (writes || []).length; i += chunkSize) {
        const chunk = writes.slice(i, i + chunkSize);
        const data = chunk.map(({ tab, rowNumber, columnNumber, value }) => {
          const col = colLetter(columnNumber);
          return { range: `${tab}!${col}${rowNumber}:${col}${rowNumber}`, values: [[value]] };
        });
        if (data.length) await valuesApi.batchUpdate(data);
      }
    },

    // Creates a missing tab with its header row (row 1) and schema note
    // (row 2), in the same layout every other tab in the workbook uses. An
    // existing tab is left exactly as it is — this never rewrites a header.
    // Returns 'created' | 'exists'.
    async ensureTab(tab, headerRow, noteRow) {
      if (typeof valuesApi.listTabs !== 'function' || typeof valuesApi.addTab !== 'function') {
        throw new Error('Sheets transport does not support tab creation');
      }
      const tabs = await valuesApi.listTabs();
      if (tabs.includes(tab)) return 'exists';
      await valuesApi.addTab(tab, (headerRow || []).length + 4);
      const rows = noteRow ? [headerRow, noteRow] : [headerRow];
      await valuesApi.update(`${tab}!A1`, rows);
      return 'created';
    },

    // Appends any of `columns` missing from the tab's header, at the end, in
    // order. Existing columns are never moved or renamed. Returns the header.
    async ensureColumns(tab, columns) {
      const { header } = await getTable(tab);
      const missing = (columns || []).filter((column) => !header.includes(column));
      if (!missing.length) return header;
      if (typeof valuesApi.ensureColumnCount === 'function') await valuesApi.ensureColumnCount(tab, header.length + missing.length);
      await this.writeCellsBatch(missing.map((column, i) => ({ tab, rowNumber: 1, columnNumber: header.length + i + 1, value: column })));
      return [...header, ...missing];
    },

    // Physical row deletion, used only by the Prober's Delete action for a
    // confirmed exact duplicate after every dependency tab has been checked.
    async deleteRows(tab, rowNumbers) {
      if (typeof valuesApi.deleteRows !== 'function') throw new Error('Sheets transport does not support row deletion');
      return valuesApi.deleteRows(tab, rowNumbers);
    },
  };
}
...
export const _internal = { colLetter, getAuthClient, getAccessToken, QUOTA_RETRY_DELAYS_MS };
