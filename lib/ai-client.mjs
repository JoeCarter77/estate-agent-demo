...
// serverTools: Anthropic server-side tools (e.g. the web_search tool) the model
// may use BEFORE producing its structured result. When any are supplied,
// tool_choice relaxes from "force this tool" to "auto" — otherwise the model
// could never reach the search step. The response contract is unchanged: we
// still return only `tool`'s normalised input, and a response that never calls
// `tool` is still an error.
async function realCall({ system, prompt, tool, serverTools = [], model = DEFAULT_MODEL, maxTokens = DEFAULT_MAX_TOKENS, purpose }) {
  ...
  try {
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        tools: [...serverTools, tool],
        tool_choice: serverTools.length ? { type: 'auto' } : { type: 'tool', name: tool.name },
      }),
    });
    ...
    const result = normaliseToolInput(toolUse.input, tool, { truncated: data.stop_reason === 'max_tokens' });
    success = true;
    return result;
  } finally {
    logAnthropicUsage({
      environment, model, purpose, usage,
      serverToolsEnabled: serverTools.length > 0,
      webSearchEnabled: serverTools.some((candidate) => String(candidate?.type || '').startsWith('web_search')),
      success, latencyMs: Date.now() - startedAt,
    });
  }
}
...
// { system, prompt, tool: { name, description, input_schema }, model?, maxTokens?, purpose? }
...
export async function callAi(args) {
  ...
  if (process.env.NOVUS_ALLOW_RETIRED_AI !== 'true') {
    throw new Error('NOVUS AI generation is disabled (NOVUS_ALLOW_RETIRED_AI is required to explicitly re-enable it)');
  }
