...
export function computeObservation(probe, communications) {
  ...
  // Per-channel counts (§9). These are INTELLIGENCE columns that already exist
  // in the workbook but were never populated, so channel mix was invisible in
  // the sheet even though the underlying communications were all captured.
  ...
  return {
    ...
    // Proactive = the agency initiated further contact beyond its first
    // attempt; Reactive = it responded once and stopped. Derived from the same
    // contact-attempt grouping that drives the grade, so the two can never
    // disagree.
    proactive_reactive: humanTouches.length === 0 ? '' : (followUpCount >= 1 ? 'Proactive' : 'Reactive'),
