...
// WHAT COUNTS AS THE AGENCY RESPONDING (behaviour change — see
// lib/comm-assessment.mjs for the evidence behind it).
//
// Every rollup below that feeds the A-H grade — first_human_touch,
// human_lag_hours, contact attempts, follow-ups — now counts GENUINE
// RESPONSES: a human message with real content actually addressed to this
// enquiry. It previously counted any message from a human sender, which in
// the live data meant three identical bulk brochure blasts that never named
// the property graded an agency B for "fast contact with genuine follow-up
// persistence", and a voicemail transcribed as "Hello." set the response-speed
// band for its whole probe.
//
// The A-H rules themselves (lib/grading.mjs) are untouched: same speed bands,
// same persistence test, same G and H. Only the question "did a person from
// the agency actually respond to this enquiry?" is now answered from evidence
// instead of assumed from the sender being human.
function genuineResponses(probe, sorted) {
  const enquiry = parseEnquiry(probe);
  return sorted.filter((c) => assessCommunication(c, enquiry).genuine_response);
}
...
export function computeObservation(probe, communications) {
  ...
  // First human touch — the first actual person genuinely responding, any
  // channel (§9). See genuineResponses() above for what now qualifies.
  const humanTouches = genuineResponses(probe, sorted);
