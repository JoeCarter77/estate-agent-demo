...
// Proactive vs reactive contact (Source Master §11, listed as its own
// intelligence item alongside contact quality). Deterministic and derived
// from the same fact contactQuality() already encodes, rather than a second
// competing measurement: the probe methodology forbids replying during the
// observation window (§7), so the agency's first contact attempt is always a
// reaction to the original enquiry, and any genuine follow-up attempt is
// contact made with no further prompting — i.e. proactive.
// Provisional in the same sense as contactQuality/persistenceProfile: §11
// names the field but gives no numeric/mapping rule.
function proactiveReactive(followUpCount, hasHumanContact) {
  if (!hasHumanContact) return '';
  return followUpCount >= 1 ? 'Proactive' : 'Reactive';
}
...
export function computeObservation(probe, communications) {
  ...
  // Per-channel touch counts (Source Master §11 "Inbound SMS", §18). Plain
  // counts of evidence already held — no new measurement concept. Channel
  // values are written by the webhooks as 'email' | 'sms' | 'voice'.
  const inboundSmsCount = sorted.filter((c) => c.channel === 'sms').length;
  const emailTouchCount = sorted.filter((c) => c.channel === 'email').length;
  ...
  return {
    ...
    proactive_reactive: proactiveReactive(followUpCount, humanTouches.length > 0),
