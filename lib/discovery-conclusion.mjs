...
export const CONCLUSION_STEPS = Object.freeze([
  ...
  { id: 'help', title: 'How NOVUS would help', nav: 'Help', blurb: 'The solution cards, with talking points and implementation answers' },
  { id: 'needs', title: 'What we\'d need from you', nav: 'Needs', blurb: 'Access, a short setup, acting on opportunities' },
  { id: 'deployment', title: 'Your first 60 days', nav: '60 days', blurb: 'Four phases, with the implementation tasks underneath' },
...
// ── the implementation SEQUENCE behind "How NOVUS would help" ─────────────
// Three fixed stages, in build order — not the three highest-ranked
// problems. Each stage covers a fixed set of dimensions; between them every
// rule id is covered exactly once, so nothing selected is left out of the
// slide's private guidance. A stage is included only when this agency's
// AGREED diagnosis has something to say about it (a selected rule, or an
// existing strength to reuse) — never invented for agencies it doesn't fit.
export const IMPLEMENTATION_STAGES = Object.freeze([
  { id: 'foundations', dims: ['F1', 'F2'] },
  { id: 'identify', dims: ['I1', 'I2', 'I3'] },
  { id: 'progress', dims: ['F3', 'F4', 'F5', 'I4', 'I5'] },
]);
const STAGE_HEADINGS = Object.freeze({
  foundations: { build: 'Make customer information usable', reuse: 'Build on what already works', mixed: 'Make customer information usable' },
  identify: { build: 'Identify opportunities in your existing demand', reuse: 'Keep finding opportunities the way you do now', mixed: 'Identify opportunities in your existing demand' },
  progress: { build: 'Progress the opportunities', reuse: 'Progress and measure what already works', mixed: 'Progress the opportunities' },
});
// A short, stage-level description of the commercial payoff — not tied to
// any one agency, used only as the second clause of a talking point.
const STAGE_EFFECTS = Object.freeze({
  foundations: 'so nothing else we do depends on memory or on who happened to take the call',
  identify: 'so more of the sellers already in your enquiries and your database get found',
  progress: 'so sellers get followed up and you can see what turned into a valuation',
});
...
// established.
function stageActionClause(item, rule, situation, crmAlreadyNamed) {
  ...
  if (item.feasibility === 'REQUIRES_ASSESSMENT') {
    ...
    if (cond) clause += `, once we've confirmed ${lower(clientSafe(cond)).replace(/\.$/, '')}`;
  }
  return clause;
}

export function buildSolutionSequence(agreed, situation) {
  // Nothing proposed anywhere (not currently suitable, or a strong agency
  // with no gap) → no solution stages; the caller falls back to next_step.
  if (!agreed.proposed.length) return [];
  const stages = [];
  for (const def of IMPLEMENTATION_STAGES) {
    const selected = agreed.interventions.filter((i) => i.selected && def.dims.includes(i.rule_id));
    const preserved = def.dims.filter((d) => agreed.assessments[d]?.evidence_status === 'EXISTING_STRENGTH')
      .map((d) => ({ dimension: d, label: agreed.assessments[d].label }));
    if (!selected.length && !preserved.length) continue; // nothing established here for this agency
    const variant = selected.length && preserved.length ? 'mixed' : selected.length ? 'build' : 'reuse';
    let crmNamed = false;
    const clauses = selected.slice(0, 2).map((i) => {
      const clause = stageActionClause(i, RULE_BY_ID[i.rule_id], situation, crmNamed);
      if (situation.crm.known && ['F1', 'F2', 'I2', 'I3'].includes(i.rule_id)) crmNamed = true;
      return clause;
    });
    let sentence = clauses.length ? `We'd ${join(clauses)}.` : '';
    if (preserved.length) {
      const labels = preserved.map((p) => lower(p.label));
      const preserveClause = `Your ${join(labels)} already work${preserved.length === 1 ? 's' : ''} well, so we'd reuse ${preserved.length === 1 ? 'it' : 'them'} rather than rebuild${sentence ? ', and put the effort where it is actually needed.' : '.'}`;
      sentence = sentence ? `${sentence} ${preserveClause}` : preserveClause;
    }
    stages.push({ id: def.id, dims: def.dims, heading: STAGE_HEADINGS[def.id][variant], sentence, variant, rule_ids: selected.map((i) => i.rule_id), preserved });
  }
...
// Which finding dimensions this stage's talking points can draw on: any
// dimension that is part of a SELECTED rule in the stage, cross-referenced
// against the owner's own established findings (so the "why it matters" is
// always their words, never invented for the stage).
function stageFindingContext(stage, findings, agreement) {
  const dims = new Set(stage.rule_ids.map((id) => RULE_BY_ID[id].dimension));
  const points = []; let note = '';
  for (const f of findings) {
    if (!f.dimensions.some((d) => dims.has(d.dimension))) continue;
    for (const d of f.dimensions) if (dims.has(d.dimension)) points.push(cap(d.gap));
    const a = agreement?.[f.id];
    if (a?.note) note = a.note;
  }
  return { points: [...new Set(points)], note };
}
export function buildGuidance({ agreed, changes, findings, agreement, situation, pilot, solutions }) {
  ...
  const help = {};
  for (const stage of solutions) {
    const items = agreed.interventions.filter((i) => i.selected && stage.rule_ids.includes(i.rule_id));
    const rules = items.map((i) => ruleGuidance(i, RULE_BY_ID[i.rule_id], agreed));
    const ctx = stageFindingContext(stage, findings, agreement);
    const conditions = [...new Map(rules.flatMap((r) => r.validation.map((x) => [x, x]))).values()];
    const talking = [
      ctx.points.length
        ? `Why it matters here: you told me ${join(ctx.points.map(lower))}${ctx.note ? ` — and you said "${ctx.note}"` : ''}.`
        : (stage.preserved.length ? `Why it matters here: your ${join(stage.preserved.map((p) => lower(p.label)))} already work${stage.preserved.length === 1 ? 's' : ''} well — this stage is about reuse, not a gap.` : ''),
      `What we'd actually do: ${stage.sentence}`,
      `Why that's ${priority}: ${STAGE_EFFECTS[stage.id]}.${rules.length ? ` We'd see it in ${lower([...new Set(rules.flatMap((r) => r.measurement))][0] || 'the weekly outcome view')}.` : ''}`,
      stage.preserved.length && stage.rule_ids.length ? `What stays: ${join(stage.preserved.map((p) => lower(p.label)))} works — we plug into it, we don't replace it.` : '',
      conditions.length ? `Say honestly: ${conditions.map((x) => lower(clientSafe(x))).join('; ')} — confirmed in the first days, not promised now.` : '',
    ].filter(Boolean);
    help[stage.id] = { talking_points: talking, implementation: rules, questions: questionsFor(rules, agreed) };
  }
...
export function buildConclusion({ session, base, agreed, conclusion: stored } = {}) {
  ...
  const out = {
    ...
    changes,
    solutions: buildSolutionSequence(agreed, situation),
...
export function presentationPayload(conclusion) {
  ...
  const screens = [
    ...
    {
      // At most three cards, in implementation order (foundations/connect →
      // identify → progress & measure), not the three highest-ranked
      // problems: a short heading and the plain-English action, from the
      // AGREED deployment for this agency.
      id: 'help', title: 'How NOVUS would help', subtitle: c.mode === 'PILOT' ? 'What we would put in place for you' : 'What we would need to confirm first',
      cards: c.solutions.map((st) => ({ id: st.id, heading: st.heading, sentence: st.sentence_polished || st.sentence })),
