...
function questionsFor(group, rules, agreed, changes) {
...
export function buildGuidance({ agreed, changes, findings, agreement, situation, pilot }) {
  const priority = situation.objective.priority_label ? lower(situation.objective.priority_label) : 'more from the sales side';
  const help = {};
  for (const g of changes.groups.slice(0, 3)) {
    const finding = findings.find((f) => f.id === g.id);
    const a = finding ? agreement?.[finding.id] : null;
    const items = agreed.interventions.filter((i) => i.selected && g.rules.some((r) => r.rule_id === i.rule_id));
    const rules = items.map((i) => ruleGuidance(i, RULE_BY_ID[i.rule_id], agreed));
    const talking = [
      `Why it matters here: ${finding ? `you told me ${join(g.problem_points.map(lower))}${a?.note ? ` — and you said "${a.note}"` : ''}.` : `the intelligence work depends on it: ${join(g.problem_points.map(lower))}.`}`,
      `What we'd actually do: ${join(items.map((i) => lower(RULE_BY_ID[i.rule_id].spoken_change)))} — NOVUS does the finding and the dating; your team do the conversations.`,
      `Why that's ${priority}: ${g.effect}${g.measured_by.length ? ` We'd see it in ${lower(g.measured_by[0])}.` : ''}`,
      g.preserve.length ? `What stays: ${join(g.preserve.map(lower))} works — we plug into it, we don't replace it.` : (changes.preserved.length ? `Nothing here is replaced; your existing ${join(changes.preserved.map((p) => lower(p.label)))} stays as it is.` : ''),
      g.conditions.filter((x) => x.kind !== 'delivery').length ? `Say honestly: ${g.conditions.filter((x) => x.kind !== 'delivery').map((x) => lower(clientSafe(x.text))).join('; ')} — confirmed in the first days, not promised now.` : '',
    ].filter(Boolean);
    help[g.id] = { talking_points: talking, implementation: rules, questions: questionsFor(g, rules, agreed, changes) };
  }
...
export function buildConclusion({ session, base, agreed, conclusion: stored } = {}) {
  ...
  out.guidance = buildGuidance({ agreed, changes, findings, agreement, situation, pilot: out.pilot });
  if (state.polish) {
...
export function presentationPayload(conclusion) {
  ...
  const screens = [
    ...
    {
      // At most three cards: a short heading and ONE sentence on what we'd
      // actually implement, from the agreed findings and feasible rules.
      ...
      cards: c.changes.groups.slice(0, 3).map((g) => ({
        id: g.id,
        heading: GROUP_SOLUTION_HEADINGS[g.id] || g.short,
        sentence: `We'd ${join(g.rules.filter((r) => ['FEASIBLE', 'FEASIBLE_WITH_FOUNDATION'].includes(r.feasibility)).concat(g.rules.filter((r) => !['FEASIBLE', 'FEASIBLE_WITH_FOUNDATION'].includes(r.feasibility))).slice(0, 2).map((r) => lower(r.spoken_change)))}.`,
      })),
