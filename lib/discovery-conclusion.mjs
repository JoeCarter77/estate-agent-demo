...
const COMPONENT_DEFS = Object.freeze({
  find_database: { rules: ['I3'], stage: 'identify', heading: 'Find existing opportunities', plan_heading: 'Start finding existing opportunities', plan_phrase: 'identifying the first historical contacts and previous valuations worth approaching again' },
  find_enquiries: { rules: ['I1'], stage: 'identify', heading: 'Spot the sellers in new enquiries', plan_heading: 'Start spotting the sellers in new enquiries', plan_phrase: 'reading every incoming enquiry for buyers who also have a property to sell' },
  recognise: { rules: ['I2'], stage: 'identify', heading: 'Recognise new opportunities', plan_heading: 'Start recognising renewed activity', plan_phrase: 'recognising when existing contacts show new activity and may be back in the market' },
  recognise_enquiries: { rules: ['I2'], stage: 'identify', heading: 'Recognise returning customers', plan_heading: 'Start recognising returning customers', plan_phrase: 'connecting new enquiries with what you already hold on the customer' },
  business: { rules: ['F3', 'F4', 'I4', 'F5', 'I5'], stage: 'progress', heading: 'Turn opportunities into business', plan_heading: 'Progress the opportunities', plan_phrase: 'getting the opportunities to your team and following them through' },
  next_steps: { rules: ['F3', 'F4'], stage: 'identify', heading: 'Give every opportunity a next step', plan_heading: 'Put a next step on every opportunity', plan_phrase: 'a dated next step with a named owner on every seller who isn\'t ready yet' },
  prioritise: { rules: ['I4'], stage: 'identify', heading: 'Start with the likeliest to instruct', plan_heading: 'Order the day by likelihood to instruct', plan_phrase: 'ordering the team\'s day by who is most likely to instruct' },
  track: { rules: ['F5', 'I5'], stage: 'progress', heading: 'Track what turns into business', plan_heading: 'Track what turns into business', plan_phrase: 'tracking every opportunity through to valuation and instruction' },
  track_outcomes: { rules: ['F5'], stage: 'identify', heading: 'Track every opportunity', plan_heading: 'Start tracking every opportunity', plan_phrase: 'recording every seller opportunity and following it through to valuation and instruction' },
  learn: { rules: ['I5'], stage: 'progress', heading: 'Learn what works', plan_heading: 'Review what works', plan_phrase: 'reviewing what actually produced valuations and instructions' },
  capture: { rules: ['F1'], stage: 'identify', heading: 'Record what your team hear', plan_heading: 'Start recording what your team hear', plan_phrase: 'recording the selling situation on every conversation' },
  history: { rules: ['F2'], stage: 'identify', heading: 'Put the history in one place', plan_heading: 'Put the customer history in one place', plan_phrase: 'bringing each customer\'s history into one place' },
});
...
function componentSentence(id, sel, ctx) {
  const { crm, situation, strengths } = ctx;
  const has = (r) => sel.has(r);
  const n = (entry) => entry.display.replace(' a month', '');
  if (id === 'find_database') return `Go through ${situation.database_size.known ? `the ${n(situation.database_size)}` : 'the contacts'} ${crm ? `in ${crm}` : 'in your database'} to identify the previous valuations and past applicants worth approaching again`;
  if (id === 'find_enquiries') return `Read ${situation.enquiries_per_month.known ? `the ${n(situation.enquiries_per_month)} enquiries you get each month` : 'every incoming enquiry'} for buyers who also have a property to sell, and check whether you're already on it`;
  if (id === 'recognise') return `Use ${crm ? `${crm}'s` : 'your'} customer history and new activity to recognise when existing contacts may be back in the market`;
  if (id === 'recognise_enquiries') return `Connect each new enquiry with ${crm ? `what ${crm} already holds on that customer` : 'their previous history'}, so a returning seller is recognised straight away`;
  if (id === 'business') {
    const progress = has('F3') || has('F4')
      ? 'give each one a dated next step with a named owner'
      : strengths.has('F3') ? 'progress them through your existing follow-up process' : 'progress them through the agreed process';
    const track = has('F5') || has('I5');
    return `Get the relevant contacts in front of your team${has('I4') ? ', starting with those most likely to instruct' : ''}, ${progress}${track ? ' and track the valuations and instructions they produce' : ''}`;
  }
  if (id === 'next_steps') return has('F3')
    ? `Give every seller who isn't ready yet a dated next step with a named owner${has('F4') ? ', and flag the follow-ups that didn\'t happen before they\'re lost' : ''}`
    : 'Flag the follow-ups that didn\'t happen before the seller is lost';
  if (id === 'prioritise') return 'Order the team\'s day by who is most likely to instruct, with the reason shown against each name';
  if (id === 'track') return has('F5')
    ? `Track every opportunity through to valuation and instruction${has('I5') ? ', and use the results to decide what to do more of' : ''}`
    : 'Review what actually produced valuations and instructions, and use it to decide what to do more of';
  if (id === 'track_outcomes') return `Record every seller opportunity${crm ? ` in ${crm}` : ''} and follow it through to valuation and instruction, with where it came from`;
  if (id === 'learn') return 'Review what actually produced valuations and instructions each month, and put more of the team\'s effort behind it';
  if (id === 'capture') return `Make sure what your team hear about selling gets recorded${crm ? ` in ${crm}` : ''}, with a prompt when it's missing`;
  if (id === 'history') return 'Put each customer\'s history in one place, whoever picks up the phone';
  return '';
}
...
function projectDescription(type, ctx) {
  const { situation, crm, outcomeNoun } = ctx;
  const db = situation.database_size.known ? `the ${situation.database_size.display}${crm ? ` in ${crm}` : ''}` : (crm ? `your existing customers in ${crm}` : 'your existing customers');
  const enq = situation.enquiries_per_month.known ? `the ${situation.enquiries_per_month.display.replace(' a month', '')} enquiries you already get each month` : 'the enquiries you already get';
  if (type === 'existing_customers') return `We'd focus on finding potential sellers within ${db} and putting an ongoing process in place to turn more of those opportunities into ${outcomeNoun}.`;
  if (type === 'incoming_demand') return `We'd focus on finding the sellers within ${enq} and putting an ongoing process in place to turn more of them into ${outcomeNoun}.`;
  if (type === 'conversion') return `We'd focus on making sure the seller opportunities you already have each get a clear next step and are followed through, so more of them turn into ${outcomeNoun}.`;
  if (type === 'visibility') return 'We\'d focus on tracking every seller opportunity through to valuation and instruction, so you can see what is actually working and put more effort behind it.';
  return 'We\'d focus on making sure what your team hear about selling gets recorded and followed up, so fewer sellers slip away between conversations.';
}

// The owner's ambition, in their own words, or the objective they chose.
// Never a number we supplied, never a promise.
function projectAmbition(situation) {
  const o = situation.objective.outcome;
  const said = text(o?.text).replace(/[.!\s]+$/, '').slice(0, 160);
  if (said) return `Built around your ambition: ${/^I\b/.test(said) ? said : lower(said)}.`;
  if (situation.objective.priority_label && !['other'].includes(situation.objective.priority)) return `Built around your aim of ${lower(situation.objective.priority_label)}.`;
  return '';
}
...
export function buildProject(agreed, situation) {
  ...
  for (const id of def.components) {
    ...
    let sentence = componentSentence(id, sel, ctx);
    ...
    components.push({
      id, heading: cd.heading, sentence: `${sentence}.`, stage: cd.stage,
  ...
  return {
    ...
    ambition: projectAmbition(situation),
...
export function buildProjectGuidance({ agreed, project, findings, agreement, situation }) {
  ...
  const say_aloud = [
    ...
    `And it's not a one-off: once it's running we keep finding the opportunities, get them to your team and look at what actually turns into ${project.description.includes('instructions.') ? 'instructions' : 'valuations'}, so it gets sharper over time. That's all aimed at ${objectiveText}.`,
  ].filter(Boolean).join(' ');
  ...
  return {
    ...
    components: project.components.map((c) => ({ id: c.id, heading: c.heading, implementation: c.rule_ids.map((id) => byId.get(id)) })),
...
export function presentationPayload(conclusion) {
  ...
  const screens = [
    ...
    {
      // ONE personalised commercial project: a headline, one or two
      // sentences, the owner's ambition, and the two or three components it
      // is delivered through — generated for this agency, never a feature list.
      id: 'help', title: 'Here\'s what I\'d propose for your agency.', subtitle: c.mode === 'PILOT' ? '' : 'What we would need to confirm first',
      headline: pr ? pr.title : '',
      description: pr ? (pr.description_polished || pr.description) : '',
      ambition: pr ? pr.ambition : '',
      components: pr ? pr.components.map((st) => ({ id: st.id, heading: st.heading, sentence: st.sentence_polished || st.sentence })) : [],
