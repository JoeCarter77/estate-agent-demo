...
// Their current commercial strategy (C12*), as private supporting answers.
function strategyLines(st) {
  if (!st) return [];
  const q = (label, values) => (values?.length ? [`${label}: ${join(values.map((x) => `"${lower(x)}"`))}`] : []);
  return [
    ...q("How they'd get more today", st.approach), ...q('How they would pick who to speak to', st.selection),
    ...q('How consistently it happens', st.consistency), ...q('What their system does', st.existing),
    ...(st.output_per_month !== null && st.output_per_month !== undefined ? [`It produces about ${st.output_per_month} valuations a month`] : []),
    ...q('What they would change', st.change), ...q('Existing customers exhausted?', st.exhausted),
    ...q('Would they know what it produced', st.results), ...q('Do they think it would be enough', st.belief),
  ];
}
export function buildProjectGuidance({ agreed, project, findings, agreement, situation }) {
  ...
  return {
    ...
    supporting_answers: [
      ...
      ...strategyLines(agreed.objective.strategy),
      ...ctx.said,
...
// ── 7c. PRIVATE personalised NOVUS explanation ────────────────────────────
// What Joe says when he explains NOVUS — not an elevator pitch. Built ONLY
// from the agreed project, limited to the rules in the pilot scope (so it
// can never promise something the project slide, roadmap or scope left out),
// in five moves: alongside what they have → where the value is → how we'd go
// after it → what stays → their objective. Anything that still needs a
// technical check is said as a check, not a promise. Never in the
// presentation payload.
const STRENGTH_LINES = Object.freeze([
  { dims: ['F3', 'F4'], text: "your existing follow-up process sounds like something you've already got working" },
  { dims: ['F3'], text: 'a follow-up already gets set on every seller who isn\'t ready' },
  { dims: ['I1'], text: 'your team are already good at spotting sellers in new enquiries' },
  { dims: ['I3'], text: "you're already working the database properly" },
  { dims: ['F5'], text: 'you can already see what turns into business' },
  { dims: ['F1'], text: 'the selling information already gets recorded' },
]);
const STRENGTH_ORDER = Object.freeze({
  existing_customers: ['F3', 'I1', 'F5'], incoming_demand: ['F3', 'I3', 'F1'], conversion: ['I1', 'I3', 'F1'], visibility: ['F3', 'I1', 'I3'], capture: ['F3', 'I1'],
});
const DATA_DEPENDENT = new Set(['I2', 'I3', 'F2']);
const wordsIn = (t) => text(t).split(/\s+/).filter(Boolean).length;

export function buildExplanation({ agreed, project, situation, pilot }) {
  if (!project) return null;
  const scope = new Set((pilot?.scope_rule_ids?.length ? pilot.scope_rule_ids : project.rule_ids).filter((id) => project.rule_ids.includes(id)));
  if (!scope.size) return null;
  const has = (id) => scope.has(id);
  const items = new Map(agreed.interventions.filter((i) => i.selected).map((i) => [i.rule_id, i]));
  const strong = new Set(Object.values(agreed.assessments).filter((a) => a.evidence_status === 'EXISTING_STRENGTH').map((a) => a.dimension));
  const name = situation.owner_first_name;
  const crm = situation.crm.known ? situation.crm.display : '';
  const noun = project.outcome_noun;
  const type = project.type;

  // 1. alongside what they already have
  const alongside = `So the way NOVUS would work for you${name ? `, ${name},` : ''} is alongside the ${crm ? `${crm} setup` : 'systems and team'} you've already got.`;

  // 2. where the commercial value is — only what is in scope
  let where = '';
  if (type === 'existing_customers') {
    where = has('I3')
      ? `From what we've discussed, the areas I'd be interested in are the previous valuations and existing customers sitting in your database${has('I2') ? ", particularly people whose circumstances may have changed without necessarily coming back to the team's attention" : ''}.`
      : "From what we've discussed, the area I'd be interested in is your existing customers, particularly people whose circumstances may have changed without necessarily coming back to the team's attention.";
  } else if (type === 'incoming_demand') {
    where = `From what we've discussed, the area I'd be interested in is the sellers who are already contacting you${has('I1') ? ` — buyers${situation.enquiries_per_month.known ? ` in your ${situation.enquiries_per_month.display.replace(' a month', '')} enquiries a month` : ''} who mention they've got somewhere to sell` : ''}${has('I2') ? ', and past customers who come back and enquire again' : ''}.`;
  } else if (type === 'conversion') {
    const parts = [has('F3') ? "people who aren't ready to sell just yet" : '', has('F4') ? "the follow-ups that don't always happen" : '', has('I4') ? "knowing who's most worth calling first" : ''].filter(Boolean);
    where = `From what we've discussed, the area I'd be interested in is the seller opportunities your team already hear about${parts.length ? ` — particularly ${join(parts)}` : ''}.`;
  } else if (type === 'visibility') {
    where = "From what we've discussed, the area I'd be interested in is knowing which of your opportunities actually turn into valuations and instructions, so the effort goes behind what's working.";
  } else {
    where = "From what we've discussed, the area I'd be interested in is what your team already hear about selling, which isn't always making it into the system.";
  }

  // 3. how we'd go after THOSE opportunities
  let how = [];
  if (type === 'existing_customers') how = [has('I3') ? 'use the information we can reliably access to identify suitable people worth speaking to again' : '', has('I2') ? 'connect previous customer history with new activity where possible' : '', `get those opportunities in front of your team${has('I4') ? ', starting with the people most likely to instruct' : ''}`];
  else if (type === 'incoming_demand') how = [has('I1') ? 'read each enquiry for those selling signals' : '', has('I1') ? 'check whether your team already know about them' : '', has('I2') ? 'connect a returning customer with what you already hold on them' : '', 'get the new ones in front of your team while they\'re still warm'];
  else if (type === 'conversion') how = [has('F3') ? 'make sure each of those has a clear next step and someone responsible for it' : '', has('F4') ? 'flag the follow-ups that slip before the seller is lost' : '', has('I4') ? 'put the people most likely to instruct at the top of the list' : '', has('F5') || has('I5') ? 'track which of them turn into valuations' : ''];
  else if (type === 'visibility') how = [has('F5') ? 'follow each seller opportunity through to valuation and instruction' : '', has('I5') ? 'review each month what actually produced business, so the team do more of it' : '', has('F3') || has('F4') ? 'make sure each opportunity has a next step' : ''];
  else how = [has('F1') ? "make sure the selling situation gets recorded as it's heard" : '', has('F2') ? "put each customer's history in one place" : '', 'get those opportunities followed up'];
  how = how.filter(Boolean);
  const howLine = `We'd ${join(how)}.`;

  // technical checks still to be done, said as checks
  const access = text(agreed.facts?.crm_access);
  const unverified = [...scope].filter((id) => { const i = items.get(id); return i && (i.feasibility === 'REQUIRES_ASSESSMENT' || i.delivery_status === 'REQUIRES_ASSESSMENT' || (DATA_DEPENDENT.has(id) && !['export', 'api'].includes(access))); });
  let check = '';
  if (unverified.some((id) => DATA_DEPENDENT.has(id))) check = `The first step would be checking what we can reliably get out of ${crm || 'your CRM'}, so I wouldn't want to promise anything on the historical side until we've seen that.`;
  else if (unverified.includes('I1')) check = "The first step would be connecting to your enquiries, so I'd want to confirm that before promising anything.";
  else if (unverified.length) check = "There are a couple of things we'd need to check first, so I wouldn't want to over-promise until we have.";

  // 4. what stays — strengths next to this project, never re-sold
  const order = STRENGTH_ORDER[type] || [];
  const kept = [];
  for (const lead of order) {
    const line = STRENGTH_LINES.find((l) => l.dims[0] === lead && l.dims.every((d) => strong.has(d)) && !l.dims.some((d) => scope.has(d)));
    if (line && kept.length < 2) kept.push(line);
  }
  const keepLine = (list) => (list.length ? `${cap(join(list.map((l) => l.text)))}, so we wouldn't be looking to replace ${list.length > 1 ? 'those' : 'that'}.` : '');
  let keep = keepLine(kept);

  // 5. their objective
  const OBJECTIVE = {
    existing_customers: `The objective would be to generate more ${noun} from the demand you've already got, rather than simply relying on more incoming enquiries.`,
    incoming_demand: `The objective would be to turn more of the enquiries you already get into ${noun}, rather than relying on generating more of them.`,
    conversion: `The objective would be to make sure more of the sellers you already hear about turn into ${noun}.`,
    visibility: `The objective would be to put the team's effort behind what's genuinely producing ${noun}, on evidence rather than impression.`,
    capture: 'The objective would be to stop losing the sellers your team already hear about.',
  };
  const assemble = () => [alongside, where, [howLine, check].filter(Boolean).join(' '), keep, OBJECTIVE[type]].filter(Boolean);
  let paragraphs = assemble();
  // ~100–160 spoken words: the second strength is the first thing to go.
  if (wordsIn(paragraphs.join(' ')) > 160 && kept.length > 1) { keep = keepLine(kept.slice(0, 1)); paragraphs = assemble(); }
  const ambition = text(project.desired_outcome?.text).replace(/[.!\s]+$/, '');
  if (ambition && !/^(I|We)\b/.test(ambition) && ambition.length <= 80 && wordsIn(paragraphs.join(' ')) < 145) paragraphs.push(`That's how we'd work towards ${lower(ambition)}.`);
  const out = paragraphs.join('\n\n');
  return { provisional: false, text: out, paragraphs, words: wordsIn(out), project_type: type, rule_ids: [...scope], checks: unverified };
}

// The SHORT provisional answer to "so what does NOVUS actually do?" asked
// mid-discovery: only what has been established so far — a confirmed gap is
// named, a provisional one is hedged, and nothing unknown is claimed.
const PROVISIONAL_ORDER = Object.freeze(['I3', 'I2', 'I1', 'I4', 'F3', 'F4', 'F5', 'I5', 'F1', 'F2']);
const PROVISIONAL_AREA = Object.freeze({
  I3: 'the customers already sitting in your database', I2: "people whose circumstances change without coming back to the team's attention",
  I1: "the buyers who mention they've got somewhere to sell", I4: "which of your contacts are most worth calling first",
  F3: "the sellers who aren't ready just yet", F4: "the follow-ups that don't always happen", F5: 'seeing which opportunities turn into valuations',
  I5: 'learning what actually produces valuations', F1: 'what your team hear about selling', F2: "picking up a customer's history",
});
export function provisionalExplanation({ diagnosis, situation }) {
  const crm = situation.crm.known ? situation.crm.display : '';
  const a = diagnosis.assessments;
  const gaps = PROVISIONAL_ORDER.filter((d) => ['CONFIRMED', 'PROVISIONAL'].includes(a[d]?.evidence_status)).slice(0, 2);
  const firm = gaps.filter((d) => a[d].evidence_status === 'CONFIRMED');
  const soft = gaps.filter((d) => a[d].evidence_status === 'PROVISIONAL');
  const strengths = PROVISIONAL_ORDER.filter((d) => a[d]?.evidence_status === 'EXISTING_STRENGTH');
  const line = STRENGTH_LINES.find((l) => l.dims.every((d) => strengths.includes(d)));
  const paragraphs = [
    `Put simply, NOVUS works alongside ${crm || 'your CRM'} and your team — it isn't another CRM. It finds seller opportunities already sitting in your enquiries and database, and gets them to your team to act on.`,
    firm.length ? `From what you've told me so far, where I'd look first is ${join(firm.map((d) => PROVISIONAL_AREA[d]))}.` : '',
    soft.length ? `${firm.length ? 'There may also be something in' : 'So far, there may be something in'} ${join(soft.map((d) => PROVISIONAL_AREA[d]))}, though I'd want to understand that properly first.` : '',
    !gaps.length ? "I don't want to guess where that would matter for you yet — that's really what the rest of this conversation is for." : '',
    line ? `And ${line.text} — we wouldn't replace that.` : '',
    gaps.length ? "Give me a bit longer and I can be much more specific." : '',
  ].filter(Boolean);
  const out = paragraphs.join('\n\n');
  return { provisional: true, text: out, paragraphs, words: wordsIn(out), dimensions: gaps };
}
...
export function buildConclusion({ session, base, agreed, conclusion: stored } = {}) {
  ...
  // PRIVATE: the personalised explanation (from the SAME agreed project and
  // pilot scope — rebuilt whenever the owner corrects a finding) and the
  // short provisional answer for mid-discovery. Neither is presented.
  out.explanation = buildExplanation({ agreed, project, situation, pilot: out.pilot });
  out.explanation_provisional = provisionalExplanation({ diagnosis: base, situation });
