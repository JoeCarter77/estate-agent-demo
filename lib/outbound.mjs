...
// New probes store the canonical listing text in property_address. Older
// historical rows also carry the convenience property_street column. OUTBOUND
// must not reject an otherwise-sendable lead just because that duplicated
// convenience field was never populated, so use it when present and derive a
// short street reference from the canonical address otherwise.
export function resolvePropertyStreet(probe) {
  const explicit = String(probe?.property_street ?? '').trim();
  if (explicit) return explicit;
  const address = String(probe?.property_address ?? '').trim();
  if (!address) return '';
...
function eligibilityReasons({ agency, probe, personalisation, demo, duplicateReasons = [] }) {
  ...
  if (!demo) reasons.push('missing DEMOS match for exact agency_id + probe_id');
    ...
    // A property image is presentation polish, not an acquisition prerequisite.
    // The demo compiler deliberately treats unavailable/blank images as usable,
    // so OUTBOUND must not re-introduce a stricter image gate afterwards.
  ...
  if (!nonblank(resolvePropertyStreet(probe))) reasons.push('missing property_street_or_address');
