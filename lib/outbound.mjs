...
// ONE definition of the property street reference, shared with the PROBES
// self-heal and the backlog recovery. A stored property_street always wins; a
// blank one derives from property_address rather than killing the lead.
import { hasPropertyStreet, resolvePropertyStreet } from './property-reference.mjs';
...
const SOURCE_SPECS = {
  ...
  // NEITHER ADDRESS COLUMN IS REQUIRED AS A HEADER. PROBES has carried two over
  // its life — the legacy hand-maintained property_street and the
  // property_address every probe created by the current code actually stores —
  // and demanding either one by name breaks a workbook that only has the other.
  // The street reference is resolved per-row instead (resolvePropertyStreet:
  // stored value first, derived from the address only when blank), so a
  // workbook with either column, or both, works unchanged. This is the fix for
  // every new probe dying at this last gate with "missing property_street"
  // while holding a perfectly usable property_address.
  PROBES: ['agency_id', 'probe_id'],
  PERSONALISATION: ['agency_id', 'probe_id', 'email_observation', 'email_commercial_hook'],
...
function eligibilityReasons({ agency, probe, personalisation, demo, duplicateReasons = [] }) {
  ...
  if (!agency) reasons.push('missing AGENCIES match for exact agency_id');
    ...
    // Outbound (like probing) now requires a confirmed-good address: RISKY
    // bounces too often to risk Instantly deliverability, so only an exact
    // VALID clears this gate.
    const verificationStatus = String(agency.email_verification_status || '').trim().toUpperCase();
    if (verificationStatus !== 'VALID') {
      reasons.push(`email_verification_status_not_VALID:${verificationStatus || '(blank)'}`);
    }
    if (!nonblank(agency.clean_agency_name)) reasons.push('missing clean_agency_name');
  }

  // The question is "can this probe supply a street reference at all?", not
  // "is one particular legacy column populated?". A nonblank property_street
  // answers yes; so does a property_address the street derives from.
  if (!hasPropertyStreet(probe)) reasons.push('missing property reference (no property_street and no usable property_address)');
...
function compiledFields({ agency, probe, personalisation, demo }) {
  return {
    ...
    // The stored value verbatim when there is one; the derived street only
    // when the cell is blank. This is the value Instantly's {{property_street}}
    // custom variable renders, so it must never be invented — see
    // lib/property-reference.mjs.
    property_street: resolvePropertyStreet(probe),
