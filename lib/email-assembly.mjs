...
// Generated placeholder sentinels are never prospect copy. Keep the one
// intentional merge field out of this check by applying it only to stored
// Personalisation fields, before the greeting is assembled.
export function hasUnresolvedPlaceholder(value) {
  ...
  return /<\s*[^<>]+\s*>/.test(candidate)
    || /\{\{\s*[^{}]+\s*\}\}/.test(candidate)
    || /\[\s*(?:UNKNOWN|UNRESOLVED|PLACEHOLDER|TBD|TBC|NULL|N\/?A)\s*\]/.test(candidate)
    || /\b(?:UNKNOWN|UNRESOLVED|PLACEHOLDER|TBD|TBC|NULL|N\/?A)\b/.test(candidate)
...
function namesAProperty(segment) {
  ...
  if (/^\d+[A-Za-z]?(?:[-/]\d+[A-Za-z]?)?\s+\S/.test(trimmed)) return true; // 14 Perry Street
...
export function propertyReference(propertyAddress) {
  ...
  if (hasUnresolvedPlaceholder(propertyAddress)) return '';
  if (/^\d+[A-Za-z]?$/.test(segments[0])) return '';          // "4" / "4A" is not an address
  if (SUBDIVISION_RE.test(segments[0]) && !segments[1]) return '';
...
export function emailContractViolations(personalisation) {
  ...
  if (!propertyReference(p.property_address)) violations.push('missing_property_address');
  ...
  const prospectFields = [
    p.enquiry_date, p.property_address, p.commercial_consequence,
    ...(normaliseVariant(p.email_variant) === 'normal' ? [p.fair_observation, p.main_finding] : []),
    ...(text(p.wider_observation) ? [p.wider_observation, p.wider_consequence] : []),
  ];
  if (prospectFields.some(hasUnresolvedPlaceholder)) violations.push('unresolved_placeholder');
  return violations;
}
