...
const COMMERCIAL = [
  ...
  q({
    id: 'C8', section: 'commercial', role: 'context', key: 'valuations_per_month', type: 'number', unit: 'per month', source_toggle: true,
    purpose: 'Current monthly valuation volume — the baseline.',
    primary: 'How many market appraisals are you doing a month at the moment?', simpler: 'How many valuations a month, roughly?', example: '',
  }),
  q({
    id: 'C9', section: 'commercial', role: 'context', key: 'instructions_per_month', type: 'number', unit: 'per month', source_toggle: true,
  ...
  q({
    id: 'C10', section: 'commercial', role: 'context', key: 'fee_per_instruction', type: 'number', unit: '£', source_toggle: true,
  ...
  q({
    id: 'C11', section: 'commercial', role: 'context', key: 'conversion_pct', type: 'number', unit: '%', source_toggle: true, derived_from: ['C8', 'C9'],
    purpose: 'Valuation-to-instruction conversion. Derived from C8/C9 when both are known; otherwise asked.',
  ...
  ...exploration('F2', {
    ...
    cause: {
      ...
      options: [
        ...
        { value: 'history_hard', label: 'The CRM makes history hard to read' }, { value: 'not_recorded', label: 'Not much gets recorded in the first place' },
        { value: 'other', label: 'Something else (note it)' },
      ],
    },
    consequence: {
...
export const QUESTIONS = Object.freeze([...COMMERCIAL, ...FOUNDATIONS, ...INTELLIGENCE]);
...
// A question without show_when is always visible.
export function isVisible(question, answers) {
  ...
  if (!ref || !answer || answer.skipped) return false;
  if (rule.levels) return rule.levels.includes(ref.role === 'primary' ? effectiveLevel(ref.id, answers) : levelOf(ref, answer));
...
export function visibleQuestions(answers, section = null) {
  return QUESTIONS.filter((question) => (!section || question.section === section) && isVisible(question, answers));
}
...
export function snapshotFor(answers) {
  ...
  for (const id of Object.keys(answers || {})) {
    ...
    out[id] = {
      primary: question.primary, section: question.section, dimension: question.dimension || '', role: question.role,
...
export const _internal = { exploration, q };
