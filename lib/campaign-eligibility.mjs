...
export const RULES = Object.freeze([
  ...
  { code: 'PROBE_NOT_CLOSED', severity: 'WARN', when: (c) => c.policy.requires_probe && c.facts.probe_sent && !c.facts.probe_complete },
  { code: 'NO_PERSONALISATION', severity: 'WARN', when: (c) => c.policy.requires_probe && c.facts.probe_complete && !c.facts.personalisation_ready },
]);
...
export function deriveFacts(candidate, { nowMs = Date.now() } = {}) {
  ...
  return {
    ...
    active_conversation: ['REPLIED_NEEDS_HUMAN', 'MEETING_INTENT', 'DEMO_REQUESTED', 'MANUAL_REPLY_SENT_WAITING', 'DEMO_FOLLOWUP_SENT', 'DEMO_ENGAGED', 'DEMO_OPENED', 'DEMO_SENT_UNOPENED'].includes(stage)
      || classifications.some((c) => c === 'POSITIVE_SEND_DEMO' || c === 'POSITIVE_MEETING')
    ...
    positive_reply: classifications.some((c) => c === 'POSITIVE_SEND_DEMO' || c === 'POSITIVE_MEETING') || events.some((row) => ['LEAD_INTERESTED', 'LEAD_MEETING_BOOKED'].includes(upper(row.event_type))),
