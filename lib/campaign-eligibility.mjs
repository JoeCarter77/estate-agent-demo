...
import { lockedPreset } from './campaign-presets.mjs';
...
export const REASON_LABEL = Object.freeze({
  ...
  HAS_PROBE: 'Agency has a probe (belongs in the probe-led cohort)',
  IN_OTHER_COHORT: 'Already in the other arm of this experiment',
  // positive facts
...
export const RULES = Object.freeze([
  ...
  { code: 'INVALID_EMAIL_FORMAT', severity: 'BLOCK', when: (c) => c.facts.locked_campaign && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(c.facts.email) },
  ...
  // Experiment arms never overlap, whatever the other campaign's status.
  { code: 'IN_OTHER_COHORT', severity: 'BLOCK', when: (c) => c.facts.in_other_cohort_campaign },
  // Sales state.
  ...
  { code: 'HAS_PROBE', severity: 'BLOCK', when: (c) => c.facts.forbids_probe && c.facts.probe_sent },
...
export function deriveFacts(candidate, { nowMs = Date.now() } = {}) {
  ...
  const preset = lockedPreset(candidate.campaign?.campaign_type);
  ...
  return {
    ...
    active_conversation: ['REPLIED_NEEDS_HUMAN', 'MEETING_INTENT', 'DEMO_REQUESTED', 'MANUAL_REPLY_SENT_WAITING', 'DEMO_FOLLOWUP_SENT', 'DEMO_ENGAGED', 'DEMO_OPENED', 'DEMO_SENT_UNOPENED'].includes(stage)
      || classifications.some((c) => c === 'POSITIVE_SEND_DEMO' || c === 'POSITIVE_MEETING' || c === 'CALL_REQUESTED' || c === 'POSITIVE_INTEREST')
    ...
    positive_reply: classifications.some((c) => c === 'POSITIVE_SEND_DEMO' || c === 'POSITIVE_MEETING' || c === 'CALL_REQUESTED' || c === 'POSITIVE_INTEREST') || events.some((row) => ['LEAD_INTERESTED', 'LEAD_MEETING_BOOKED'].includes(upper(row.event_type))),
    probe_sent: probeSent,
    // A locked preset that relies on the probe (probe-call, founding B) gets
    // the strict probe rules; founding A forbids a probe to keep the arms clean.
    locked_campaign: Boolean(preset),
    probe_call_campaign: Boolean(preset?.requires_probe),
    forbids_probe: Boolean(preset?.forbids_probe),
    in_other_cohort_campaign: Boolean(preset?.cohort) && memberships.some((row) => text(row.campaign_id) !== text(candidate.campaign?.campaign_id)
      && upper(row.member_status) !== 'EXCLUDED' && lockedPreset(row.campaign_type)?.cohort === preset.cohort),
