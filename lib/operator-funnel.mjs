...
import { ACTION_RECOMMENDER, actionQueue, deriveExpectedActions, effectiveActionOwner, isManualSalesAction } from './acquisition-actions.mjs';
...
export const ACQUISITION_OPTIONAL_TABS = Object.freeze(['SALES_MESSAGES', 'ACTIONS', 'COMMUNICATIONS']);
...
export function buildAgencyEvidence(tables, { now = new Date().toISOString(), execution = null } = {}) {
  ...
  const calls = group(
    records(tables.COMMUNICATIONS, 'communication_id').filter((row) =>
      text(row.direction).toLowerCase() === 'outbound'
      && text(row.communication_type).toLowerCase() === 'sales_call'
    ),
    'agency_id',
  );
...
export function buildAcquisitionDashboard(tables, { now = new Date().toISOString(), actionsAvailable = Boolean(tables.ACTIONS?.header?.length), execution = null } = {}) {
  ...
  const leads = evidenceRows.map((evidence) => {
    ...
    lead.call_history = evidence.calls
      .slice()
      .sort((a, b) => ts(b.occurred_at) - ts(a.occurred_at))
      .slice(0, 20)
      .map((row) => {
        let analysis = null;
        try { analysis = text(row.ai_summary) ? JSON.parse(text(row.ai_summary)) : null; } catch { analysis = null; }
        return {
          communication_id: text(row.communication_id),
          occurred_at: text(row.occurred_at),
          call_status: text(row.call_status),
          duration_seconds: Number(row.duration_seconds) || 0,
          recording_available: Boolean(text(row.recording_reference)),
          transcript: text(row.transcript),
          ai_model: text(row.ai_model),
          analysis,
        };
      });
