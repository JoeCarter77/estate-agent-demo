...
import { ACTION_RECOMMENDER, actionQueue, deriveExpectedActions, effectiveActionOwner, isManualSalesAction, isSortLeadAction } from './acquisition-actions.mjs';
...
export const ACQUISITION_OPTIONAL_TABS = Object.freeze(['SALES_MESSAGES', 'ACTIONS', 'CONTACTS']);
...
function blankLead(agency) {
  return {
    ...
    current_state: 'UNKNOWN', current_state_reason: '', priority: null, needs_human: false, needs_sort: false,
...
export function buildAgencyEvidence(tables, { now = new Date().toISOString(), execution = null } = {}) {
  ...
  const contacts = group(records(tables.CONTACTS, 'contact_id'), 'agency_id');
  ...
  return agencies.map((agency) => {
    ...
    const contactReady = Boolean(text(agency.outreach_contact_email)) && upper(agency.email_verification_status) === 'VALID';
    ...
    const evidence = {
      agency, probe, probes: probes.get(agencyId) || [], outbound: primary, otherOutbound: journeys.filter((row) => row !== primary),
      ...
      replyEvents: replies.get(agencyId) || [], contacts: contacts.get(agencyId) || [], salesMessages: sales.get(agencyId) || [],
...
export function buildFunnelMetrics(evidenceRows) {
  ...
  const positive = (e) => e.replyEvents.some((r) => ['POSITIVE_SEND_DEMO', 'CALL_REQUESTED', 'POSITIVE_MEETING', 'POSITIVE_INTEREST'].includes(upper(r.classification)));
...
export function buildAcquisitionDashboard(tables, { now = new Date().toISOString(), actionsAvailable = Boolean(tables.ACTIONS?.header?.length), execution = null } = {}) {
  ...
  const leads = evidenceRows.map((evidence) => {
    ...
    // A campaign-layer reply (no OUTBOUND journey) still belongs on its lead.
    if (!lead.reply.reply_event_id && evidence.replyEvents.length) {
      const r = latest(evidence.replyEvents, ['received_at', 'processed_at']);
      lead.reply = { reply_event_id: text(r.reply_event_id), classification: text(r.classification), confidence: text(r.confidence),
        classifier_reason: text(r.classifier_reason), suppression_type: text(r.suppression_type), text: text(r.cleaned_reply_text || r.body_text),
        received_at: text(r.received_at), reply_count: evidence.replyEvents.length, error: text(r.error), notes: text(r.notes) };
    }
    ...
    const active = evidence.actions.filter((row) => ['PENDING', 'DUE', 'IN_PROGRESS', 'SNOOZED'].includes(upper(row.action_status)))
      .map((row) => upper(row.action_status) === 'SNOOZED' && text(row.due_at) && ts(row.due_at) <= Date.parse(now)
        ? { ...row, action_status: 'DUE' } : row);
    const failed = evidence.actions.filter((row) => upper(row.action_status) === 'FAILED');
    // Completing an action removes it from `active` (correctly), but with no
    // active row left, the fallback below would re-derive the very same
    // action from unchanged evidence and show it as if Complete never
    // happened — the ledger says COMPLETED while the queue resurrects a
    // phantom projection of it. A dedupe_key marked COMPLETED must stay
    // retired here exactly as it does in reconcileActions().
    const completedKeys = new Set(evidence.actions
      .filter((row) => upper(row.action_status) === 'COMPLETED')
      .map((row) => text(row.dedupe_key)));
    const candidates = active.length ? active : expected.filter((row) => !completedKeys.has(text(row.dedupe_key)));
    ...
    lead.current_action = current ? { ...current, ...actionMetadata(current), persisted: true } : null;
    ...
    // owner is the EFFECTIVE owner (lib/acquisition-actions.mjs's execution
    // authority registry), never the raw stored column: an action type with no
    // enabled automated executor cannot be presented as NOVUS-executed just
    // because an older ledger row says so. recommended_by carries the other
    // half of the distinction — NOVUS still decides what should happen next.
    const owner = current ? effectiveActionOwner(current) : null;
    const currentMeta = actionMetadata(current);
    lead.next_action = current ? { id: current.action_id, type: current.action_type, title: currentMeta.title || '', note: currentMeta.note || '', priority: currentMeta.priority || '', interested: currentMeta.interested === true, status: current.action_status, owner, recommended_by: ACTION_RECOMMENDER, due_at: current.due_at, reason: current.reason, queue: lead.action_queue, requires_human: owner === 'JOE' } : { id: null, type: null, title: '', note: '', priority: '', status: null, owner: null, recommended_by: null, due_at: null, reason: '', queue: null, requires_human: false };
    ...
    // Sort leads is lead-pool housekeeping, not a commercial sales action —
    // kept as its own flag so it can have its own queue/tab without ever
    // being counted as "needs your attention".
    lead.needs_sort = isSortLeadAction(current);
    ...
    if (lead.needs_human && !lead.priority) {
      lead.priority = currentMeta.priority ? upper(currentMeta.priority) : current.action_type === 'CALL_PROSPECT' && text(evidence.demo?.cta_clicked_at) ? 'HIGH'
        : current.action_type === 'RESOLVE_EXCEPTION' ? 'HIGH' : 'NORMAL';
    }
    lead.action_history = evidence.actions.map((row) => ({ ...row, ...actionMetadata(row) })).sort((a, b) => ts(b.created_at) - ts(a.created_at));
  ...
  if ((legacy.counts?.orphan_reply_events || 0) > 0) {
    globalExceptions.push({ type: 'UNMATCHED_REPLY', reason: `${legacy.counts.orphan_reply_events} durable inbound REPLY_EVENTS row(s) need matching to an OUTBOUND journey` });
  }
  ...
  const counts = {
    ...
    sort_leads: leads.filter((lead) => lead.needs_sort).length,
