...
import { demoSentEvidence, resolveLifecycleStage, TERMINAL_STAGES } from './acquisition-stage.mjs';
import { deriveExpectedActions } from './acquisition-actions.mjs';
...
export function buildAgencyEvidence(tables, { now = new Date().toISOString() } = {}) {
...
export function buildAcquisitionDashboard(tables, { now = new Date().toISOString(), actionsAvailable = Boolean(tables.ACTIONS?.header?.length) } = {}) {
  const warnings = [];
  const legacy = buildOperatorLeads(tables, { now });
  const legacyByAgency = new Map(legacy.leads.map((lead) => [text(lead.agency_id), lead]));
  const evidenceRows = buildAgencyEvidence(tables, { now });
  const leads = evidenceRows.map((evidence) => {
    ...
    candidates.sort((a, b) => {
      const owner = (upper(a.action_owner) === 'JOE' ? 0 : 1) - (upper(b.action_owner) === 'JOE' ? 0 : 1);
      return owner || ts(a.due_at) - ts(b.due_at);
    });
    ...
    lead.next_action = current ? { type: current.action_type, status: current.action_status, owner: current.action_owner, due_at: current.due_at, reason: current.reason, requires_human: upper(current.action_owner) === 'JOE' } : { type: null, status: null, owner: null, due_at: null, reason: '', requires_human: false };
    lead.needs_human = Boolean(current && upper(current.action_owner) === 'JOE' && ['PENDING', 'DUE', 'IN_PROGRESS'].includes(upper(current.action_status)));
  ...
  const counts = {
    ...
    ready_to_probe: metrics.ready_to_probe, probe_observing: metrics.probe_observing,
