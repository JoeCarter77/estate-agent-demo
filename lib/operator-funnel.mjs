...
export function buildAcquisitionDashboard(tables, { now = new Date().toISOString(), actionsAvailable = Boolean(tables.ACTIONS?.header?.length), execution = null } = {}) {
  ...
  const leads = evidenceRows.map((evidence) => {
    ...
    lead.next_action = current ? { type: current.action_type, status: current.action_status, owner, recommended_by: ACTION_RECOMMENDER, due_at: current.due_at, reason: current.reason, queue: lead.action_queue, requires_human: owner === 'JOE' } : { type: null, status: null, owner: null, recommended_by: null, due_at: null, reason: '', queue: null, requires_human: false };
